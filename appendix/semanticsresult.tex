...
axiomatic specification $(\RP_{\LWW}, \Ax)$. Let $I_{\ET}$ be 
...
$\T^{i}_{\cl} = \Tx(\aexec_{i}, \viewFun_{i}(\cl))$ 
...
for any client $\cl$ we have that $\T_{\cl}^{0} = \Tx(\viewFun_{0}(\cl)) = \emptyset$. Therefore 
...
$\T_{\cl}^{i} = \Tx(\hh_{i}, \viewFun_{i})(\cl) \viewleq \Tx(\hh_{i}, \vi'_{i}) = (\T_{\cl}^{i})'$. 
...
(by the fact that $\hh_{\aexec_{i}} = \hh_{i}$, $(\T^{i}_{\cl})' = \Tx(\hh_{i}, \vi'_{i})$, 
and \cref{prop:getview.tx}), and because $I(\aexec_{i}, \cl_{i}) \subseteq (\T^{i}_{\cl})'$, 
...
\item $I(\aexec_{i+1}, \cl) \subseteq \Tx(\hh_{i+1}, \viewFun_{i+1}(\cl)) = \T_{\cl}^{i+1}$.
\end{itemize} 
