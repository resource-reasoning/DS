...
Similarly by the \cref{lem:identical-step}, there is step \((\cl_p, \vi_p, \f_p)\) from the trace of \( \ET_2 \) such that \( \cl_p = \cl_{j+2}\), \( \vi_p = \vi_{j+2} \), \( \f_p = \f_{j+2}\) and \( p < i \).
...
Therefore, there exist a new kv-stores \( \mkvs'''_{j+2}\) and a new view environment \( \viewFun'''_{j+2} \) such that:
\begin{equation}
...
\begin{equation}
    ...
    \begin{array}{@{}l}
        ...
        \land \fora{\cl \in \dom(\conf''_k\projection{2}),\ke \in (\conf''_k\projection{1})} \\
        \qquad \conf''_k\projection{2}(\cl)(\ke) = \max\Set{\conf_n\projection{2}(\cl)(\ke), \conf'_i\projection{2}(\cl)(\ke)}
\end{array}
\end{equation}
...
\[
    ...
    \end{array}
\]
\sx{Here it is not true.}
so that we have:
\[
    \ET_1 \cap \ET_2 \vdash ( \conf''_{k-1}\projection{1}, \conf''_{k-1}\projection{2}(\cl_{i}) )  \csat \f_{i}, \max\Set{\conf'_{i+1}\projection{2}(\cl_{i}), \conf_{n}\projection{2}(\cl_{i})}
\]
Therefore \cref{equ:et1-et2-in-et12} holds when \( m = i + 1\) by appending the i-\emph{th} step from \( \ET_2 \) to the end of the trace shown in \cref{equ:ih-for-merge-two-trace}:
\[
\begin{array}{@{}l}
