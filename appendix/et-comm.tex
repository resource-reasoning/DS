...
\caseB{\(n = 0\)} The \cref{equ:et12-in-et1-et2} holds when \( n = 0 \), because all initial configurations \( \conf_0 \) are included in the \( \Confs(\ET_1)\) and \( \Confs(\ET_2) \) by the definition of the \( \Confs \) function (\cref{def:cm}).
\caseI{\(n = i+1\)} Suppose the \cref{equ:et12-in-et1-et2} holds when \( n = i \) for some \( i \).
