...
\begin{mathpar}
    \inferrule[Put]{%
        ...
        \txidCOPS{\repl}{\cl}{n'}{0}
            ...
            \txidCOPS{\stub}{\stub}{n}{\stub} = \WTx(\mkvsCOPS(\ke',i))
            \implies n' > n 
...
\begin{mathpar}
    \inferrule[GetTrans]{%
        ...
        (\ke', \WTx(\mkvsCOPS(\ke', m))) \in \depOf{\ver}
        \implies m \in \viCL'(\ke')
        ...
        \\
        \mkvsCOPS' = \updKV{\mkvsCOPS, \viCL', \f, \txidCOPS{\repl}{\cl}{n'}{n+1}} 
    }{%
        \repl, \cl \vdash 
...
\begin{mathpar}
    \inferrule[sync]{%
        ...
        (\ke', \WTx(\mkvsCOPS(\ke', m))) \in \ver\projection{4}
        \implies m \in \viREPL'(\ke') 
        \end{array}\right)} 
    }{%
...
%\begin{theorem}[COPS satisfy parallel snapshot isolation]
    ...
    %\[
%\ET_\PSI \vdash ( \mkvsCOPS'_i, \viCL' ) \csat \f' : ( \mkvsCOPS'_{i+1}, \viewFunCOPS'_{i+1}(\cl) )
    %\]
%\end{theorem}
%\begin{proof}
    %By \cref{thm:cops-cc}, we already the trace \( \tr \) satisfying \( \ET_\CC \).
    %It is sufficient to construct the trace \( \tr' \) so that 
%\( \ET_\PSI \vdash ( \mkvsCOPS'_i, \viCL' ) \csat \f' : ( \mkvsCOPS'_{i+1}, \viewFunCOPS'_{i+1}(\cl) ) \) since \( \ET_\PSI = \ET_\CC \cap \ET_\UA\).
