...
\begin{mathpar}
    \inferrule[GetTrans]{%
        ...
        {\begin{bracketarray}
        \fora{\key, \key', m, \ver} \\
        \quad \ver = \func{ccv}{\mkvsCOPS, \key, \viCL} \land 
        (\key', \wtOf(\mkvsCOPS(\key', m))) \in \depOf{\ver} \\
        {} \qquad \implies m \in \viCL'(\key')
        \end{bracketarray}} \texttt{ ---> \cref{lst:get-trans}, \cref{line:get-trans-ccv-1,line:get-trans-ccv-2,line:get-trans-ccv-3}}
        \\\\
...
\begin{align*}
    ...
    \max\Set{(\val, \txid, \txidset, \dep)}[\exsts{i} (\val, \txid, \txidset, \dep) = \mkvsCOPS(\key, i) \land i \in \viCL(\key)] \\
...
\begin{align*}
    ...
    \lambda \key \ldotp \left( \max_\txid\Set{(\val, \txid, \txidset, \dep)}[\exsts{i} (\val, \txid, \txidset, \dep) = \mkvsCOPS(\key, i) \land i \in \viCL(\key)] \right)\projection{1}
\end{align*}
