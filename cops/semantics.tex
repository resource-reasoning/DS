...
\begin{mathpar}
    \inferrule[Put]{%
        ( \stk, \ss, \emptyset ), \pmutate{\key}{\vx} \toL
        ( \stk', \stub, \Set{(\otW, \key, \val )} ), \pskip
        \\\\
        \dep = \Setcon{(\key', \txid)}{\exsts{i} i \in \viCL(\key') \land \txid = \WTx(\mkvsCOPS(\key', i)) \lor (\key', \txid) \in \depOf{\mkvsCOPS(\key', i)} } \texttt{ ---> \cref{lst:simplified-put}, \cref{line:put-ctx-to-deps}} 
        \\\\
        \txid = \min\Setcon{%
        \txidCOPS{\repl}{\cl}{n'}{0}
        }{%
            \fora{\key', i \in \viREPL(\key'), n} 
            \txidCOPS{\stub}{\stub}{n}{\stub} = \WTx(\mkvsCOPS(\key',i))
            \implies n' > n 
    ...
    \repl, \cl \vdash 
    \mkvsCOPS, \viREPL, \viCL, \stk, \ptrans{\pmutate{\key}{\vx};} \toT{\viCL, \Set{(\otW, \key, \val )} }
...
\begin{mathpar}
    \inferrule[GetTrans]{%
        ...
        (\key', \WTx(\mkvsCOPS(\key', m))) \in \depOf{\ver}
        ...
        \\\\
        \trans =  \pderef{\vx_1}{\key_1}; \dots; \pderef{\vx_j}{\key_j};
        \\\\
        ...
        \\
        \mkvsCOPS' = \updKV{\mkvsCOPS, \viCL', \fp, \txidCOPS{\repl}{\cl}{n'}{n+1}} 
    }{%
        \repl, \cl \vdash 
        \mkvsCOPS, \viREPL, \viCL, \stk, \ptrans{\pderef{\vx_1}{\key_1}; \dots; \pderef{\vx_j}{\key_j}; } \toT{\viCL', \fp}
        \mkvsCOPS', \viREPL, \viCL', \stk', \pskip
    }
    \and
    \inferrule[ClientCommit]{%
        \repl, \cl \vdash 
        \mkvsCOPS, \viewFunCOPS(\repl), \viewFunCOPS(\cl), \stk, \prog(\cl) \toT{\viCL'', \fp}
        \mkvsCOPS', \viREPL', \viCL', \stk', \cmd'
    }{%
        \mkvsCOPS, \viewFunCOPS, \thdenv, \prog \toG{\viCL'', \fp}
...
\begin{mathpar}
    \inferrule[sync]{%
        \viREPL = \viewFunCOPS(\repl)\rmto{\key}{\viewFunCOPS(\repl)(\key) \uplus i} 
        \\\\
        {\left(\begin{array}{@{}l@{}}
        \fora{\key', m, \ver} 
        \ver = \mkvsCOPS(\key, i) \land {}
        (\key', \WTx(\mkvsCOPS(\key', m))) \in \ver\projection{4}
    ...
    \begin{itemize}
        ...
        \item \( \ET_\MW \).
            ...
            \begin{itemize}
                ...
                \item For \rl{Put}, the new view \( \viewFunCOPS_{i+1}(\cl) = \viCL\rmto{\key}{j} \),
...
%\begin{theorem}[COPS satisfy parallel snapshot isolation]
    ...
    %\[
        %\mkvsCOPS'_i, \viewFunCOPS'_i, \thdenv'_i, \prog'_i \toG{\viCL',\fp'} \mkvsCOPS'_{i+1}, \viewFunCOPS'_{i+1}, \thdenv'_{i+1}, \prog'_{i+1} 
    %\]
    %then the i-\emph{th} step satisfies \( \ET_\PSI \), \ie
    %\[
%\ET_\PSI \vdash ( \mkvsCOPS'_i, \viCL' ) \csat \fp' : ( \mkvsCOPS'_{i+1}, \viewFunCOPS'_{i+1}(\cl) )
    %\]
%\end{theorem}
%\begin{proof}
    %By \cref{thm:cops-cc}, we already the trace \( \tr \) satisfying \( \ET_\CC \).
    %It is sufficient to construct the trace \( \tr' \) so that 
%\( \ET_\PSI \vdash ( \mkvsCOPS'_i, \viCL' ) \csat \fp' : ( \mkvsCOPS'_{i+1}, \viewFunCOPS'_{i+1}(\cl) ) \) since \( \ET_\PSI = \ET_\CC \cap \ET_\UA\).
    ...
    %who installs a version \( \mkvsCOPS_{i+1}(\key,j) = (\val,\txid,\T,\dep )\) with a initial view \( \viCL \) does not contain all versions \( \mkvsCOPS_i(\key,m) \)
    ...
    %Suppose the version \( \mkvsCOPS_i(\key,j-1) = (\val',\txid',\T',\dep' ) \) for some transaction \( \txid' \).
    ...
    %\begin{itemize}                                                               
        %\item Suppose \( \depOf(\txid') \subseteq \depOf(\txid_\rd) \), for all read-only transactions \( \txid_\rd \in \T \).
            ...
            %Given that, all the initial views for the read-only transactions \( \txid_\rd \in \T \) also include \( \mkvsCOPS(\key,j) \).
        ...
        %\item there exists a read-only transaction \( \txid_\rd \in \T \) such that 
              ...
              %\begin{itemize}
                  ...
                  %\item If there exists a read-only transaction \( \txid_\rd \) that is not safe to include \( \depOf(\txid') \).
                      ...
                      %therefore 
                      %\[
                          %\fora{\txid'_\rd \in \T'} \depOf{\mkvsCOPS_i(\key',x)} \subseteq \depOf{ \txid'_\rd}
                      %\]
            %\end{itemize}
    %\end{itemize}
%\end{proof}
