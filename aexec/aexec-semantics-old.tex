...
To model the global machine states, instead of heap-based states, we use \emph{abstract executions} (\cref{def:abs-exec}).
...
\begin{definition}[Runtime abstract executions and abstract executions]
...
\begin{itemize}
\item
The \emph{transactions} \( \tx \) is a partial function from transaction identifiers to their corresponding events (\cref{def:transactions}).
\item 
...
\end{rclarray}
\]
The set of the units is \( \unitAEX \defeq \Setcon{\aexec}{\fora{\txid} \aexec\prjT(\txid) = \unitE} \).
...
\aexecrun_{1} \ordAEXRUN \aexecrun_{2} & \iffdef & 
    \begin{array}[t]{@{}l@{}}
        ( \fora{\txid} \aexecrun_{1}\prjT(\txid) \implies \aexecrun_{2}\prjT(\txid) )  
...
A \emph{consistency model} contains two parts, a \emph{resolution policy} and a \emph{consistency guarantee} (\cref{def:consistency-models}) \cite{cerone_et_al:lipics:2017:7794}.
...
\begin{rclarray}
    ...
    \Setcon{%
        ...
        \quad {} \land \fora{\aexec, \txidset, \addr } \respo(\aexec, \txidset, \addr)\isdef \implies \txidset \subseteq \dom(\aexec\prjT)
    }
\end{rclarray}
\]
...
\Setcon{%
        ...
        \land \fora{\txid, \txid'} (\txid, \txid') \in \conguar(\aexec) 
...
\begin{definition}[Thread transition labels]
\label{def:label}
Given the set of thread identifiers \(\ThreadID\) (\cref{def:abs-exec}), the set of \emph{thread transition labels}, $\lb \in \Translabel$, is defined by the following grammar, where $\prog$ denotes a program (\cref{def:language}), the $\txid$ demotes a transaction identifier and $\thstk$ denotes a thread stack (\cref{def:stacks}),
\[
    \begin{rclarray}
...
\begin{definition}[Thread semantics]
...
\]
Given the set of consistency model \( \ConsisModels \) (\cref{def:consistency-models}), the set of thread stacks \( \ThdStacks \) (\cref{def:stacks}) and runtime abstract executions \( \Aexecrun \) (\cref{def:run-abs-exec}), the \emph{per-thread operational semantics} of programs,
\[
\begin{rclarray}
	\toT{} & : &
    \begin{array}[t]{@{}l@{}}
...
\end{rclarray}
\]
is defined in \cref{fig:thread_semantics}.
\end{definition}
...
Given the transaction code \( \trans \), first pick an initial heap \( \h \) and then given the transaction semantics (\cref{fig:transaction_semantics}), we can get the events set \( \evset \).
...
Note that these two rules are labelled with the \(\lbF{\thid, \prog}\) and \(\lbJ{\thid, \thstk}\) which are used by the semantics of the thread pool described shortly (\cref{fig:thread_pool_semantics}).
...
\begin{rclarray}                                 
    ...
    \begin{array}[t]{@{}l@{}}
        \thid \in \aexecrun\prjI \land  \fora{ \txid' } ( (\txid', \txid) \in \conguar(\eraseAEX{\aexecrun}) \implies (\txid', \txid) \in \aexecrun\prjV ) \\
    \end{array} \\
    \newaexecName & : & 
    \left(\begin{array}{l}
...
%
    ...
    \Setcon{%
        ...
        \fora{\addr, \val}  \val \in \respo(\eraseAEX{\aexecrun}, \txidset, \addr) \iff \h(\addr) = \val \\
    } \\
%
%              
...
Given the set of thread stacks $\ThdStacks$ (\cref{def:stacks}) and intermediate programs $\IntermediatePrograms$ (\cref{def:thread_semantics}), a \emph{thread pool} is a a finite partial function from thread identifiers to triples of thread stacks and intermediate programs, \(\thpl \in \TPool \eqdef \ThreadID \parfinfun \ThdStacks \times \IntermediatePrograms\).
\end{definition}
 
\begin{definition}[Thread pool semantics] 
\label{def:thread_pool_semantics}
Given the set of consistent models \( \ConsisModels \) (\cref{def:consistency-models}), runtime abstract executions \(\Aexecrun\) (\cref{def:abs-exec}), transition labels \( \Translabel \) (\cref{def:label}) and thread pools  \( \TPool \) (\cref{def:thread_pools}), the \emph{thread pool semantics}, 
\[
	\toG{} : \Como \times (\Aexecrun \times \TPool) \times \Translabel \times (\Aexecrun \times \TPool) 
\]
is defined in \cref{fig:thread_pool_semantics}.
\end{definition}
 
The thread pool operational semantics is given in \cref{fig:thread_pool_semantics}, where an arbitrary thread in the pool \(\thpl\) is picked to run for one step.
...
Given a consistency model, we can define the set of abstract executions that satisfy a consistency model (\cref{def:valid-aexec}) \cite{cerone:snapshot,cerone_et_al:lipics:2017:7794,cerone_et_al:LIPIcs:2015:5375}.
The \cref{lem:consistency-monotonicity} is for sanity check.

\begin{definition}[Valid abstract executions]
\label{def:valid-aexec}
Given a consistency model \( (\respo, \conguar) = \como \in \Como \) (\cref{def:consistency-models}), the set of valid abstract executions under the model, denoted by \( \evalCOM{\como} \),  is defined as the follows,
\[
    \begin{rclarray}
...
\begin{lemma}[Consistency models monotonicity]
...
\end{lemma}
\begin{proof}
For any abstract execution \( \aexec \) that satisfies the stronger consistency model \( \como_{2} = (\respo_{2}, \conguar_{2}) \), first, it has \( \conguar_{2}(\aexec) \subseteq \aexec\prjV \) by the \cref{def:valid-aexec}.
Given the hypothesis \( \como_{1} \ordAEX \como_{2} \) and the order definition (\cref{def:consistency-models}), we know \( \conguar_{1}(\aexec)  \subseteq \conguar_{2}(\aexec)  \) so that \( \conguar_{1}(\aexec) \subseteq \aexec\prjV \).
...
\begin{theorem}[Soundness of the semantics]
...
This is,
 \[
 \begin{array}{@{}l@{}}
    \fora{\como, \thid, \aexecrun, \aexecrun', \thstk, \thstk', \iprog, {\iprog}', \lb} \\
...
\end{align}
First for the \cref{equ:res_policy}, it only needs to check the new transaction \( \txid \) as others are proved directly from the hypothesis.
...
\]
Because by the transaction semantics (\cref{fig:transaction_semantics}), a transaction only records the first read event for each address.
...
If the new read event is included in the events set after flushing read (\cref{def:transaction-event}), \ie \( (\etR, \addr, \val) \in ( \evset \flushR (\etR, \addr, \val) ) \), this means there is no other read and write to the same address before, so that the value \( \val  \) associate with the address \( \addr \) is the initial value, \ie \( \h(\addr)\), and after that no other read event can over-write.
By the \( \obsstateName \) function (\cref{fig:thread_semantics}), where \( \h(\addr) = \val \iff \respo(\aexec, \txidset, \addr) \), the following hold,
\[
...
\[
    (\etR, \addr, \val) \in \evset \implies \respo(\aexec', \txidset, \addr)
\]
By the \( \obsstateName \) function, we have \( \evset = \aexec'\prjT(\txid)\) and \( \txidset = \aexec'\prjV(\txid)\) therefore we prove \cref{equ:res_policy}.

Second for \cref{equ:con_guarantee}, the visibility relation of the new abstract execution \( \aexec'\prjV \) contains the minimum relation required by the consistency guarantee.
...
Then, given the consistency guarantee (\cref{def:consistency-models}), it cannot violate arbitration order, this is, \( \conguar(\aexec') \subseteq \aexec'\prjA\).
Thus, it is safe to assume a transaction \( \tsid' \in \dom(\aexec'\prjT) \) such that \( (\tsid', \tsid) \in \conguar(\aexec') \).
By the \( \predn{consis}\) predicate, we have \( (\tsid', \tsid) \in \conguar(\aexec') \implies (\tsid', \tsid) \in \aexec'\prjV\), so that we prove \cref{equ:con_guarantee}.
...
For sanity check and also proving the completeness of this semantics, we first prove the semantics is monotonic (\cref{lem:semantics-monotonicity}), which means that for any reduction that can happen in the stronger consistency model, it can also happen in the weaker one.

\begin{lemma}[Semantics monotonicity]
...
\[
\begin{array}{@{}l@{}}
    \fora{\como_{1}, \como_{2}, \thid, \aexecrun, \aexecrun', \iprog, \iprog', \thstk, \thstk', \lb}  \\
...
Given an initial runtime abstract execution \( \aexecrun \), a set of observable transactions \( \txidset \), a new transaction identifier \( \txid \) and a thread identifier \( \thid \), by the \rl{PCommit} rule (\cref{fig:thread_pool_semantics}), it is sufficient to prove, first, all the observable states under the stronger consistency model can also be observed under the weaker one,
\begin{equation}
...
\end{equation}
To prove \cref{equ:obs-state-included}, assume an observable heap \( \h \) under stronger consistency model \( \como_{2}\), which means \( \h \in \obsstate{\aexecrun, \txidset, \respo_{2}} \).
Then, assume an address \( \addr\) and the corresponding value \( \val \) such that \(  \h(\addr) = \val \).
By the \( \obsstateName \) function  (\cref{fig:thread_semantics}) and resolution policy (\cref{def:consistency-models}), it is known that \( \val \in \respo_{2}(\eraseAEX{\aexecrun}, \txidset, \addr)  \).
Because of \( \respo_{2}(\eraseAEX{\aexecrun}, \txidset, \addr) \subseteq \respo_{1}(\eraseAEX{\aexecrun}, \txidset, \addr) \) (\cref{def:consistency-models}), we have \(  \val \in \respo_{1}(\eraseAEX{\aexecrun}, \txidset, \addr) \), so \( \h \in \obsstate{\aexecrun, \txidset, \respo_{1}} \).
For \cref{equ:consis-both-exist}, assume the \( \consis{\aexecrun, \txid, \thid, \conguar_{2}} \) predicate holds and assume an edge \( (\txid', \txid) \in \conguar_{2}(\eraseAEX{\aexecrun'}) \) for some \( \txid' \).
...
This means for any edges that satisfy the consistency guarantee for stronger model, they also satisfy the weaker consistency guarantee, thus the \cref{equ:consis-both-exist} holds.
Combining \cref{equ:obs-state-included} and \cref{equ:consis-both-exist}, we prove \rl{PCommit}.
...
\begin{lemma}[Preservation of the consistency model]
\label{lem:preserve-of-consistency}
\[
 \begin{array}{@{}l@{}}
    \fora{\como_{1}, \como_{2}, \thid, \aexecrun, \aexecrun', \thstk, \thstk', \iprog, {\iprog}', \lb} \\
...
\end{theorem}
\begin{proof}
For any runtime abstract execution after one step such that \( \eraseAEX{\aexecrun'} \in \evalCOM{\como} \), by the \cref{lem:preserve-of-consistency}, it is known that the initial configuration also satisfies the consistency model, this is, \( \eraseAEX{\aexecrun} \in \evalCOM{\como} \).
Since \( \btmCOM \) is the bottom element such that \( \btmCOM \ordCOM \como \), because the semantics are monotonic with respect to the order of consistency model (\cref{lem:semantics-monotonicity}), we have the proof.
\end{proof}
