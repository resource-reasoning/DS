...
\quad ((\txid \xrightarrow{\VIS_{\aexec_1}} \txid') \iff (\txid \xrightarrow{\VIS_{\aexec_2}} \txid'))
...
Similar \rl{PSingleThread} rule, the \rl{ASinglethread} rule in \cref{fig:aexec.semantics}
...
have a similar counterpart in the key-value store semantics.

Note that \rl{ACommit} is more general than Rule \rl{Pcommit} in the key-value store semantics.
...
In contrast, the snapshot of a transaction used in \rl{ACommit}
