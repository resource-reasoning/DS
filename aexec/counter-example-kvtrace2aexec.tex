...
for any key $\ke$: that is, for given a kv-store $\mkvs$, we required from a view $\vi \in \Views(\mkvs)$ that 
$\vi(\ke) = [i]$ for some $i: 0 < \lvert \mkvs(\ke) \rvert$, in addition to the constraints posed by \cref{def:views}. 
In this setting, the most permissive execution test $\ET$ is the following: 
\[
\left(\ET \vdash (\mkvs, \vi) \triangleright \fp : \vi' \right) \iff \forall \ke.\;\exists i_{\ke}, j_{\ke}.\; \vi(\ke) = [ i ] \wedge \vi'(\ke) = [ j ].
\]
...
\end{array}
\]
where $\fp_1 = \{(\otW, \ke_1, \val_1), (\otW, \ke_2, \val_1)\}$, $\fp_2 = \{(\otW, \ke_1, \val_2)\}$, 
$\fp_3 = \{(\otR, \ke_1, \val_2), (\otR, \ke_2, \val_0)\}$,\\ $\hh_1 = \updateKV(\hh_0, \cl_1, \_, \fp_1)$, 
