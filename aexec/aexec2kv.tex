...
(\cite{laws}). Together with \cref{thm:kv2graph} it follows that $\hh_{\aexec}(\ke, 0) = (\val_0, \txid_0, \_)$.
...
Because $\vi$ is atomic by definition, and because $\hh_{\aexec}(\ke, i) = (\_, \txid, \_)$, then we have that $i \in \vi(\ke)$. 
...
$\RP_{\LWW}(\aexec, \T) = \snapshot(\hh_{\aexec}, \vi)$. 
It suffices to choose $\T = \bigcup\limits_{\ke \in \Keys}(\{ \WTx(\hh_{\aexec}(\ke, i)) \mid i > 0 
\wedge i \in \vi(\ke)\})$.
