...
\begin{proof}
    Let assume:
    \[
        \mkvs_0, \vienv_0 \toET{\cl, \vi, \fp}[\MR] \mkvs_1, \vienv_1 \toET{\cl', \vi', \fp'}[\MR] \mkvs_2, \vienv_2 
    \]
    where \( \cl \neq \cl' \), \( \fora{ \key } (\otW, \key, \stub) \notin \fp \cap \fp' \) and \( \vi, \vi' \in \Views(\mkvs_0)\).
    Given the specification of \(\MR\) (\cref{fig:execution-tests}), we know \( \vienv_1 = \vienv_0\rmto{\cl}{\vi''}\) for some \( \vi'' \) such that \( \vi \sqsubseteq \vi'' \).
    ...
    \[
        \mkvs_0, \vienv_0 \toET{\cl', \vi', \fp'}[\MR] \mkvs'_1, \vienv'_1 \toET{\cl, \vi, \fp}[\MR] \mkvs_2, \vienv_2 
    \]
\end{proof}                                                                                    
...
\begin{proof}
    ...
    \[
        \fora{j,i, \key, \key'} j \leq \vi(\key) \land \wtOf(\mkvs'_1(\key', i)) \toEDGE{\SO ?} \wtOf(\mkvs'_1(\key, j)) \implies i \leq \vi'(\key')  \\
    \]
