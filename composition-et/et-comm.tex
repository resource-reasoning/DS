...
(\mkvs''_k, \vienv''_k\rmto{\cl_{m}}{\max\Set{\conf_n\projection{2}(\cl_{m+1}), \conf'_{m+1}\projection{2}(\cl_{m+1})} })
\end{centermultline}
...
there exists a view \( \vi \) such that \( \fora{\key} \conf''_{k-1}(\cl_m)(\key) \subseteq \vi(\key)\) such that
\begin{equation}
    \label{equ:et12-csat}
    \begin{array}{l}
...
\begin{centermultline}
    ...
    \conf''_{k+1}\projection{2}(\cl)(\key) = \max\Set{\conf_n\projection{2}(\cl)(\key), \conf'_{m+1}\projection{2}(\cl)(\key)}
\end{centermultline}
