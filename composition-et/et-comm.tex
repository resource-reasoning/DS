...
%\conf_n\projection{2}(\cl)(\key) \cup \conf'_m\projection{2}(\cl)(\key) \subseteq \conf''_k\projection{2}(\cl)(\key) 
\end{centermultline}
and assuming that the last fingerprint is \( \fp \) committed by a client \( cl \) at i-\emph{th} step ( \( i \leq k\) ) in the trace \( \tr'' \):
\begin{itemize}
...
\end{itemize}
then the \cref{eq:minimum-view-for-last-fp,eq:minimum-view-fix-after-last} hold :
\begin{centermultline}[eq:minimum-view-for-last-fp]
\ET_1 \cap \ET_2 \vdash (\mkvs,\vi) \csat : (\mkvs',\vi') \land \dagger(\vi',\mkvs') \\
{} \land \begin{bracketarray} 
    \fora{\vi''} \ET_1 \cap \ET_2 \vdash (\mkvs,\vi) \csat : (\mkvs',\vi') 
    \land \dagger(\vi'',\mkvs')  \implies \vi' \viewleq \vi'' 
\end{bracketarray}
\end{centermultline}
where
\begin{align*}
    \dagger(\vi,\mkvs) & \equiv 
    \begin{multlined}[t]
        \fora{\key} \min(\ET_1, \mkvs, \cl)(\key) \cup \min(\ET_2, \mkvs, \cl)(\key) \subseteq \vi(\key) \\
    \end{multlined}
\end{align*}
and
...
\begin{itemize}
    \item \( \fora{\vi,\key} \min(\ET_1,\mkvs_m,\cl_m)(\key) \subseteq \vi(\key) \land \ET_1 \vdash ( \mkvs_m, \vi )  \csat \fp_{m} : (\mkvs_{m+1}, \stub ) \implies \conf'''_{n-1}\projection{2}(\cl_{m}) \viewleq \vi \)
    \item \( \conf'''_{n}\projection{2}(\cl_{m}) = \min(\ET_1, \mkvs_{m+1}, \cl_m)\)
    \item \( \fora{\vi,\key} \min(\ET_2,\mkvs,\cl_m)(\key) \subseteq \vi(\key) \land \ET_2 \vdash ( \mkvs_m, \vi )  \csat \fp_{m} : (\mkvs_{m+1}, \stub ) \implies \conf'_{m-1}\projection{2}(\cl_{m}) \viewleq \vi \)
    \item \( \conf'_{m}\projection{2}(\cl_{m}) = \min(\ET_2, \mkvs_{m+1}, \cl_m)\)
\end{itemize}
...
\begin{centermultline}[equ:et12-csat]
    \ET_1 \cap \ET_2 \vdash (\mkvs_m,\vi) \csat \fp : (\mkvs_{m+1}, \vi' )
    \land \dagger(\vi,\mkvs_m) 
    \land \dagger(\vi',\mkvs_{m+1}) \\
    {} \land \begin{bracketarray} \fora{\vi'', \vi'''} 
        \ET_1 \cap \ET_2 \vdash (\mkvs_m,\vi'') \csat \fp : (\mkvs_{m+1}, \vi''' ) 
    \land \dagger(\vi'',\mkvs_m) 
    \land \dagger(\vi''',\mkvs_{m+1})  \\
    \qqquad \implies \vi \viewleq \vi'' 
    \land \vi' \viewleq \vi''' \end{bracketarray} 
\end{centermultline}
where
\begin{align*}
    \dagger(\vi,\mkvs) & \equiv 
    \begin{multlined}[t]
        \fora{\key} \min(\ET_1, \mkvs, \cl_m)(\key) \cup \min(\ET_2, \mkvs, \cl_m)(\key) \subseteq \vi(\key) \\
    \end{multlined}
\end{align*}
...
\end{centermultline}
Yet the p-\emph{th} kv-store contains less information than (k-1)-\emph{th} once:
\begin{centermultline}
    \min(\ET_1,\mkvs_p,\cl_m) \viewleq \min(\ET_1,\mkvs_{k-1},\cl_m) \\
    {} \land \min(\ET_2,\mkvs_p,\cl_m) \viewleq \min(\ET_2,\mkvs_{k-1},\cl_m) 
\end{centermultline}
Given above and \cref{equ:ih-for-merge-two-trace,equ:et1-et2-csat}, it is known:
\begin{centermultline}
