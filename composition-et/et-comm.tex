...
\begin{proof}
    ...
    By the definition of  $\conf \xrightarrowtriangle{\cl, \fp}_{\ET} \conf'$ (\cref{def:cm}), we have \(\mkvs' \in \updKV{\mkvs, \vi, \f, \cl}\) and  \( \viewFun' = \viewFun\rmto{\cl}{\vi'} \) for some \( \vi' \) such that \( \ET \vdash (\mkvs, \vi) \csat \f : (\mkvs',\vi') \).
    Given that \( \ET \subseteq \ET'\), we know \( \ET' \vdash (\mkvs, \vi) \csat \f : (\mkvs',\vi') \) and so $\conf \xrightarrowtriangle{\cl, \fp}_{\ET'} \conf'$.
\end{proof}
...
\begin{equation}
    \label{equ:last-et-2}
    \begin{array}{@{}l @{}}
        (\mkvs'_i, \viewFun'_i) \toET{\cl'_{i}, \fp'_{i}}{\ET_2} (\mkvs'_{i+1}, \viewFun'_{i+1}) \land \ET_2 \vdash (\mkvs'_i, \vi'_i) \csat \f'_i  : (\mkvs'_{i+1},\vi'_{i+1}) \\
...
\begin{equation}
    ...
    \begin{array}{@{} l@{}}
        \ET_1 \vdash ( \conf'_{i}\projection{1}, \conf'_{i}\projection{2}(\cl_{i}) )  \csat \f_{i} : ( \conf'_{i+1}\projection{1}, \conf'_{i+1}\projection{2}(\cl_{i}) ) \\
        \quad {} \land \ET_2 \vdash ( \conf'''_{n-1}\projection{1}, \conf'''_{n-1}\projection{2}(\cl_{i}) )  \csat \f_{i} : (\conf_{n}\projection{1}, \conf_{n}\projection{2}(\cl_{i}) )
    \end{array}
\end{equation}
...
\[
    \begin{array}{@{}l@{}}
    \for{ \mkvs, \mkvs', \vi, \vi', \vi'', \f, \ke } 
    (\stub, \ke, \stub) \in \f \land \vi(\ke) = \vi'(\ke) \land (\mkvs, \vi, \f, \mkvs', \vi'') \in \ET 
    \implies (\mkvs, \vi', \f, \mkvs', \vi'') \in \ET
    \end{array}
\]
...
Then,  because for any \( \mkvs, \mkvs', \vi, \vi', \vi'' \) and \( \f \), if \( (\mkvs, \vi, \f, \mkvs', \vi' ) \in \ET_1 \) and \( \vi' \sqsubseteq \vi'' \) then \( (\mkvs, \vi, \f, \mkvs', \vi'' ) \in \ET_1 \), and similarly for \( \ET_2 \).
It means:
\begin{equation}
    \label{equ:et12-csat}
    \begin{array}{l}
    \ET_1 \cap \ET_2 \vdash ( \conf''_{k-1}\projection{1}, \conf''_{k-1}\projection{2}(\cl_{i}) ) \csat
    \f_{i} : ( \conf_{n}\projection{1}, \max\Set{\conf'_{i+1}\projection{2}(\cl_{i}), \conf_{n}\projection{2}(\cl_{i})} )
    \end{array}
\end{equation}
