...
\conf''_k\projection{2}(\cl)(\key) = \conf_n\projection{2}(\cl)(\key) \cup \conf'_m\projection{2}(\cl)(\key)
\end{centermultline}
...
(\mkvs''_k, \vienv''_k\rmto{\cl_{m+1}}{\vi })
\end{centermultline}
where the new view \( \vi \):
\begin{centermultline}
    \fora{\key} = \vi(\key) = \conf_n\projection{2}(\cl_{m+1})(\key) \cup \conf'_{m+1}\projection{2}(\cl_{m+1})(\key) 
\end{centermultline}
...
\begin{centermultline}[equ:et1-et2-csat]
        \ET_1 \vdash ( \conf'''_{n-1}\projection{1}, \conf'''_{n-1}\projection{2}(\cl_{m}) )  \csat \fp_{m} : (\conf_{n}\projection{1}, \conf_{n}\projection{2}(\cl_{m}) ) \\
        {} \land \ET_2 \vdash ( \conf'_{m}\projection{1}, \conf'_{m}\projection{2}(\cl_{m}) )  \csat \fp_{m} : ( \conf'_{m+1}\projection{1}, \conf'_{m+1}\projection{2}(\cl_{m}) ) 
\end{centermultline}
Given \cref{equ:ih-for-merge-two-trace,equ:et1-et2-csat,def:match-view},
there exists views \( \vi, \vi' \) such that:
\begin{centermultline}
\fora{\key} \conf''_{k-1}(\cl_m)(\key) \subseteq \vi(\key)
\land \vi'(\key) =  \conf'_{m+1}\projection{2}(\cl_{m}) \cup \conf_{n}\projection{2}(\cl_{m})
\end{centermultline}
and
\begin{equation}
    \label{equ:et12-csat}
    \begin{array}{l}
    \ET_1 \cap \ET_2 \vdash ( \conf''_{k-1}\projection{1}, \vi ) \csat
    \fp_{m} : ( \conf_{n}\projection{1}, \vi' )
    \end{array}
\end{equation}
...
\begin{centermultline}
    \conf_0 \toET{\stub}[\ET_1 \cap \ET_2] \dots \toET{\stub}[\ET_1 \cap \ET_2] \conf''_{k-1}  \toET{\cl_m, \vi}[\ET_1 \cap \ET_2] \stub \toET{\cl_m, \fp_m}[\ET_1 \cap \ET_2] \conf''_{k+1} 
    \land \conf'''_{n}\projection{1} = \conf'_{m+1}\projection{1} = \conf''_{k+1}\projection{1}  \\
    {} \land \fora{\cl \in \dom(\conf''_{k+1}\projection{2}),\key \in (\conf''_{k+1}\projection{1})} 
    \conf''_{k+1}\projection{2}(\cl)(\key) = \conf_n\projection{2}(\cl)(\key) \cup \conf'_{m+1}\projection{2}(\cl)(\key)
\end{centermultline}
