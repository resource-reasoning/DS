...
The state of all transactions running as part of a program is a total function $\mathsf{TState} : \mathsf{Tid} \rightarrow (\mathsf{Stack}, \mathsf{Bool})$ which is ranged over by $S$.
...
\begin{gather*}
(-, -, -, -, -) \rightsquigarrow (-, -, -, -, -) \\
: \mathsf{Storage} \times \mathsf{LMan} \times \mathsf{Stack} \times \mathsf{Bool} \times \mathsf{Cmd} \times \mathsf{Storage} \times \mathsf{LMan} \times \mathsf{Stack} \times \mathsf{Bool} \times \mathsf{Cmd}
\end{gather*}
...
\infer[\textsc{LoopT}]
{
	...
	\rightsquigarrow
	(h, \Phi, s, p, \mathds{K}; \ploop{\mathds{B}}{\mathds{C}})
}
{
	\llbracket \mathds{B} \rrbracket_s^B = \top
}
...
\infer[\textsc{Lock}]
{
	\iota \vdash (h, \Phi, s, \bot, \mathds{C})
	\rightsquigarrow
	(h, \Phi[k \mapsto (\{\iota\}, \kappa)], s, \bot, \mathds{C})
}
{
	k \in \pred{dom}{h}\ \
	\kappa \geq \textsc{s}\ \
...
\infer[\textsc{LockS}]
{
	\iota \vdash (h, \Phi, s, \bot, \mathds{C})
	\rightsquigarrow
	(h, \Phi[k \mapsto (\{\iota\} \cup I, \textsc{s})], s, \bot, \mathds{C})
}
{
	k \in \pred{dom}{h}\ \
	(I, \textsc{s}) = \Phi(k)
...
\infer[\textsc{Unlock}]
{
	...
	\rightsquigarrow
	(h, \Phi[k \mapsto (\emptyset, \textsc{u})], s, \top, \mathds{C})
}
{
	k = \llbracket \mathds{E} \rrbracket_s\ \
	k \in \pred{dom}{h}\ \
...
\infer[\textsc{UnlockS}]
{
	\iota \vdash (h, \Phi, s, p, \mathds{C})
	\rightsquigarrow
	(h, \Phi[k \mapsto (I, \textsc{s})], s, \top, \mathds{C})
}
{
	k = \llbracket \mathds{E} \rrbracket_s\ \
	k \in \pred{dom}{h}\ \
...
\begin{gather*}
(-, -, -, -, -) \twoheadrightarrow (-, -, -, -, -) \\: \mathsf{Storage} \times \mathsf{LMan} \times \mathsf{Stack} \times \mathsf{Bool} \times \mathsf{Prog} \times \mathsf{Storage} \times \mathsf{LMan} \times \mathsf{Stack} \times \mathsf{Bool} \times \mathsf{Prog}
\end{gather*}
