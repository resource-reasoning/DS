...
\begin{center}
	\begin{tikzpicture}[->, semithick]
		...
		\node[tleft] (s2) at (5.5, 2) {$h[1 \mapsto 5], \Phi, S, \mathds{P}_a$};
		\node[tleft] (s3) at (5.5, -2) {$h, \Phi[2 \mapsto (\{j\}, \textsc{x})], S, \mathds{P}_b$};
		\node[tleft] (s4) at (11, 0) {$h[1 \mapsto 5], \Phi[2 \mapsto (\{j\}, \textsc{x})], S, \mathds{P}'$};
		
		\draw
		(s1) edge[pleft] node[midway, sloped] {$\actwrite{i}{1}{5}$} (s2.195)
		(s1) edge[pright] node[midway, sloped] {$\actlock{j}{2}{\textsc{x}}$} (s3.165)
		(s2) edge[pleft] node[midway, sloped] {$\actlock{j}{2}{\textsc{x}}$} (s4.40)
		(s3) edge[pright] node[midway, sloped, below] {$\actwrite{i}{1}{5}$} (s4.320);
	\end{tikzpicture}
	\captionof{figure}{An example of swapping consecutive actions in a program reduction.}
\end{center}
