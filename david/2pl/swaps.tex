...
\begin{defn}
	(Redundant lock).
	A lock in mode $\kappa$ on cell $k$ done by transaction $\iota$ as part of trace $\tau$ is \emph{redundant} if and only if $\kappa$ is exclusive and every time $\iota$ accesses $k$, it does so only to read.
	\begin{gather*}
...
\begin{defn}
	...
	\begin{gather*}
		...
		\left( (\actlock{\iota}{k}{\kappa}, n) \in \tau \lor (\actunlock{\iota}{k}, n) \in \tau \right) \\
			\implies
		\pred{needed}{\iota, k, \kappa, \tau} \land \lnot \pred{redundant}{\iota, k, \kappa, \tau}
	\end{gather*}
\end{defn}
