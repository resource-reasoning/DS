...
\begin{gather*}
	...
	(h', \emptyset, S', \pskip)
\end{gather*}
then when $k = k'$, from the definition of $\mathsf{conflict}$ it follows that the two actions must be conflicting. From the definition of $\mathsf{SG(\tau_{seq})}$ and the fact that the strict total order $\sqsubset$ keeps the serialization graph's edges, it must be the case that $j \sqsubset \iota$ which is not possible due to the fact that $\iota$ is the minimal element of the $\sqsubset$ relation from (\ref{thm:atom3}) and we get a contradiction.
...
\begin{enumerate}[label=({\roman*})]
	...
	\item \label{thm:atom8} If $\alpha = \actread{j}{k}{v}$ and $\alpha' = \actwrite{\iota}{k}{v'}$ or $\alpha = \actwrite{j}{k}{v}$ and $\alpha' = \actread{\iota}{k}{v'}$ or $\alpha = \actwrite{j}{k}{v}$ and $\alpha' = \actwrite{\iota}{k}{v'}$, then the operational semantic rules do not allow the reductions. This is because in all those 3 cases we would need to have $(\{\iota, j\}, \textsc{x}) = \Phi_a(k)$ which is instead not possible.
	
	\item If $\alpha = \actalloc{j}{n}{l}$ and $\alpha' \in \{ \actwrite{\iota}{k}{v}, \actread{\iota}{k}{v}, \actlock{\iota}{k}{\kappa}, \actunlock{\iota}{k} \}$ then when $l \leq k < l + n$ the operational semantic rules do not allow the reductions. This is because $\Phi_b(k) = (\{j\}, \textsc{x})$ while $\alpha'$ requires $\Phi_b = (\{\iota\} \uplus I, \kappa)$ for $\kappa \geq \textsc{s}$. On the other hand, when $k < l \lor k \geq l + n$ we can always swap $\alpha'$ in the place of $\alpha$.
	
	\item If $\alpha \in \{ \actwrite{j}{k}{v}, \actread{j}{k}{v}, \actlock{j}{k}{\kappa}, \actunlock{j}{k} \}$ and $\alpha' = \actalloc{\iota}{n}{l}$ and $l \leq k < l + n$, then the operational semantic rules do not allow the reductions. This is because $\alpha$ requires $k \in \pred{dom}{h_a}$ which is only added by $\alpha'$. On the other hand, when $k < l \lor k \geq l + n$ we can always swap $\alpha'$ in the place of $\alpha$.
	
	\item $\alpha = \actalloc{\iota}{n}{l}$ and $\alpha' = \actalloc{\iota}{n'}{l'}$ then when $\{l, \ldots, l + n - 1\} \cap \{l', \ldots, l' + n - 1\} \equiv \emptyset$ we can always swap $\alpha'$ in the place of $\alpha$, in every other case the operational semantic rules do not allow the reductions.
	...
	\item If $\alpha = \actunlock{j}{k}$ and $\alpha' = \actlock{\iota}{k}{\kappa}$ for $\kappa = \textsc{x}$, then from (\ref{thm:atom7}) we know that $\tau_{seq}$ does not contain any spurious locks which implies that $\iota$ is obtaining the exclusive lock on $k$ in order to later read or write to it. Given that $j$ is releasing a lock on $k$, it means that it either read or wrote to it beforehand (since again $\pred{clean}{\tau_{seq}}$ holds from (\ref{thm:atom7}))
	\begin{itemize}
		\item If $\iota$ later writes to $k$ then this situation leads us back to case \ref{thm:atom8} since there is a conflict between $j$ and $\iota$ on item $k$ which must be accounted for by $\pred{SG}{\tau_{seq}}$.
		
		\item If $\iota$ later reads storage cell $k$ then if $j$ wrote to cell $k$ then we are back to case \ref{thm:atom8} as we have a conflict. Otherwise, if $j$ also read from $k$
	\end{itemize}
