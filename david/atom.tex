...
\begin{enumerate}[label=({\roman*})]	
	...
	\item If $\alpha = \alpha(j, k)$ and $\alpha' = \actunlock{\iota}{k}$ then
		\begin{itemize}
			\item If $\alpha$ is a $\mathsf{write}$ action then it must be the case that $(\emptyset, \textsc{u}) = \Phi_b(k)$ and $\alpha'$ could have never succesfully reduced from $(h_b, \Phi_b, S_b, \mathds{P}_b)$.
			
			\item If $(\{\iota\}, \textsc{x}) = \Phi_a(k)$ then $\alpha$ could have never reduced from $(h_a, \Phi_a, S_a, \mathds{P}_a)$ since $(\{j\} \uplus I, \kappa) = \Phi_a(k)$ is required.
		\end{itemize}			
\end{enumerate}

The last cases to be considered, are the ones involving $\mathsf{alloc}$ actions together with read, write, lock and unlock operations performed on keys in the range of the ones allocated.
\end{proof}
}
