...
\end{gather}
We justify the statement in (\ref{thm:atom5}) and (\ref{thm:atom6}) by looking at one particular consecutive sequence (that follows from (\ref{thm:atom4}) and the definition of $\mathsf{tgen}$) of labelled reductions as part of $\tau_{seq}$, for $\alpha' = \alpha(\iota)$:
\begin{gather*}
	...
	(h', \emptyset, S', \pskip)
\end{gather*}
Let's assume $\alpha = \alpha(j)$ for $j \in \mathsf{Tid}$. In the case where $\iota = j$, (\ref{thm:atom6}) holds trivially given that the implication condition is false; let's instead focus on the case where $\iota \neq j$ and establish that such situation cannot occur. In fact if one of $\alpha$ of $\alpha'$ is an $\mathsf{id}$ action, then by Lemma \ref{lem:idx} we know that the swap would have already happen as part of $\tau_{seq}$. We can now move to the more specific case of read, write, lock, unlock operations $\alpha = \alpha(j, k)$ and $\alpha' = \alpha(\iota, k')$ for some keys $k, k' \in \mathsf{Key}$. If $k \neq k'$ then by Lemma \ref{lem:rwlu} we know that the swap would have already happen as part of $\tau_{seq}$; on the other hand, if $k = k'$ we further distinguish among operations:
\begin{enumerate}[label=({\roman*})]
	\item If both $\alpha$ and $\alpha'$ are $\mathsf{read}$ operations on the same $k$ then by Lemma \ref{lem:rr} we know that the swap would have already happen as part of $\tau_{seq}$.
	
	\item If at least one of $\alpha$ and $\alpha'$ is a $\mathsf{write}$ operation on $k$ and the other is a $\mathsf{read}$ or $\mathsf{write}$ also on $k$, then from the definition of $\mathsf{conflict}$ it follows that the two must be conflicting. From the definition of $\mathsf{SG(\tau_{seq})}$ and the fact that the strict total order $\sqsubset$ keeps the serialization graph's edges, it must be the case that $j \sqsubset \iota$ which is not possible due to the fact that $\iota$ is the minimal element of the $\sqsubset$ relation from (\ref{thm:atom3}).
	
	\item If $\alpha = \actlock{j}{k}{\kappa}$ and $\alpha' = \actlock{\iota}{k}{\kappa'}$ for some $\kappa, \kappa' \in \mathsf{Lock}$, then we know that by the semantic interpretation of the $\mathsf{lock}$ label that $\kappa = \kappa' = \textsc{s}$ and therefore from **LEMMA**, a swap would have already happen as part of $\tau_{seq}$. $\kappa'$ could have not been $\textsc{x}$ since the exclusive mode acquisition requires the set of $k$'s owners, i.e. $I$, to be empty or to only contain $\iota$ but the action $\alpha$ succesfully reduced meaning that $j \in I$ and $\iota \neq j$ by assumption.
	
	\item If $\alpha = \actunlock{j}{k}$ and $\alpha' = \actunlock{\iota}{k}$
	
	\item If $\alpha = \actlock{j}{k}{\kappa}$ and $\alpha' = \actunlock{\iota}{k}$ then it follows that $\kappa = \textsc{s}$ since $j$ acquires the lock on $\kappa$ before $\iota$ unlocks it and $\iota \neq j$ by assumption. From **LEMMA**, a swap would have already happen as part of $\tau_{seq}$.
	
	\item If $\alpha = \actunlock{j}{k}$ and $\alpha' = \actlock{\iota}{k}{\kappa}$ for some $\kappa \in \mathsf{Lock}$, then 
\end{enumerate}
\end{proof}
}
...
\begin{itemize}
	\item If $\mathds{P} = \pskip ; \mathds{P}''$ for some $\mathds{P}'' \in \mathsf{Prog}$, then from the \textsc{PSeqSkip} rule we get that $\mathds{P}' = \mathds{P}''$. From the \textsc{AtPSeqSkip} rule we know that as part of the atomic semantics, $\mathds{P}$ will also reduce to $\mathds{P}''$.
	\item If $\mathds{P} = \pskip \| \pskip$ then from the \textsc{ParEnd} rule we get that $\mathds{P}' = \pskip$. From the \textsc{AtParEnd} rule we know that as part of the atomic semantics, $\mathds{P}$ will also reduce to $\pskip$.
	\item If $\mathds{P} = \mathds{P}_0^*$ for some $\mathds{P}_0 \in \mathsf{Prog}$, then from the \textsc{Loop} rule we get that $\mathds{P}' = \pskip + (\mathds{P}_0;\mathds{P}_0^*)$. From the \textsc{AtLoop} rule we know that as part of the atomic semantics, $\mathds{P}$ will also reduce to $\pskip + (\mathds{P}_0;\mathds{P}_0^*)$.
	\item $\mathds{P} = \mathds{P}_1 + \mathds{P}_2$ for some $\mathds{P}_1, \mathds{P}_2 \in \mathsf{Prog}$, then we can apply one of two reduction rules:
		\begin{itemize}
			\item If $\mathsf{sys}$ was generated by the \textsc{ChoiceL} rule then $\mathds{P}' = \mathds{P}_1$ and through the \textsc{AtChoiceL} rule $\mathds{P}$ will also reduce to $\mathds{P}_1$.
			\item If $\mathsf{sys}$ was generated by the \textsc{ChoiceR} rule then $\mathds{P}' = \mathds{P}_2$ and through the \textsc{AtChoiceR} rule $\mathds{P}$ will also reduce to $\mathds{P}_2$.
		\end{itemize}
\end{itemize}
\end{proof}
\fi