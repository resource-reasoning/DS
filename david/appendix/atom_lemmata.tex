...
\begin{lem}
	...
	\[
		...
		(h, \emptyset, S, \mathds{P}) \rightarrow^* (h', \Phi', S', \pskip) \implies \Phi' \equiv \emptyset
	\]
	{\parindent0pt
	\begin{proof}
		The proof is done by induction on the number of $\rightarrow^*$ reductions, $n$.
		
		\textit{Base case 1}: $n  = 0$
		
		\textit{To show}:
		\[
			\forall h, h', S, S', \Phi', \mathds{P} \ldotp
			(h, \emptyset, S,  \mathds{P}) \rightarrow^0 (h', \Phi', S', \pskip) \implies \Phi' \equiv \emptyset
		\]
		We assume that $(h, \emptyset, S,  \mathds{P}) \rightarrow^0 (h', \Phi', S', \pskip)$ and we know that a zero-step reduction of $\mathds{P}$ to $\pskip$ can only happen when $\mathds{P} = \pskip$ and $h = h', S = S'$ and $\Phi' = \emptyset$. \\
		
		\textit{Inductive case}: $n > 0$
		
		\textit{Inductive hypothesis}:
		\[
			\forall m \leq n, h, h', S, S', \Phi', \mathds{P} \ldotp
			(h, \emptyset, S,  \mathds{P}) \rightarrow^m (h', \Phi', S', \pskip) \implies \Phi' \equiv \emptyset
		\]
		
		\textit{To show}:
		\[
			\forall h, h', S, S', \Phi', \mathds{P} \ldotp
			(h, \emptyset, S,  \mathds{P}) \rightarrow^{n + 1} (h', \Phi', S', \pskip) \implies \Phi' \equiv \emptyset
		\]
		We assume that $(h, \emptyset, S,  \mathds{P}) \rightarrow^{n + 1} (h', \Phi', S', \pskip)$. From this we know there must exist $h'', \Phi'', S'', \mathds{P}'', \alpha$ such that:
		\[
			(h, \emptyset, S,  \mathds{P}) \xrightarrow{\alpha} (h'', \Phi'', S'', \mathds{P}'') \rightarrow^n (h', \Phi', S', \pskip)
		\]
		The proof procedes with a case-by-case analysis on the action $\alpha$, for $\iota \in \mathsf{Tid}$:
		\begin{itemize}
			\item If $\alpha \in \{\actprog, \actid{\iota}\}$ for some then by the semantic interpretation of $\alpha$ we know that $\Phi'' = \Phi$ and $h'' = h$ and by induction hypothesys we get the result.
			
			\item If $\alpha = \actlock{\iota}{k}{\kappa}$ then it must be the case that, for $\alpha' = \actunlock{\iota}{k}$:
			\[
				(h'', \Phi'', S'', \mathds{P}'') \xrightarrow{\alpha'} (h_m, \Phi_m, S_m, \mathds{P}_m) \rightarrow^m (h', \Phi', S', \pskip)
			\]
			Since $\mathds{P}$ reduces to $\pskip$ and $\mathds{T}_\iota$ must also do so through rule \textsc{PSkip} which requires the terminating transactions not to be holding any locks in $\Phi$. Now by inductive hypothesys on $m < n$ we conclude.$\alpha = \actalloc{\iota}{n}{l}$
			
			\item In all other cases, i.e. $\alpha \in \{  \}$, 
		\end{itemize}
	\end{proof}
	}
\end{lem}
