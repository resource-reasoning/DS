...
\begin{lem}
	...
	\end{align}
	Since we know that $(h'', s'') \in \tsem{\mathds{C}}_\mathsf{C}(h, s)$, we get that from (\ref{lem:atomViewsC3}) and the I.H. on the number of steps of reduction, it must be the case that one of the $(\dot{h}, \dot{s})$ part of the set $\tsem{\ploop{\mathds{B}}{\mathds{C}}}_\mathsf{C}(h'', s'')$ is equivalent to $(h', s')$, which concludes our proof.
	\end{proof}
	}
\end{lem}
