...
\defn (Machine state assertions). We define the set of machine state assertions $\mathsf{MAssn}$, ranged over by $\mathcal{M}, \mathcal{M}_1, \ldots, \mathcal{M}_n$ to be the standard separation logic ones, describing the logical environment and a cell in the heap.
\[
...
The structure of assertions follows from the separation logic one, with the addition of the given capability assertions $\mathcal{K}$, shared region (or \textit{boxed}) assertions $\boxed{P}^r_I$ and \textit{interference} assertions $I$. Machine state assertions, $\mathcal{M}$ are interpreted over a world's local logical state, i.e. $\mathcal{M}$ holds true when it is satisfied by $m$, as part of $(m, \mathbf{0}_\mathbb{K})$. In a similar way, a capability assertion $\mathcal{K}$ is true in a local state $(\mathbf{0}_\mathbb{M}, \emptyset[r \mapsto \kappa])$ for some region $r$ and capability $\kappa$ that satisfy $\mathcal{K}$. The assertion $\mathtt{emp}$ is only satisfied by the unit of logical states, formally $\mathbf{0}_\mathsf{L}$. The separating assertion $P \sep Q$ is true of worlds that can be split in two parts such that the first one satisfies $P$ while the second one $Q$. A predicate assertion holds in all worlds belonging to its semantic interpretation as given by the environment $\delta$. The predicate's arguments are evaluated with respect to the logical environment $e$.
...
\defn (Assertion semantics). Assertion semantics are given with respect to a world $w \in \mathsf{World}$, a logical environment $e \in \mathsf{LEnv}$ and a predicate environment $\delta \in \mathsf{PEnv}$. They tell us whether a configuration $w, e, \delta$ satisfies a particular assertion.
\begingroup
...
\\
	...
	\iff
	&
	l = \mathbf{0}_\mathsf{L} \text{ and } \exists l' \ldotp (l', g, \mathcal{J}), e, \delta \vDash P
...
\\
	...
	\iff
	&
	\exists r, \kappa \ldotp l = (\mathbf{0}_\mathbb{M}, \emptyset[r \mapsto \kappa]) \text{ and } (\kappa, r) \in \tsem{\mathcal{K}}_e^\textsc{k}
\\
