...
We will use \( \gpre, \gpost \) to denote states of clients, and \( \lpre, \lpost\) for states of transactions.
In the sketch proof (\cref{fig:write-skew-si-proof}), the assertion in the form of \( \boxass{\bar{\lpre}}{\lrid}{\intass}\),
...
and \( \lpre \) describes the state of views on key-value stores.
...
Interference is a set of actions, each of which has the form \( \kap : \fp \),
where \( \kap \) is the \emph{client-specified capability} and \( \fp \) is the \emph{fingerprint}.
An action \( \kap : \fp \) describes that if a client holds the capability \( \kap \), 
...
For example, \( \null \fpA \cass{\CB{L}}{\lrid} \) means that \( \CB{L} \) needs to return to the shared region.
Last, the \emph{client-specified capability} forms \emph{a partial commutative monoid (PCM)} where \( \composeK \) denotes the composition function.
...
The assertions \( \bar{\lpre} \) inside regions describe a client's view on a key-value store.
It is interpreted as a triple \((\hh, \vi, \ca) \in \evalAUX{\lpre}  \) (details in \cref{def:prog-assertion}), 
consisting of a key-value store \( \mkvs \), a view \( \vi \) and capabilities \( \ca \),
where \( \lpre \) satisfies the snapshots induced by the view \( \snapshot(\mkvs, \vi) \).
