...
this is modelled in the second and third premises of \rl{CAtomicTrans}.
...
The execution test $\ET \vdash (\mkvs, \vi'') : \fp$ ensures that the
final fingerprint $\fp$ of the transaction is compatible with the (original) kv-store
$\mkvs$ and the client view $u''$. We give many examples of such execution tests in \cref{sec:cm}. 
...
\sx{Typos (1), \( \OK_\mathsf{Any}\).
Typos (2), should be \( \vi \viewleq  \vi' \).
Dots in (3), 
\( \fora{\txid, \key, i} \txid \in \mkvs' \setminus \mkvs \implies \wtOf[\mkvs'(\key,i)] \toEDGE{\SO^*} \txid \implies i \in \vi'(\key) \). 
For any client \( \cl \), any versions wrote the by the client must be included in the view. Note that in our operational model, 
\( \mkvs' \setminus \mkvs \) is a singleton set containing a new transaction committed by the client.
}
