...
\end{enumerate*}
When a kv-store satisfies these three conditions, we say that it is \emph{well-formed} (defined formally in \cref{def:mkvs-appendix}). 
...
2) we choose here to always advance the view between transactions.
...
Technically, monotonic read is because view can only advance between transactions and after transaction.
}
