...
\(\SO \defeq \Set{ (\txid, \txid') \mid \exsts{ \cl, n,m } \txid = \txid_{\cl}^{n} \land \txid' = \txid_{\cl}^{m} \land n < m}\).
...
\begin{align*}
    ...
    & \defeq 
    \begin{multlined}[t]
    \text{let} \ i = \max_{<}(\vi(\key)) \ \text{and} \ (\val,\txid',\txidset) = \mkvs(\key,i) \text{ in } 
    \\[-20pt] \updateKV[\mkvs\rmto{\key}{\mkvs(\key)\rmto{i}{(\val, \txid', \txidset \uplus \Set{\txid})}},\vi, \fp, \txid] 
    \end{multlined}
