...
The superscript $n$ is used to determine the order in which transactions 
...
atomically visible~\cite{laws,sureshConcur}: reads are taken from a single snapshot of the kv-store, 
...
with fingerprint $\fp$ in $\mkvs$. This operation results 
...
and computes the new kv-store via $\mkvs' = \updateKV[\mkvs, \vi'', \fp, \txid]$; details of th functions $\nextTxid$ 
...
\item check if the transition from \( (\mkvs, \vi'') \)  to 
    ...
    using the judgement $\ET \vdash (\mkvs, \vi'') \csat \fp : (\mkvs', \vi')$, and 
\item update the view of client $\cl$ to $\vi'$, specified by the judgement 
$\ET \vdash (\mkvs, \vi'') \csat \fp : (\mkvs', \vi')$.
\end{enumerate*}
