...
%\ac{
%\sx{Keep the key-value stores above as the full name. After this point, I change to kv-stores.}
%I changed everything to kv-store. Ideally, we introduce the notation in the introduction.}
...
resolution policy~\cite{vogels:2009:ec:1435417.1435432}. It  is straightforward to generalise our work
to other resolution policies~\cite{terry1994session}.
...
\pg{Above  might need to dovetail with what we say in section 2.  Maybe give an example that relates to
...
\sx{For example, given the snapshot induced by \cref{fig:counter_kv_view} \(\ss : \ss(\ke) = 0 \),
the fingerprint of \( \ctrinc(\ke) \) is \( \Set{(\otR,\ke, 0), (\otW,\ke,1)} \).}
}
