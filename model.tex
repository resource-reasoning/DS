...
The constraint \eqref{eq:view.atomic} in \eqref{def:views} models 
the fact that clients can see either all or none of the updates of a transaction.
...
and it also see that in $\mkvs$ the version 
...
we identify the \emph{fingerprint} of $\trans$ on $\sn$.
A fingerprint of $\trans$ on $\sn$ and $\mkvs$
is a set of \emph{read} and \emph{write operations}, which 
%the read operations 
capture the values $\trans$ reads from $\sn$ (prior to overwriting them), and
%the write operations capture 
the values $\trans$ writes to $\sn$ and aims to commit to $\mkvs$, respectively. 
...
The effect of client $\cl$ committing a transaction with final fingerprint $\fp$ results 
...
and compute the new kv-store via $\mkvs' = \updateKV[\mkvs, \vi'', \fp, \txid]$; these 
