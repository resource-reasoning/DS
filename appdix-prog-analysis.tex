...
and saving and checking map customer IDs to saving balances and checking balances respectively.
We ignore the account table since it is a immutable lookup table.
We encode the saving and checking tables together as a kv-store.
Each customer is represent as an integer \( n \), its checking balance is associated with 
...
\( \codeFont{depositChecking(n,v)} \) deposit \( v \) from the checking account of customer \( n \),
...
even thought it is unchanged.
Let the bank libraries be 
\[ 
