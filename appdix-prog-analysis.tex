...
\begin{align}
(\mkvs, \vienv) \toET{(\cl, \func{inc}[\key])}[\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}] (\mkvs', \vienv') \label{eq:psi_counter_inc}\\
(\mkvs, \vienv) \toET{(\cl, \func{read}[\key])}[\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}] (\mkvs', \vienv). \label{eq:psi_counter_read}
\end{align}
...
Assume that
\[
(\mkvs, \vienv) \toET{(\cl, \func{inc}[\key])}[\ET_{\UA} \cap \ET_{\MRd} \cap  \ET_{\RYW}] (\mkvs', \vienv')
\]
...
%assume also that 
\[
(\mkvs, \vienv) \toET{(\cl, \func{read}[\key])}[\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}] (\mkvs', \vienv').
\]
...
\txid, \txid' \in \T_{i} \wedge \txid \toEDGE{\SO} \txid' &\implies \txid \dashrightarrow \txid'\\
...
or $\txid, \txid' \in \T_{i}$ and $\txid \toEDGE{\SO_{\mkvs}} \txid'$. A consequence of this fact, 
...
\txid, \txid' \in \T^{\key_{h}}_{i} \wedge \txid \toEDGE{\SO} \txid' &\implies \txid \dashrightarrow \txid'\\
\txid \dashrightarrow \txid'' \dashrightarrow \txid &\implies \txid \dashrightarrow \txid'\\
\txid \in (\T^{\key_{h}}_{I} \cup \{\txid^{\key_{h}}_{l}\}) \wedge \txid' \in (\T^{\key_{l}}{j} \cup \{\txid^{\key_l}_{j}\} \wedge \txid \toEDGE{\SO} \txid' &\implies \txid \twoheadrightarrow \txid'\\
...
%\txid'' \toEDGE{PO} \txid' \wedge i < j \implies p \leq q \label{eq:si_counter_crossRD}
%\end{align}
%\end{proposition}
