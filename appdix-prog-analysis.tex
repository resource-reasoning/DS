...
%Specifically, 
$\fp(\mkvs, \vi, \ctrinc(\key)) = \Set{(\otR, \key, n), (\otW, \key, n+1) }[n = \snapshot[\mkvs, \vi]]$, 
and $\fp(\mkvs, \vi ,\mathsf{read}(\key)) = \Set{(otR, \key, n) }[ n = \snapshot[\mkvs, \vi]]$.
...
$\mathsf{Traces}(\ET, \Set{\ptrans{\trans_i}}_{i \in I})$ 
...
For simplicity, let us assume that $\Keys = \Set{ke}$.
...
\fora{ i : 0 \leq i \leq n } \txidset^{\key_{h}}_{i} \cap \{\txid^{\key_{h}}_{i}\}_{i=0}^{n_{h}-1} = \emptyset \label{eq:si_counter_rwtxs}\\
