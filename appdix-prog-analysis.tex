...
%(\txid \in \txidset_{i} \wedge \txid' = \txid_{j} \implies i < j) \wedge {} \\
%(\txid \in \txidset_{i} \wedge \txid' \in \txidset_{j} \implies i \leq j)
...
%Proposition \ref{prop:si_counter_hhshape1}. Let $h, l \in \{1,2\}$ with $h \neq l$; then 
%\begin{align} 
%\forall i, j = 0,\cdots, \key_{h} - 1, p, q = 0,\cdots, \key_{l} - 1, \txid, \txid', \txid'', \txid'''. \txid \in \txidset_{i}^{\key_h} \wedge 
%\txid' \in \txidset_{j}^{\key_{h}} \wedge \txid'' \in \txidset_{p}^{\key_{l}} \wedge \txid''' \in \txidset_{q}^{\key_{k}} \wedge 
