...
%$\mkvs(\key) = \left(\prod_{i=0}^{n-1} (i, \txid_{i}, \T_{i} \uplus \{\txid_{i+1}\}) \right) \lcat 
%(n, \txid_{n}, \T_{n})$. Furthermore, if there exist four indexes $i,j, p, q$ such that 
%$\txid_{\cl}^{p} \in \WTx(\mkvs(\key, i) \cup \RTx(\mkvs(\key, i))$ and 
%$\txid_{\cl}^{q} \in \WTx(\mkvs(\key, j) \cup \RTx(\mkvs(\key, j))$, then 
%$i < j \implies p < q$.
\end{proposition}
...
\begin{align}
(\mkvs, \viewFun) \xrightarrow{(\cl, \mathsf{inc}(\key))}_{\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}} (\mkvs', \viewFun') \label{eq:psi_counter_inc}\\
(\mkvs, \viewFun) \xrightarrow{(\cl, \mathsf{read}(\key))}_{\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}} (\mkvs', \viewFun). \label{eq:psi_counter_read}
\end{align}
...
%\begin{equation}
%\forall i, n.\; \forall \cl.\; \txid_{\cl}^{n} \in \{\WTx(\mkvs(\key, i))\} \cup \RTx(\mkvs(\key, i)) \implies 
%\exists j \geq i.\;\viewFun(\cl) = [\key \mapsto \{0, \cdots, j\}] \label{eq:psi_counter_view}
%\end{equation} 
...
\wedge i \in \viewFun(\cl)(\key) \implies 
j \in \viewFun(\cl)(\key). \label{eq:psi_counter_view}
\end{equation} 
...
Assume that
\[
(\mkvs, \viewFun) \xrightarrow{(\cl, \mathsf{inc}(\key))}_{\ET_{\UA} \cap \ET_{\MRd} \cap  \ET_{\RYW}} (\mkvs', \viewFun')
\]
for some $\cl, \mkvs', \viewFun'$. Let $n+1 = \lvert \mkvs(\key) \rvert$. Because of the definition of 
$\ET_{\UA}$, we must have that $\viewFun(\cl) = [\key \mapsto \{0, \cdots, n\}]$. Also, 
because $\mkvs$ satisfies \eqref{eq:psi_counter_shape}, we have that $\snapshot(\mkvs, \viewFun(\cl))(\key) = n$. 
In particular, $\fp(\key, \viewFun(\cl), \mathsf{inc}(\key)) = \{(\otR, \key, n), (\otW, \key, n+1)\}$. 
Thus we have that $\mkvs' \in \updateKV(\mkvs, \viewFun(\cl), \cl, \{(\otR, \key, n), (\otW, \key, n+1)\})$. 
Let $\txid_{n+1}$ be the transaction identifier 
chosen to update $\mkvs$, i.e. $\mkvs' = \updateKV(\mkvs, \viewFun(\cl), \txid_{n+1}, \{(\otR, \key, n), (\otW, \key, n+1)\})$, 
where $\txid_{n+1} \in \nextTxid(\mkvs, \cl)$; 
...
%Recall that $\mkvs' \in \updateKV(\mkvs, \viewFun(\cl), \cl, \{(\otR, \key, n), (\otW, \key, n+1)\}$. 
...
%Consider an arbitrary client $\cl'$. If $\cl' \neq \cl$, $\viewFun'(\cl') = \viewFun(\cl')$: if $\txid_{\cl'}^{m} \in \{\WTx(\mkvs'(\key, i))\} 
%\cup \RTx(\mkvs'(\key, i))$ for some $i = 0,\cdots, n+1$, then because $\RTx(\mkvs'(\key, n+1)) = \emptyset$, $\cl' \neq \cl$ and $\txid_{n+1} = \txid_{\cl}^{\cdot}$, 
%then it must be the case that $i \leq n$. Because $(\mkvs, \viewFun)$ satisfies Property \eqref{eq:psi_counter_view}, 
%then there exists an index $j \geq i$ such that $\viewFun'(\cl') = \viewFun(\cl') = [\key \mapsto \{0,\cdots, j\}]$. 
%Finally, suppose that $\cl' = \cl$. 
%By definition of $\ET_{\UA}$, $\viewFun(\cl) = [\key \mapsto \{0,\cdots, n\}]$, and the definition 
%of $\ET_{\MRd}$ and $\ET_{\RYW}$ imply that $\viewFun'(\cl) = [\key \mapsto \{0,\cdots, n+1\}]$. 
%Clearly, whenever $\txid_{\cl}^{\cdot} \in  \{\WTx(\mkvs'(\key, i))\} 
%\cup \RTx(\mkvs'(\key, i))$ for some $i$, then it must be the case that $i \leq n$.
\end{itemize}
...
%assume also that 
\[
(\mkvs, \viewFun) \xrightarrow{(\cl, \mathsf{read}(\key))}_{\ET_{\UA} \cap \ET_{\MRd} \cap \ET_{\RYW}} (\mkvs', \viewFun').
\]
As in the previous case, we have that $\mkvs' = \updateKV(\mkvs, \viewFun(\cl), \txid, \{(\otR, \key, i)\})$, where 
$m = \snapshot(\mkvs, \viewFun(\cl))(\key)$  - 
in particular, because $(\mkvs, \{\txid_{i}\}_{i=1}^{n}, \{\T_{i}\}_{i=0}^{n}$ satisfies 
Property \eqref{eq:psi_counter_shape}, then it must be the case that $m = \max_{<}(\viewFun(\cl)(\key))$ - 
and $\txid \in \nextTxid(\mkvs, \cl)$. 
...
%Henceforth, we let $i = \max_{<}\viewFun(\cl)(\key)$.
...
%otherwise $i = n$ and $\mkvs(\key, i) = \mkvs(\key, n) = (n, \txid_{n}, \T_{n})$. 
%In both cases we have that 
%hence $m = \snapshot(\mkvs(\key), \viewFun(\cl)) = \valueOf(i, \_, \_) = i$.
%
%Because $\mkvs' = \updateKV(\mkvs, \viewFun(\cl), \txid, \{(\otR, \key, n)\}$, 
...
%\[
%\begin{array}{l}
%\mkvs'(\key, i) = \text{let } (v, \txid', \T) = \mkvs(\key, i) \text{ in } (v, \txid', \T \cup \{\txid\}) = \\
%\text{let } (v, \txid', \T) = (i, \txid_{i}, \T_{i} \cup \{\txid_{i+1}\}) \text{ in } 
%(v, \txid', \T \cup \{\txid\}) = (n ,\txid', \T_{i} \cup \{\txid \} \cup \{\txid_{i+1}\}.
%\end{array}
%\]
%Similarly, in the case that $i = n$, we have that $\mkvs'(\key, i) =(i, \txid_{i}, \T_{i} \cup \{\txid\})$.
...
Recall that $m = \max_{<}(\viewFun(\cl)(\key))$; let $i=0,\cdots,n$.

Let again $i = \max_{<}\viewFun(\cl)(\key)$ . 
...
it must be $i \leq j$. Recall that $j = m = \max(\viewFun(\cl)(\key))$, by the Definition of 
$\ET_{\UA}$ it must be the case that $\viewFun(\cl) = [\key \mapsto \{0,\cdots, j\}]$. 
...
case that $i \in \viewFun(\cl)(\key)$, hence  $i \leq m = j$.
...
$\mathsf{Counter}(\key), \mathsf{Counter}(\key'), \cdots$. We show that in this
 ...
 && \\
 \viewFun_0 &=& [\cl_1 \mapsto [\key_1 \mapsto \{0\},  \key_2 \mapsto \{0\}], \cl_2 \mapsto [\key_1 \mapsto \{0\}, \key_2 \mapsto \{0\}]]\\
 \viewFun_1 &=& [\cl_1 \mapsto [\key_1 \mapsto \{0,1\}, \key_2 \mapsto \{0\}], \cl_2 \mapsto [\key_1 \mapsto \{0\}, \key_2 \mapsto \{0\}]]\\
 \viewFun_2 &=& [\cl_1 \mapsto [\key_1 \mapsto \{0,1\}, \key_2 \mapsto \{0\}], \cl_2 \mapsto [\key_1 \mapsto \{0\}, \key_2 \mapsto \{0,1\}]]\\
 \viewFun_3 &=& \viewFun_2\\
 \viewFun_4 &=& \viewFun_2\\
\end{array}
\]
 ...
 \[
 (\hh_0, \viewFun_0) \xrightarrowtriangle{\cl_1, \mathsf{inc}(\key_1)}_{\ET_{\PSI}} (\hh_1, \viewFun_1) \xrightarrowtriangle{(\cl_2, \mathsf{inc}(\key_2)} 
 (\hh_2, \viewFun_2) \xrightarrowtriangle{(\cl_1, \mathsf{read}(\key_2)}_{\ET_{\PSI}} (\hh_3, \viewFun_3) \xrightarrowtriangle{(\cl_2, \mathsf{read}(\key_1)} 
 (\hh_4, \viewFun_4)
 \]
...
%In fact, by definition, $\txid^{i+1} \in \{\WTx(\mkvs(\key_{h}, p)\} \cup \RTx(\mkvs(\key_{h}, p))$ 
%for $h=1,2$ and $p = 0,\cdots, \lvert \mkvs(\key_{h}) \rvert - 1$, 
%$\txid_{j} \in \{ \WTx(\mkvs(\key_{h}, q)\} \cup \RTx(\mkvs(\key_{h}, q))$ for some $q = 0,\cdots, \lvert \mkvs(\key_{h}) \rvert - 1$. 
...
%We have that $\txid^{j} \in \{\WTx(\mkvs(\key_{h}), p_{h})\} \cup \RTx(\mkvs(\key_{h}, \p_{h})$ for some $h=1,2$ and $p_{h} = 0,\cdots, 
...
%Proposition \ref{prop:si_counter_hhshape1}. Let $h, l \in \{1,2\}$ with $h \neq l$; then 
%\begin{align} 
%\forall i, j = 0,\cdots, \key_{h} - 1, p, q = 0,\cdots, \key_{l} - 1, \txid, \txid', \txid'', \txid'''. \txid \in \T_{i}^{\key_h} \wedge 
%\txid' \in \T_{j}^{\key_{h}} \wedge \txid'' \in \T_{p}^{\key_{l}} \wedge \txid''' \in \T_{q}^{\key_{k}} \wedge 
