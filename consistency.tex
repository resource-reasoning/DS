...
disallowed by \SI.  
In our specification, \( \ET_\SI \) is the combination of \( \ET_\UA \), all constrains from \( \ET_\CP \),
and \( \ddagger \), a strengthening of \( \dagger \) (\cref{fig:execution.test}).
Previously in \( \CP \), if \( (\txid, \txid'') \in \WW_\hh \) and \( (\txid'',\txid') \in \RW_\hh \),
transaction \( \txid \) is not necessary committed before \( \txid' \):
\[
    \begin{array}{l}
    ...
    \end{array}
\]
However, write-conflict freedom enforces \( \txid''\) must see \( \txid \), that is,
