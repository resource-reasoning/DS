...
\citeN{session2causal} defined \emph{Causal Consistency} 
as the conjunction (composition) of the four \emph{session guarantees} \(\MR\), \(\MW\), \(\RYW\) and \(\WFR\). 
Yet it is NOT the case \( \ET_\CC = \ET_{\MR} \cap \ET_{\MW} \cap \ET_{\RYW} \cap \ET_{\WFR}\) here.
...
which are weaker than \( \rel_{\MW} \cup \rel_{\WFR} \), that is \( \rel_{\MW} \cup \rel_{\WFR} \subseteq \SO \cup \WR_\mkvs \).
