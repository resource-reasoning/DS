...
%In~\cref{sec:other_formalisms,app:compositionality}, we also prove that $\CMs(\ET_1 \cap \ET_2) = \CMs(\ET_1) \cap
...
We now give examples of execution tests
in~\cref{fig:execution.tests}. The associated consistency models for kv-stores correspond to
widely adopted consistency guaranteees for distributed databases, as
we show in Section|ref{5} and the Appendix~\ref{.}. 
...
but this information is enough to provide the consistency models we
seek. 


Consider the relations $\RF_{\hh}$ and $\VO_{\hh}$ in Figure~\ref{.},
and $\PO$ defined in Section~\ref{.}. If $(t,t')$ is in one of these
relations, it {\em must} be the case that $t$ commits before the
invocation of $t'$. 
...
the task
is to identify the writes that occured before $t$. 
In Figure~\ref{.}, we define the relations


Given key $k$ and transaction $t$ that writes to index $i$, 
...
the direct writes that have occured before $t$.
...
Theorem~\ref{??}  only holds under certain conditions on the
...
\pg{In above, presumably you can pinpoint exactly why these conditions are not
  ...
  need $\ddagger$ not $\dagger$? For me $\ddagger$ seems to be much
  simpler than $\dagger$ in the sense that we know what it captures.} 


We are able to capture  $\SI$  by strengthening the $\dagger$ in Figure~\ref{.} to $\ddagger$. 
The transation relation $ ((\PO \cup \RF_{\hh} \cup \VO_{\hh}) ;
\AD_{\hh}^?)^{+} \ni (t,t') $  states that, if transaction $t'$
writes to the kv-store, then the relation identifies  {\em all} the previous
transactions $t$ that {\em must} have written to the kv-store. For
example, consider $(t,t') \in \VO_{\hh}; \AD_{\hh}$ with $(t,t'') \in
\VO_{\hh}$ and $(t'', t') \in \AD_{\hh}$. Since $t''$ starts before
$t'$ commits and $t$ commits before $t''$ starts, it means that $t$
{\em must} have commited before $t'$ starts. The $\ddagger$ relation
captures the property that if a transation observes a
