...
%and an initial configuration $\conf_0$ (\cref{def:configuration}),
...
%     \wedge \WTx(\hh(\ke', j)) \toEdge{(\WW_\hh \cup \WR_\hh \cup \SO \cup ( (\WR_\hh \cup \SO) ; \RW_\hh ) \cup (\WW_\hh; \RW_\hh) )^{+}} \WTx(\hh(\ke, i)) 
        \implies j \in \vi(\ke')    
    \end{rclarray}
\]
\vspace*{-5pt}
...
Similarly, $(\txid, \txid') \in \SO$ implies that $\txid'$ 
...
%$\SI = \CP \wedge \UA$~\cite{framework-concur,SIanalysis}
...
place of kv-stores. See \cref{?}}. For example, the kv-store of \cref{fig:si-disallowed} is
...
%is the combination of \( \ET_\UA \), all constraint from \( \ET_\CP \),
%and \( \ddagger \), a strengthening of \( \dagger \) (\cref{fig:execution.tests}). 
...
%Because Snapshot Isolation enforces write-conflict freedom, 
Under $\SI$, if  $\txid \xrightarrow{\VO_{\hh}} \txid''$ it must be not only the case that $\txid$ commits 
before $\txid''$, but $\txid$ commits before $\txid''$ start. Because $\txid'' \xrightarrow{\AD_{\hh}} 
\txid'$ implies that $\txid''$ starts before $\txid'$ commits, then it must be the case that 
when $\txid \xrightarrow{\VO_{\hh} ; \AD_{\hh}} \txid'$, then $\txid$ commits before $\txid'$ does. 
%Previously in \( \CP \), if \( (\txid, \txid'') \in \WW_\hh \) and \( (\txid'',\txid') \in \RW_\hh \),
%transaction \( \txid \) may not commit before \( \txid' \):
%\[
%    \begin{array}{l}
%    \func{commit}{\txid} \to \func{commit}{\txid''} 
%    \land \func{start}{\txid''} \to \func{commit}{\txid'}  \\
%    {} \qquad \not\implies \func{commit}{\txid} \to \func{commit}{\txid'}
%    \end{array}
%\]
%However, write-conflict freedom (\( \UA \)) enforces \( \txid''\) must see \( \txid \), that is,
%\( \func{commit}{\txid} \to \func{start}{\txid''} \).
%This means, as result of \( \UA \), commit of transactions \( \txid \) must commit before \( \txid' \) commits.
%----------

\pg{
explain WW; RW tc --> t''c and t''s ---> t'c, so not always before
with UA (write-conflict freedom), we have tc --> t''s so not must be before
hence double dagger
explain example
}
