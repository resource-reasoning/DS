...
\[  
    \begin{rclarray}
        ...
        \fora{\ke, \ke', i, j}
         i \in \vi(\ke)  \wedge 
         \WTx(\hh(\ke', j)) \toEdge{(\WW_\hh \cup \WR_\hh \cup \SO \cup ( (\WR_\hh \cup \SO) ; \RW_\hh ) )^{+}} \WTx(\hh(\ke, i))
        \implies j \in \vi(\ke')  \\
        \ddagger 
        & \eqdef &
        \fora{\ke, \ke', i, j}
...
If $(\txid,\txid') \in \WW_\hh$ then it {must} be
the case that $\txid$ commits before the commit of $\txid'$.
If $(\txid,\txid') \in (\WR_\hh \cup \SO)$ then it {must} be
the case that $\txid$ commits before the start of $\txid'$, hence before the commit of $\txid'$.
If $(\txid,\txid'') \in (\RF_{\hh} \cup \PO)$ and $(\txid'',\txid') \in \AD_\hh$, then it {must} also be the case that $\txid$ commits before the commit  of $\txid'$, since a property of
$\txid'' \toEdge{\AD_\hh} \txid$ is that the start of $\txid''$ must occur before $\txid'$ commits.
The relation $(\WW_\hh \cup \WR_\hh \cup \SO \cup (\WR_\hh \cup \SO) ; \RW_\hh )^{+} \ni (\txid, \txid')$
captures that, if transaction $\txid'$ in kv-store \( \mkvs \), then the
relation identifies {\em all} the transactions $\txid$ that {must} have already committed to the kv-store.
...
disallowed by \SI.  
In our specification, we also strengthen \( \dagger \) to \( \ddagger \) (\cref{fig:execution.test})
