...
\item $\alpha = \fp$ for some $\fp$, $\ET \vdash (\mkvs, \vi ) \csat \fp : (\mkvs', \vi')$ for some $\vi'$, $\mkvs' \in \updateKV[\mkvs, \vi, \fp, \cl]$, 
    ...
    have the form $\ET \vdash (\mkvs, \vienv) \csat_{\cl} \fp: \vi'$; then one can lift this to a $\ET$-reduction in a style similar to monadic lifting. \sx{It makes sense to do so. } }
\end{enumerate}
