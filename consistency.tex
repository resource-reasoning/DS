...
When the total order in which transactions commit is known, then $\CP$
is described by the following property: if a client observes the
effects of a transaction $\txid$, then it must also observe the
effects of any transaction that commits before $\txid$.  Our
configurations of kv-stores and views only provides {\em partial}
information about the order in which transactions commit, but this
information is enough to define the consistency models we seek.
...
$(t,t')$ is in $\RF_{\hh}$ or $\PO$ then it {must} be
the case that $t$ commits before the start of $t'$.
 If
$(t,t')$ is in 
 $\VO_{\hh}$ then it {must} be
the case that $t$ commits before the commit of $t'$.
If $(t,t')$ is in
$(\RF_{\hh} \cup \PO); \AD_{\hh}$, then it {must} also be the case that
$t$ commits before the commit  of $t'$, since a property of
