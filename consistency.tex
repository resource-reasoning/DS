...
\[  
    \begin{rclarray}
        ...
        \fora{\key, \key', i, j}
        i \in \vi(\key)
        \wedge \WTx(\mkvs(\key', j)) \toEDGE{((\SO ; \RW_{\mkvs}^{?}) \cup (\WR_{\mkvs} ; \RW_{\mkvs}^{?}) \cup (\WW_\mkvs; \RW_\mkvs^{?}) )^{+}} \WTx(\mkvs(\key, i))
        \implies j \in \vi(\key')    
    \end{rclarray}
\]
%
\[
    \begin{rclarray}
       \WR_{\mkvs} &\defeq& \{ (\txid, \txid') \mid \exists \key, i.\; \txid = \WTx(\mkvs(\key, i)) \wedge \txid' \in \RTx(\mkvs(\key, i))\}\\
     \WW_{\mkvs} &\defeq& \{ (\txid, \txid') \mid \exists \key, i, j.\; \txid = \WTx(\mkvs(\key, i)) \wedge \txid' = \WTx(\mkvs(\key, j)) \wedge i < j\}\\
...
$\CMs_{\CC} \defeq \CMs_{\MRd} \cap \CMs_{\MW} \cap \CMs_{\RYW} \cap \CMs_{\WFR}$. 
...
be computed from $\SO, \WR_{\mkvs}, \WW_{\mkvs}, \AD_{\mkvs}$. This property is captured 
...
$\txid'$ commits. 
As a consequence, if 
$(\txid,\txid'') \in \WR_{\mkvs}$ (resp. \SO) and $(\txid'',\txid') \in \AD_\mkvs$, then it {must} also be the case that $\txid$ commits before the commit  of $\txid'$, 
...
from $\SO, \WR_{\mkvs}, \WW_{\mkvs}, \AD_{\mkvs}$. However, because snapshot isolation enforces 
...
when $(\txid,\txid') \in \WW_{\mkvs} ; \AD_{\mkvs}$ then $\txid$ commits before $\txid'$ does. 
