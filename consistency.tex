...
If $(\txid,\txid'') \in \RF_{\hh}$ (resp. \SO) and $(\txid'',\txid') \in \AD_\hh$, 
then it {must} also be the case that $\txid$ commits before the commit  of $\txid'$, since a property of
...
captures that, {\em all} the transactions $\txid$ that {must} 
have already committed to the kv-store before the commit of \( \txid' \).
...
disallowed by \SI.  
In our specification, \( \ET_\SI \) is the combination of \( \ET_\UA \), all constraint from \( \ET_\CP \),
...
As result of \( \UA \) the commit of transactions \( \txid \) must commit before \( \txid' \) commits, so it should be included in the view.
In \cref{fig:si-disallowed}, transaction \( \txid_4 \) reads the last version of \( \ke_2 \) written by \( \txid_3 \) then installs a newer version so does not violate \( \UA \).
Let us consider the read especially for \( \ke_2 \).
From the \( \txid_3 \) we have edges:
\(
...
the Clock-SI implementation~\cite{Du:2013:CSI:2553409.2553434} satisfies
$\ET_{\SI}$. 
