...
Recall from Definition~\ref{def:execution.test} that an execution test is a relation
...
they impose 
constraints on the structure of a kv-store. On the other hand, execution 
...
%%%transaction commit: 
Given $\mkvs, \mkvs'$, let $\txid$ be a transaction included in $\mkvs'$ 
but not in $\mkvs$. For our purposes, $\txid$ is uniquely determined 
...
Define $\rel_{\RYW} \defeq \Set{(\txid', \txid) }[ \txid \in \T \land (\txid', \txid) \in \SO\rflx ]$. 
Then the predicate $\closed[\mkvs', \vi', \rel_{\RYW}]$ states that a 
...
if a client sees a version written by another
...
the commit of $\txid'$: as a consequence, if $\txid \xrightarrow{\WR} \_ \xrightarrow{\RW} \txid'$, 
or $\txid \xrightarrow{\WR} \_ \xrightarrow{\RW} \txid'$, then $\txid$ commits before $\txid'$.
...
approximates the order in which transactions have been executed. 
...
it forbids all the  anomalies forbidden by these consistency models,  
e.g the long fork (\cref{fig:cp-disallowed}) and the lost update (\cref{fig:ua-disallowed}). 
On the other hand, $\WSI$ is also weaker than snapshot isolation, meaning that it allows any of the anomalies 
of $\SI$, such as the write skew (\cref{fig:ser-disallowed}). The inclusion between $\WSI$ 
and $\SI$ is strict, and in fact the kv-store from \cref{fig:si-disallowed} is allowed 
in $\WSI$: if we use the execution test $\ET_{\CP} \cap \ET_{\UA}$, then 
...
However, transaction $\txid_{4}$ cannot be executed using this same view when we use $\ET_{\SI}$. 
Because $\txid_{4}$ reads the version of $\key_2$ written by $\txid_3$, 
