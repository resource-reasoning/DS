...
%for all $\mkvs, \mkvs', \vi, \vi', \fp, \ke$:
%\[
%\begin{array}{@{} r @{\hspace{10pt}} l @{}}
%\text{if} & \ET \vdash (\mkvs, \vi) \triangleright \fp: (\mkvs',\vi' ) 
%\text{and} (\otW, \ke, \_) \in \fp \\
%\text{then} & (\otR, \ke, \_) \in \fp
%\end{array} 
%\]
...
%that can be obtained after a client commits $\f$ to $(\mkvs, \vi)$ using $\ET_1$ and $\ET_2$. 
