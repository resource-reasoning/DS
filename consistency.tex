...
such as the one depicted in \cref{fig:ua-disallowed}, it has obvious performance drawbacks due to clients
...
and achieve very fast transaction processing, at the expense
...
an \emph{execution test}, which specifies whether a client is allowed to commit a transaction in a given 
kv-store. Each execution test induces a consistency model as the set of kv-stores that 
can be obtained when a client non-deterministically commits transactions that respect the constraints 
...
%    \ac{This second condition was needed for compositionality results, and is no longer needed. 
%    I am removing it.}
%\end{align}%
%}%
\end{definition}
...
\begin{enumerate}
    \item $\alpha = \varepsilon$, $\mkvs' = \mkvs$ and $\vienv' = \vienv\rmto{\cl}{\vi}$ for some $u: \vienv(\cl) \sqsubseteq u$, i.e.\  
	client $\cl$ advances its view to a more up-to-date one, where the label $\varepsilon$ denotes that there was no interaction between the client and the kv-store; or 
\item $\alpha = \fp$ for some $\fp$, $\ET \vdash (\mkvs, \vi ) \csat \fp : (\mkvs', \vi')$ for some $\vi'$, $\mkvs' \in \updateKV[\mkvs, \vi, \fp, \cl]$, 
    $\vienv' = \vienv\rmto{\cl}{\vi'}$, i.e.\ client $\cl$ 
...
We refer to a finite sequence of $\ET$-reductions starting in an initial configuration $\conf_{0}$ as an $\ET$-trace. 
Each $\ET$-trace determines a configuration $(\mkvs, \stub)$: the 
...
Note that in $\ET$-traces the two operations performed by a client, namely view-shifts and 
transaction commits, are decoupled. This is in contrast with what happens in the operational model (\cref{sec:model}), 
where view-shifts and transaction commits are always coupled together in a single transition of a program. 
...
protocol implements a given consistency model, in which case it is convenient to separate the operation of shifting a view from that of committing a transactions, 
...
That is, if there exists a kv-store $\mkvs \in \CMs(\ET)$ that can never be obtained as a 
...
constraining the view of clients either before or after committing a transaction: intuitively, such constraints impose properties of the form \emph{if the client 
observes version $\ver$ in the kv-store, then it must also observe 
the set of versions $f(\ver)$, for a suitable notion of $f$}. 
In practice, this is achieved by requiring views to be prefix-closed with respect 
to a relation  expressed in terms of dependencies between transactions.
We present the formal notions of transaction dependencies, inspired by 
...
\end{enumerate*}
\noindent \cref{fig:dependencies} graphically illustrates dependencies for kv-stores.
...
possible to recover the view of the client whose set of visible transactions is $\T$: 
$\getView(\mkvs, \T) = \lambda \key. \{0\} \cup \{i \mid \WTx(\mkvs(\key, i)) \in \T\}$. 
The reflection between views and sets of transactions allows us to express concisely 
...
All the formal definitions are given in \cref{fig:execution.tests}. In this table we 
commit an abuse of notation and let $\closed(\mkvs, \vi, R)$ be the execution test 
$\{(\mkvs, \vi, \fp, \mkvs', \vi') \mid \closed(\mkvs, \vi, R)\}$.
...
mandating that, after a client executes a transaction, its view contains all the writes of the client 
...
as a closure property of the view after the transaction commit as follows. 
Given two kv-stores $\mkvs, \mkvs'$, let  
...
when updating the kv-store from $\mkvs$ to $\mkvs'$. 
For a given set of transactions $\T$, let $R_{\RYW}(\txid) = \{(\txid', \_) \mid \exists \txid \in \T.\; \txid' \xrightarrow{\SO\rflx} \txid\}$. 
...
This consistency model states that, if a transaction sees a version written by a
...
At first sight, one would expect the execution test for causal consistency to be exactly 
the intersection of the execution tests for the individual session guarantees: $\ET_{\MR} \cap \ET_{\MW} \cap \ET_{\RYW} \cap 
\ET_{\WFR}$. In practice, some care needs to be taken. 
Recall that $\ET_{\MW}$ and $\ET_{\WFR}$ have been 
defined in terms of closure properties over the view of a client before committing a transaction with respect 
to two relations $R_{\MW} = \SO$ and $R_{\WFR}$.
As such, to build an execution test that composes $\ET_{\MW}$ and $\ET_{\WFR}$,
we need to require that the view of a client prior to executing a transaction be 
closed simultaneously with respect to $R_{\MW}$ and $R_{\WFR}$, i.e. iclosed with respect to 
$R_{\MW} \cup R_{\WFR}$. 
Let $\ET_{\MR+\WFR} = \{(\mkvs, \vi, \fp, \mkvs', \vi') \mid \closed(\mkvs, \vi, R_{\MW} \cup R_{\WFR})\}$. 
...
to an execution test of the form $\ET_{\MR+\RYW}$. However, in this particular case we obtain 
...
This model has been proposed by \citet{framework-concur} 
...
That is, when two transactions need to write to the same key, one must see another.
...
initial version of a key, this is equivalent to requiring that the view of a client wanting 
to commit a transaction be \emph{suffix-closed} with respect to the relation $\WW(\key)$, 
or equivalently prefix-closed with respect to $\WW^{-1}(\key)$ for any key $\key$ written in the 
fingerprint $\fp$.
\pg{
--------not done from here----------------------
}
...
As with causal consistency, we cannot define $\ET_{\PSI}$ as $\ET_{\CC} \cap \ET_{\UA}$, because 
this execution test only mandates that the views before committing transactions be individually closed with respect to 
the relations $R_{\CC}$ and $R_{\UA}(\fp)$. The definition in \cref{fig:execution.tests} ensures that 
...
this information is sufficient to formalise $\CP$ \cite{laws}.

The best way to understand how the order in which transactions 
committed can be approximated from the relations $\WR_{\mkvs}, \WW_{\mkvs}, 
\RW_{\mkvs}$ and $\SO$ is in terms of an idealised implementation of 
...
With respect to this implementation, a write-read ($\WR_{\mkvs}$) dependency from transaction $\txid$ to transaction $\txid'$ 
means that 
...
A write-write ($\WW_{\mkvs}$) dependency from $\txid$ to $\txid'$ means that $\txid$ {must} commit before $\txid'$. 
Finally, a read-write ($\RW_{\mkvs}$) anti-dependency from $\txid$ to $\txid'$ 
...
related by $\WR_{\mkvs}$ or $\SO$), and there is an anti-dependency ($\RW_{\mkvs}$)
...
$\CP$. Therefore, we define $\ET_{\CP}$ by requiring that the view of a client prior to committing a transaction be 
...
where the latter ensures that a client sees its own commits.
For the long fork anomaly in \cref{fig:cp-disallowed},
...
\),
which means, by \( \dagger \) that transaction \( \txid_{\cl_2}^2 \) must see \( \txid \).
However, \(  \txid_{\cl_2}^2 \) reads an older version of \( \key_1 \) than the one written by \( \txid \).
Symmetrically,
...
As with $\CC$ and $\PSI$, we cannot define 
...
their view of the kv-store store is complete; this can be enforced by requiring the view 
of a client to be closed with respect to the relation $\WW^{-1}$.
...
\mypar{Weak Snapshot Isolation: a new consistency model} 
\label{sec:new_cm}
