...
such that for all $(\mkvs, \vi, \fp, \mkvs', \vi') \in \ET$, $\key \in
\mkvs$ and $\val \in \Val$, if $(\otR, \key, \val) \in \fp $ then 
...
fingerprint $\fp$ to obtain the resulting kv-store $\mkvs'$ (Definition~\ref{...} ) with  view
$\vi'$. We adopt the 
%more suggestive 
...
such as the long fork (\cref{fig:cp-disallowed}) and the lost update (\cref{fig:ua-disallowed}). 
However, $\WSI$ is weaker than SI, meaning that it allows any of the anomalies 
...
in $\WSI$ but  not in  SI. If we use the execution test $\ET_{\CP} \cap \ET_{\UA}$, then 
...
Because WSI is  weaker than SI, potential implementations of WSI might perform
better than those  of \(\SI\). 
Because of the similarity between these two consistency models, we believe that many applications that 
