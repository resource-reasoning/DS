...
We give example execution tests that define the well-known
consistency models from the literature We need to say something about
assumptions:
\begin{itemize}
...
Recall from Definition~\ref{4.1} that an execution test is a set of
tuples written in the form $\ET \vdash (\mkvs,\vi) \csat \fp:
(\mkvs', \vi')$.The purpose of our  execution tests are to  constrain  the views, the $\vi$
and $\vi'$, to those that make sense for a particular consistency model. 



Recall that the resulting kv-store $\mkvs'$ is uniquely
determined by the initial kv-store $\mkvs$, the initial view $u$ and
the fingerprint set $\fp$ using Definition~\ref{3.8}. 


Given $\mkvs$, $\vi$ and $\fp$, the updated kv-store 
Given $\mkvs',
\vi'$ and $\fp$, the  execution
test determines  the appropriate views, 
...
\citeN{session2causal} proved that \emph{Causal Consistency} 
...
test for the four consistency guarantees, in accordance with the result from \citeN{session2causal}.
%
