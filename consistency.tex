...
\\
\MW & 
\(\allowed[\mkvs, \vi, \SO\rflx] \)
&
\PSI & $\ET_{\CC} \cap \ET_{\UA}$
\\
\RYW & $ \txid \in \mkvs' \land \txid \notin \mkvs \land \wtOf(
\mkvs'(\key, i) ) \toEDGE{\SO\rflx} \txid \implies i \in \vi'(\key) $
&
\CP & \( \Set{(\mkvs, \vi, \fp, \mkvs', \vi')}[\allowed[\mkvs,\vi,\dagger]] \cap \ET_\MR \cap \ET_\RYW \) 
\\
\WFR & 
\( \allowed[\mkvs, \vi, \WR_{\mkvs} ; \SO\rflx] \)
&
$\SI$ & $\Set{(\mkvs, \vi, \fp, \mkvs', \vi')}[\allowed[\mkvs,\vi,\ddagger]] \cap \ET_\MR \cap \ET_\RYW  \cap \ET_\UA $
\\
\CC & $\ET_{\MR} \cap \ET_{\MW} \cap \ET_{\RYW} \cap \ET_{\WFR}$
&
...
\end{align*}
\begin{align*}
    \allowed[\mkvs, \vi, R] & \defeq 
    \vi = \getView[\mkvs, \lfpTx[\mkvs,\Tx[\mkvs,\vi], R]] 
    \quad \lfpTx[\mkvs,\txidset, R]  \defeq \mu X . \txidset \cup R^{-1}(X)  \\
    \Tx[\mkvs, \vi] & \defeq 
...
%&
%\UA &   \( \uaWrite[\mkvs,\fp] \) & \( \emptyset \)
%\\
%\MW & \( \emptyset \) & \( \SO\rflx \)
%&
%\PSI & \( \dagger \) & \( \SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}\)
%\\
...
%&
%$\SI$ & \( \dagger  \) & \( \ddagger \cup (\WW_\mkvs; \RW_\mkvs\rflx) \)
%\\
%\CC & \( \clRead[\mkvs,\cl] \cup \clWrite[\mkvs,\cl] \)  & \( \SO \cup \WR_{\mkvs} \)
%&
...
%\begin{align*}
    %\dagger & \equiv 
    %\clRead[\mkvs,\cl] \cup \clWrite[\mkvs,\cl] \cup \uaWrite[\mkvs,\fp]
    %\quad \ddagger 
    %\equiv 
    %(\SO ; \RW_{\mkvs}\rflx) \cup (\WR_{\mkvs} ; \RW_{\mkvs}\rflx) \cup \WW_\mkvs 
%\end{align*}
...
%\begin{align*}
    ...
    %\extRead[\mkvs,\vi,\fp] & \defeq \Set{\wtOf[\mkvs(\key,\max(\vi(\key)))]}[ \exsts{\key,\val} (\otR,\key,\val) \in \fp] \\
    ...
    %\getView[\mkvs, \txidset] \defeq 
    %\lambda \key. \Set{0} \cup \Set{ i }[\wtOf(\mkvs_{\aexec}(\key, i)) \in \txidset] \\
    %\uaWrite[\mkvs,\fp] & \defeq \Set{\wtOf[\mkvs(\key,i)]}[\exsts{\val} (\otW,\key,\val) \in \fp \land 0 \leq i < \abs{\mkvs(\key)}] 
%\end{align*}
%
\hrulefill
