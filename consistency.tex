...
been proposed in the literature \cite{principle-eventual-consistency,rola,cops,redblue,PSI,clocksi}, which capture different trade-offs 
between transaction processing performance and application correctness. 

%For example, the consistency model known as 
\emph{Serialisability} allows only kv-stores 
...
Formally, a \emph{consistency model} is a set of kv-stores. We introduce the notion of 
\emph{execution test}, which specifies whether a client is allowed to commit a transaction in a given 
kv-store. Each execution test induces a consistency model: this is the set of kv-stores obtained 
by letting clients non-deterministically commit transactions, while respecting the constraints 
...
such that for all $(\mkvs, \vi, \fp, \mkvs', \vi') \in \ET$ and all $\key, \val$, \( (\otR, \key, \val) \in \fp \implies
...
a client with view $\vi$ over a kv-store $\mkvs$ can commit a transaction with 
fingerprint $\fp$. The kv-store and view $\mkvs', \vi'$ reflect how $\mkvs, \vi$ change due 
to the transaction being executed, respectively\footnote{In practice, $\mkvs'$ is almost completely 
determined by the function $\updateKV$ from \cref{def:updatekv}, with the only degree of freedom 
being the identifier of the client committing the update.}. We adopt the 
%more suggestive 
notation $\ET \vdash (\mkvs, \vi) \csat \fp: (\mkvs', \vi')$ in place of $(\mkvs, \vi, \fp, \mkvs', \vi') \in \ET$.
%The condition of 
\cref{def:execution.test} enforces the last-write-wins policy~\cite{vogels:2009:ec:1435417.1435432}: 
...
We now explain how an execution test induces a consistency model. First, given an execution test $\ET$, 
we define $\ET$-reductions: these capture how a client $\cl$ may interact with configurations. 
...
\begin{enumerate}
	...
	client $\cl$ shifts its view to a more up-to-date one, the label $\varepsilon$ denotes that there was no interaction between the client and the kv-store; or 
\item $\alpha = \fp$ for some $\fp$, and $\ET \vdash (\mkvs, \vi ) \csat \fp : (\mkvs', \vi')$, where $\mkvs' \in \updateKV[\mkvs, \vi, \fp, \txid]$ 
   for some $\txid \in \nextTxid(\cl, \mkvs)$, $\vienv' = \vienv\rmto{\cl}{\vi'}$; client $\cl$ 
...
$\ET$-traces and the operational semantics are equally expressive: 
...
\subsection{Example Execution Tests}\label{subsec:cm_examples}

----paragraph describing the whole section--------

We give examples of execution tests to define 
consistency models on kv-stores. In Section~\ref{Section5}
we show that these definitions  are 
equivant to 
definitions of consistency models for  dependency graphs and abstract
execution given in the literature~\cite{....}.
...
Recall from Definition~\ref{4.1} that an execution test is a relation
given by a set of tuples written in the form
$\ET \vdash (\mkvs,\vi) \csat \fp: (\mkvs', \vi')$. The purpose of
such execution tests is  to constrain the views $\vi$ and $\vi'$ to
those that make sense for a particular consistency model. We study 
two styles of execution test: those that contrain the resulting view
...
so-called visible 
transactions visible of a view. 
Given kv-store $\mkvs$ and view $\vi$, the {\em set of visible
...
of the client by
\[
\getView[\mkvs, \T] \defeq \lambda \key. \Set{0} \cup \Set{i }[
...
relations for kv-stores inspired by the 
well-known  transaction dependency relations due to Adya \cite{adya}.
...
similar to those for dependency
graphs \cite{adya}, their use  is fundamentally different.  
The definitions of consistency 
models on dependency graphs are declarative: as 
...
This consistency model states that whenever a view includes a version for a key \( \key \) 
...
The execution test $\ET_{\RYW}$ states that the resulting view
contains all the writes of the client, ensuring that they  will be included in the view of the client when committing future 
transactions.
...
in our operational model this set is the  singleton set $\Set{\txid}$ where $\txid$ is the newly committed transaction.
