...
is the smallest relation such that for all $\cl, \hh, \hh', \viewFun, \opset, \vi', \vi''$ and $\vi = \viewFun(\cl)$:

\vspace*{-2pt}
{\small
\begin{array}{@{}l@{}}
    \vi \sqsubseteq \vi'' \\
    \quad \hh' \in \updateKV(\hh, \vi'', \opset, \cl)
    \land (\mkvs, \vi'', \opset, \mkvs', \vi') \in \ET \\
    \qquad \implies
    (\hh, \viewFun) \toET{\hspace{-5pt}(\cl, \varepsilon)\hspace{-5pt}}{\ET} 
    (\hh, \viewFun\rmto{\cl}{\vi''}) \toET{\hspace{-5pt}(\cl,\f)\hspace{-5pt}}{\ET} (\hh', \viewFun\rmto{\cl}{\vi'})$
\end{array}
}
...
\(
\txid_1 \toEdge{\WW_\hh} \txid_2 \toEdge{\RW_\hh} \txid_3
\).
