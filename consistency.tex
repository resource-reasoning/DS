...
\\
\MW & 
$j \in \vi(\key) \wedge \WTx(\mkvs(\key', i)) \toEDGE{\SO^?} \WTx(\mkvs(\key, j)) 
\implies i \in \vi(\key')$ &
\PSI & $\ET_{\PSI} = \ET_{\CC} \cap \ET_{\UA}$
\\
\RYW & $ \txid \in \mkvs' \wedge \txid \notin \mkvs \land \WTx(
\mkvs'(\key, i) ) \toEDGE{\SO^?} \txid \implies i \in \vi'(\key) $ &
\CP & \( \Setcon{(\mkvs, \vi, \fp, \mkvs', \vi')}{\dagger} \cap \ET_\MRd \cap \ET_\RYW \) 
\\
\WFR & $j \in \vi(\key) \wedge \txid \in \RTx(\mkvs(\key', i)) \wedge \txid {\toEDGE{\SO^?}}
