...
\\
\MW & 
$j \in \vi(\key) \wedge \wtOf(\mkvs(\key', i)) \toEDGE{\SO^?} \wtOf(\mkvs(\key, j)) 
\implies i \in \vi(\key')$ &
...
\\
\WFR & $j \in \vi(\key) \wedge \txid \in \rsOf(\mkvs(\key', i)) \wedge \txid {\toEDGE{\SO^?}}
...
\[  
    \begin{rclarray}
        ...
        \fora{\key, \key', i, j}
         i \in \vi(\key)  \wedge 
         \wtOf(\mkvs(\key', j)) \toEDGE{((\SO ; \RW_{\mkvs}^{?}) \cup (\WR_{\mkvs} ; \RW_{\mkvs}^{?}) \cup \WW_\mkvs )^{+}} \wtOf(\mkvs(\key, i))
        \implies j \in \vi(\key')  \\
        \ddagger 
        & \defeq &
        \fora{\key, \key', i, j}
        i \in \vi(\key)
        \wedge \wtOf(\mkvs(\key', j)) \toEDGE{((\SO ; \RW_{\mkvs}^{?}) \cup (\WR_{\mkvs} ; \RW_{\mkvs}^{?}) \cup (\WW_\mkvs; \RW_\mkvs^{?}) )^{+}} \wtOf(\mkvs(\key, i))
        \implies j \in \vi(\key')    
    \end{rclarray}
\]
%
