...
\[  
    \begin{rclarray}
        ...
        \fora{\ke, \ke', i, j}
         i \in \vi(\ke)  \wedge 
         \WTx(\hh(\ke', j)) \toEdge{(((\PO \cup \RF_{\hh}) ; \AD_{\hh}) \cup \PO \cup \RF_{\hh} \cup \VO_{\hh})^{+}} \WTx(\hh(\ke, i))
        \implies j \in \vi(\ke')  \\
        \ddagger 
        & \eqdef &
        \fora{\ke, \ke', i, j}
        i \in \vi(\ke)
        \wedge \WTx(\hh(\ke', j)) \toEdge{(((\PO \cup \RF_{\hh}) ; \AD_{\hh}) \cup \PO \cup \RF_{\hh} \cup \VO_{\hh} \cup (\WW_\hh ; \RW_\hh) )^{+}} \WTx(\hh(\ke, i))
        \implies j \in \vi(\ke')    
    \end{rclarray}
\]
\vspace*{-5pt}
...
$\AD_{\hh}$ defined in Figure~\ref{fig:execution.tests}, adapted from well-known
transaction relations associated with dependency graphs~\cite{adya-icde,adya}. 
If $(t,t')$ is in $\RF_{\hh}$, $\VO_{\hh}$ or $\PO$ then it {must} be
