...
\\
\MW & 
$j \in \vi(\key) \wedge \WTx(\mkvs(\key', i)) \xrightarrow{\SO^?} \WTx(\mkvs(\key, j)) 
\implies i \in \vi(\key')$ &
\PSI & $\ET_{\PSI} = \ET_{\CC} \cap \ET_{\UA}$
\\
\RYW & $ \txid \in \mkvs' \wedge \txid \notin \mkvs \land \WTx(
\mkvs'(\key, i) ) \xrightarrow{\SO^?} \txid \implies i \in \vi'(\key) $ &
\CP & \( \Setcon{(\mkvs, \vi, \fp, \mkvs', \vi')}{\dagger} \cap \ET_\MRd \cap \ET_\RYW \) 
\\
\WFR & $j \in \vi(\key) \wedge \txid \in \RTx(\mkvs(\key', i)) \wedge \txid {\xrightarrow{\SO^?}}
...
prefix-closed with respect to the relation $\SO^?$.
...
client $\cl$, then it must also see the versions previously read by $\cl$ (in $\SO^?$ relation).
