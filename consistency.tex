...
consistency models  over abstract executions \cite{framework-concur} and dependency graphs \cite{adya}.
...
In Section~\ref{4.1}, we give many examples of execution test. Here, we
...
(Definition~\ref{3.3}) ends in a configuration $(\mkvs, \stub)$ where the 
...
Notice that, in the definition of $\ET$-traces, the view-shifts and 
transaction commits  are decoupled. This is in contrast with the
operational semantics (\cref{sec:model}, Fig.~\ref{3}), 
...
where  $\vshift \mkvs \vi {\mkvs'} {\vi'}$ and $\cancommit \mkvs \vi \fp$. 
...
%
We define $\vshiftname$ and $\cancommitname$ for several consistency
...
anlogous relations for  dependency graphs due to Adya \cite{adya}.
...
execution tests using \vshiftname and \cancommitname in Fig.~\ref{4}. 
In Section~\ref{.}, we demonstrate that the associated consistency
