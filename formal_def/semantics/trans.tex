...
An \emph{operation map} \( \op \in \Ops \eqdef \Addr \fm \Val \times \pset{\Set{\otR, \otW}} \) is function associating each address with its tags.
The \emph{operation tags} \( \etR \) and \( \etW \) correspond to read and write respectively.
...
\[ 
\begin{rclarray}
    (\op \composeO \op')(a) & \defeq & 
    \begin{cases}
        \op(a)  & \text{if } a \in \dom(\op) \land a \not\in \dom(\op') \\
        \op'(a)  & \text{if } a \in \dom(\op') \land a \not\in \dom(\op) \\
        \text{undefined} & \text{otherwise}
    \end{cases}
\end{rclarray}
\]
%
A partial binary operation \( \addO \) updates an operation map $\op$ with a tuple $(t, \addr, \val)$ where $t \in \Set{\etR, \etW}$ as follows:
\[
\begin{rclarray}
    \op \addO (\etR, \addr, \val) & \defeq & 
    \begin{cases}
        \op[\addr \mapsto (\val, \{\etR\})] & \op(\addr) = (\val, \emptyset) \\
        \op &  \text{otherwise} \\
    \end{cases} \\
%
%
	\op \addO (\etW, \addr, \val) & \defeq & 
    \begin{cases}
        \op[\addr \mapsto (\val, S \cup \{\etW\})] & \op(\addr) = (-, S) \\
        \op &  \text{otherwise} \\
    \end{cases} \\
%      
    \op \addO \emptyop & \defeq & \op \\
\end{rclarray}
\]
%
For technical reason, if the right hand side is a special token \( \emptyop \), which represents no operation, the operations remains the same.
