...
To simulate conditional branching and loop, we have primary commands assume (\( \passume{\expr}\)) and assignment (\( \passign{\thvar}{\expr} \)), where \( \expr \) denotes arithmetic expressions.
...
To be used later, if the right hand side is special token \( \emptyop \), which represents no operation, the token has no side effect on the valid operations.
\[
\begin{rclarray}
...
We define a labelled transition system for primary transaction commands where the labels are commands.
The \( \funcn{op} \) extracts the read or write operation from primary transition commands, otherwise returns \( \emptyop \).
