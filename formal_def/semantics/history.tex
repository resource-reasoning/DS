...
\sx{
    ...
    \[
        \for{m,m'} m \in \como \land m' \in \como \implies m \compose{} m' \in \como
    \]
...
%\[
    %\infer[\rl{PViewLShift}]{
        %\como \vdash ( \stk, \hh, \vi ) , \cmd \ \toT{\como} \  ( \stk, \hh, \cu' ) , \cmd
    %}{
        %\vi \orderVI \vi'
    %}
%\]
...
Given the set of consistent models \( \ConsisModels \) (\defref{def:consistency-models}) and history heaps \(\HisHeaps\) (\defref{def:his_heap}) , the \emph{thread pool semantics}, 
\[
	(.) \toG{(.)} (.): 
