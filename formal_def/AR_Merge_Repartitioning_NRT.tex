...
%
\sx{
    \(\ls_5\) is unbound, I think it means for-all quantification?
    
    If the orthogonal(hopefully spell correct), is the same, as what define at the top, why there is transfer from \( \ca_3 \) to \( \ca_5 \) and similarly for \( \ca_4 \) to \( \ca_6 \)?
    I think it is because we can change the ghost state but cannot move resource?}
\azalea{
	Yes, \(\ls_5\) was left there from the previous definition. I have removed it. 
	
	And yes, the ghost resource (capabilities) may be updated but may not be transferred in or out.
}
\sx{It will be a singleton set?
I also simplify the 5-th line to 6-th line, which use the definition \ref{def:merge-finger-heap}.
I might cut this definition into several one, as it looks a bit scary. :)}
...
%
%
\azalea{The following definition has not changed from the previous section.}
...
I define the chain of interference in definition \ref{def:chain-intf}, which allows to specify part of a transaction and chain them up.
