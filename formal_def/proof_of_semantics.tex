...
\end{lem}
\begin{proof}
    From the \(\func{commit}\), the semantics only extend a history.
\end{proof}

\begin{lem}
    For any time, it has at most one write event, and if it has then it will be the only event in the time. This is \ \( \forall \tshp, \loc, \ts \ldotp ( \dontcare, \wop, \dontcare ) \in \tshp(\loc)(\ts) \implies \tshp(\loc)(\ts) =  \Set{( \dontcare, \wop, \dontcare )} \)
\end{lem}
\begin{proof}
