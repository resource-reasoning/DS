...
\begin{lem}[Monotonic time]
    \label{lem:mono-time}
    Given a trace the local time monotonically increase inside a thread, at fork point and at the join point.
\end{lem}
\begin{proof}
\end{proof}
...
\begin{proof}
    ...
    For \( \tsid \) and \( \tsid' \) where \( \lcmt{\thid,\tsid} < \lcmt{\thid,\tsid'} \), it means that transaction \( \tsid \) takes effect to the time-stamp heap before \( \tsid' \) takes effect by the same thread \( \thid \).
    Then by Lemma \ref{lem:mono-time}, the commit time of \( \tsid \) is smaller than the start time of \( \tsid' \), so that \( (\tsid, \tsid') \in \vis \).

    For \( \tsid \) and \( \tsid' \) where \( \lcmt{\thid,\tsid} < \lfork{\thid,\thid', \dontcare} < \lcmt{\thid',\tsid'} \), by Lemma \ref{lem:mono-time}, all the transactions by thread \( \thid \) before the fork point, which have taken effect to the time-stamp heap, must have smaller start and end times than all the transactions by the forked thread \( \thid' \).
    Therefore, \( (\tsid, \tsid') \in \vis \).

    Similarly, for \( \tsid \) and \( \tsid' \) where \( \lcmt{\thid,\tsid} < \ljoin{\thid',\thid, \dontcare} < \lcmt{\thid',\tsid'} \), by Lemma \ref{lem:mono-time}, all the transactions by thread \( \thid \) must have smaller start and end times than all the transactions by thread \( \thid' \) after the join point.
    Therefore, \( (\tsid, \tsid') \in \vis \).
