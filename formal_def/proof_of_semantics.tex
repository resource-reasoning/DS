...
\begin{lem}[No overwrite]
    ...
    \[ 
        \forall \stk, \stk', \tshp, \tshp', \ts, \ts', \progext, {\progext}' ,\lb,\loc, \ts \ldotp ( \stk, \tshp, \ts ), \progext \toT{\lb} ( \stk', \tshp', \ts' ), {\progext}' \implies \tshp(\loc)(\ts)\undef \lor \tshp(\loc)(\ts) = \tshp'(\loc)(\ts)
    \]
\end{lem}
\begin{proof}
...
\begin{lem}[Start before end]
    ...
    Formally,
    \[
        \forall \stk, \stk', \tshp, \tshp', \ts, \ts', \progext, {\progext}' ,\lb \ldotp \pred{rw}{\tshp} \land ( \stk, \tshp, \ts ), \progext \toT{\lb} ( \stk', \tshp', \ts' ), {\progext}' \implies \pred{rw}{\tshp'}
    \]
    Where,
    \[
...
\begin{lem}[Pair of start and end]
    ...
    \[
            \forall \stk, \stk', \tshp, \tshp', \ts, \ts', \progext, {\progext}' ,\lb \ldotp \pred{pair}{\tshp} \land ( \stk, \tshp, \ts ), \progext \toT{\lb} ( \stk', \tshp', \ts' ), {\progext}' \implies \pred{pair}{\tshp'}
    \]
    where,
    \[
...
\begin{proof}
    ...
    For base cases \rl{PChoiseLeft}, \rl{PChoiseRight}, \rl{PLoop}, \rl{PSeqSkip}, \rl{PPar} and \rl{PWait}, it is trivial because those rules do not change the state of time-stamp heap \( \tshp \).
    For base case \rl{Commit}, the new state \( \tshp' = \func{commitTrans}{\tshp,\hp_{s},\hp_{e},\ws,\rs, \tsid, \ts_{s},\ts_{e}} \) for some \( \hp_{s},\hp_{e},\ws,\rs, \tsid, \ts_{s},\ts_{e} \). 
    Because the function \funcn{commit} always extends a location \( \loc \) with a pair of operations, so \( \pred{rw}{\tshp'}\) holds.
    For the inductive case \rl{PSeq}, prove directly by the inductive hypothesis.
\end{proof}
...
\begin{lem}[Start/end at the same time]
    ...
    \[
        \forall \stk, \stk', \tshp, \tshp', \ts, \ts', \progext, {\progext}' ,\lb \ldotp \pred{atom}{\tshp} \land ( \stk, \tshp, \ts ), \progext \toT{\lb} ( \stk', \tshp', \ts' ), {\progext}' \implies \pred{atom}{\tshp'}
    \]
    Where,
    \[
...
\[
    \begin{rclarray}
        ...
        \func{traces}{\tshp,\tdpl,\nat} & \defeq & \Set{(\setlabels,<,\tshp',\tdpl') \middle| (\tshp'', \tdpl'') \toG{\lbID} (\tshp', \tdpl' ) \land (\setlabels,<,\tshp'',\tdpl'') \in \func{trace}{\tshp,\tdpl,\setlabels,<,\nat-1}} \\
                                                    & & \uplus \Set{(\setlabels \uplus \Set{\lb} ,< \uplus \Set{(\lb',\lb) \middle| \lb' \in \setlabels}, \tshp',\tdpl') \middle| 
        \begin{array}{@{}l@{}}
            (\tshp'', \tdpl'') \toG{\lb} (\tshp', \tdpl' ) \land \lb \neq \lbID \land {} \\
...
\[ 
    \begin{rclarray}
        ...
        \po & \equiv & \Set{(\tsid, \tsid') \middle| \lbC{\thid,\tsid} < \lbC{\thid,\tsid'} } \uplus \Set{(\tsid, \tsid') \middle| \lbC{\thid,\tsid} < \lbF{\thid,\thid', \dontcare} < \lbC{\thid',\tsid'} } \\
            & & \uplus \Set{(\tsid, \tsid') \middle| \lbC{\thid,\tsid} < \lbJ{\thid',\thid, \dontcare} < \lbC{\thid',\tsid'} }
    \end{rclarray}
\]
\end{defn}
...
\begin{lem}[Monotonic time in a thread]
    ...
    This is  
    \[ 
        \forall \stk, \stk', \tshp, \tshp', \ts, \ts', \progext, {\progext}' ,\lb,\loc, \ts \ldotp ( \stk, \tshp, \ts ), \progext \toT{\lb} ( \stk', \tshp', \ts' ), {\progext}' \implies \ts < \ts'
    \]
\end{lem}
\begin{proof}
...
\begin{proof}
    ...
    For \( \tsid \) and \( \tsid' \) where \( \lbC{\thid,\tsid} < \lbF{\thid,\thid', \dontcare} < \lbC{\thid',\tsid'} \), first note that in the \rl{PFork} the parent thread starts at time \( \ts \) and ends at \( \ts' \), and the child thread initialises with the time \( \ts' \), where  in fact \( \ts = \ts' \)
    By \( \lbC{\thid,\tsid} < \lbF{\thid,\thid', \dontcare}  \), the transaction \( \thid \) is reduced before the fork, so by Lemma \ref{lem:mono-time-thread}, the end time of \( \thid \) is smaller than \( \ts \).
    By \( \lbF{\thid,\thid', \dontcare} < \lbC{\thid',\tsid'} \), the transaction \( \thid' \) is reduced after the fork and also by Lemma \ref{lem:mono-time-thread}, the start time of \( \thid \) is greater or equal to \( \ts' \).
    Therefore, \( (\tsid, \tsid') \in \vis \).

    Similarly, for \( \tsid \) and \( \tsid' \) where \( \lbC{\thid,\tsid} < \lbJ{\thid',\thid, \dontcare} < \lbC{\thid',\tsid'} \), by Lemma \ref{lem:mono-time-fork-join}, all the transactions by thread \( \thid \) must have smaller start and end times than all the transactions by thread \( \thid' \) after the join point.
    Therefore, \( (\tsid, \tsid') \in \vis \).
\end{proof}
...
\sx{Define a function from a graph to a set of possible \( \hp \) }
