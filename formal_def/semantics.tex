...
We model the global database state as a \emph{timestamp heap}. A timestamp heap is a partial function from locations to \emph{histories}.
...
Whilst the state of the database is given by a globally-accessed (by all threads) timestamp heap, we use \emph{fingerprint heaps} to model a \emph{local snapshot} of the global timestamp heaps made available to transactions during their execution. In order to successfully commit a transaction and avoid conflicts, we must track the \emph{fingerprint} of a transaction, namely those locations read from or written to by the transaction. As such, we model fingerprint heaps as finite partial maps from locations to pairs comprising a value and a fingerprint. The fingerprint of a location may be 1) $\emptyset$ denoting that the location has not been touched; 2) $\Set{\rfp}$ denoting that the location has been read; 3) $\Set{\wfp}$ denoting that the location has been mutated (written to); and 4) $\Set{\rfp, \wfp}$ denoting that the location has been initially read and subsequently mutated.   
...
\noindent Given the sets of program values $\Val$ (\defin\ref{def:prgram_values}) and locations $\Loc$ (\defin\ref{def:timestamp_heaps}), the set of \emph{fingerprint heaps} is: $\fph \in \FPHeap \eqdef \Loc \parfinfun (\Val \times \Fingerprint)$.
\end{definition}
%
Given a fingerprint heap $\fph$ and a location $\loc$, we write $\fphVal(\loc)$ and $\fphFP(\loc)$ for the first and second projections of $\fph(\loc)$, respectively.\\
...
The operational semantics of transactions is standard with the exception of the \rl{TRead} and \rl{TWrite} rules where the fingerprint of the location read from (resp.~written to) is extended with $\rfp$ (resp.~$\wfp$).

\azalea{Shale, as with fingerprints later in the logic, shouldn't we only record the read if it has not already been written by us? See the \rl{TRead'} rule vs. \rl{TRead}.}

\begin{definition}[Transaction semantics]
Given the sets of stacks (\defin\ref{def:stacks}), fingerprint heaps (\defin\ref{def:fingerprint_heaps}) and transactions (\defin\ref{def:language}), the \emph{operational semantics of transactions}, $\localtransfer : ((\Stack \times \FPHeap) \times \Transactions) \times ((\Stack \times \FPHeap) \times \Transactions)$, is given in \fig\ref{fig:transaction_semantics}.
\end{definition}
...
\hrule\vspace{5pt}
%\[
%	\localtransfer : ((\Stack \times \FPHeap) \times \Transactions) \times ((\Stack \times \FPHeap) \times \Transactions)
%\]	
%
%
\[	
...
\[
    \infer[\rl{TRead}]{
        ( \stk, \fph) , \pderef{\var}{\expr} \ \localtransfer \  (\stk \remapsto{\var}{\val}, \fph \remapsto{\loc}{(\val, \fp \cup \{\rfp\})}), \pskip
    }{
        \eval{\expr}_{\stk} = \loc && 
        \fph(\loc) {=} (\val, \fp) 
    }
...
\begin{array}{@{}r @{\hspace{2pt}} c @{\hspace{2pt}} l@{}}
	\snapshot{.}{.} &:& \Timestampheap \times \Timestamp \rightharpoonup \FPHeap \\
%	
	\snapshot{\tshp}{\ts} 
	& \defeq & 
	\lambda \loc \ldotp
...
%
%  
	\commit{.}{.}{.}{.}{.}{.} & : & \Timestampheap \times \FPHeap \times \FPHeap \times \TransID \times \Timestamp \times \Timestamp \rightarrow \Timestampheap \\
%	
	...
	\begin{cases}
		...
		& \text{if \;} \wfp \in \fphFP[\fph_e](\loc) \\
		\tshp(\loc)\remapsto{\ts_s}{(\fphVal[\hp_s](\loc),\rop,\tsid)}\remapsto{\ts_e}{(\fphVal[\hp_e](\loc),\eop,\tsid)} 
		& \text{if \;} \rfp \in \fphFP[\fph_e](\loc) \\
%       \tshp(\loc)\remapsto{\ts_{s}}{(\hp_{s}(\loc),\rop,\tsid)}\remapsto{\ts_{e}}{(\hp_{e}(\loc),\eop,\tsid)} & \loc \in \rs \setminus \ws \\
		\tshp(\loc)\remapsto{\ts_s}{(\fphVal[\hp_s](\loc),\rop,\tsid)}\remapsto{\ts_e}{(\fphVal[\hp_e](\loc),\wop,\tsid)} & \text{if \;} \{\rfp, \wfp\} \subseteq \fphFP[\fph_e](\loc) \\
