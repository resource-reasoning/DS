...
\begin{lem}
    ...
    This is \( \forall \tshp \ldotp \exists \tshp' = \func{extend} \circ \func{stetch}(\tshp) \ldotp \forall \loc, \ts \ldotp (\tshp'(\loc)(3 * \ts + 1)\isdef \implies \tshp'(\loc)(3 * \ts + 1) = (\dontacre, \dontcare, \eop) ) \land (\tshp'(\loc)(3 * \ts - 1)\isdef \implies \tshp'(\loc)(3 * \ts - 1) = (\dontacre, \dontcare, \sop) ) \).
\end{lem}
\begin{proof}
    trivial.
\end{proof}
...
\begin{lem}
    Given a \( \tshp \), the corresponding \((\settrans, \tvis, \tar)\) can be extended so that it satisfies snapshot isolation.
\end{lem}
\begin{proof}
    First, we extend the relations \( \tar \) to a total order.
    \begin{itemize}
        \item Two write happens in the same time.
        \item Two ends happens in the same time.
    \end{itemize}
    !!Inductively pick a order.!!
\end{proof}
