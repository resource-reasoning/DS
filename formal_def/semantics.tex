...
\end{itemize} The set of abstract executions $\aexec  = (\tx, \po, \vis, \ar) \in \Aexecs$, where \( \po \subseteq \dom(\tx) \times \dom(\tx)\), is defined by erasing the runtime threads from the runtime abstract executions \( \Aexecrun \),
\[
\begin{rclarray}
...
\[
    ...
    \end{rclarray}
\]
For brevity, the \( \aexecrun\prjT \), \( \aexecrun\prjI \), \( \aexecrun\prjP \), \( \aexecrun\prjV \) and \( \aexecrun\prjA \) denote the corresponding elements in the tuple and similarly \( \aexec\projection{.} \).
Last, given two runtime abstract executions \( \aexecrun_{1} \) and \( \aexecrun_{2} \), the order between them is defined as point-wise set inclusion,
\[
\begin{rclarray}
...
Given a consistency model, we can define the set of abstract executions that satisfy the model (\defref{def:valid-aexec}) \cite{cerone:snapshot,cerone_et_al:lipics:2017:7794,cerone_et_al:LIPIcs:2015:5375}.
The \lemref{lem:consistency-monotonicity} is for sanity check.

\begin{defn}[Valid abstract executions]
\label{def:valid-aexec}
Given a consistency model \( (\respo, \conguar) = \como \in \Como \) (\defref{def:consistency-models}), the set of all the valid abstract executions under the model is defined as the follows,
\[
    \begin{rclarray}
    ...
    \end{rclarray}
\]
where \( \aexec\prjV(\txid) \) returns all the predecessors of \( \txid \) in the visibility relation.
...
Second, assume a transaction \( \txid \) and any of its read event with address \( \addr \) and \( \val \) such that \( (\etR, \addr, \val) \in \tx(\txid) \implies \val \in \respo_{2}(\aexec, \aexec\prjV(\txid), \addr) \).
...
For any runtime abstract abstract execution \( \aexecrun \) that satisfies a consistency model \( \como \), if the semantics take one step under consistency model \( \como \) to another abstract execution \( \aexecrun' \), the new execution should satisfy the consistency model.
This is,
 \[
 \begin{array}{@{}l@{}}
...
By the \rl{PCommit} rule, it has \( \iprog = \ptrans{\trans} \), \( \iprog' = \pskip \) and \( \lb = \lbC{\txid} \), for some transition code \( \trans \) and identifier \( \txid \).
...
\end{align}
First, we consider \equref{equ:res_policy} and we only need to consider the new transaction \( \txid \) as others are proved directly from the hypothesis.
Assume an initial heap \( \h \), a set of events \( \evset \) associated with \( \txid \), a set of transactions \( \txidset \) observed by the new transaction \( \txid \) that satisfy the follows,
\[
\begin{array}{@{}l@{}}
    \exsts{\thstk, \txstk, \h'} \nonumber \\
...
\end{array}
\]
Given the initial heap \( \h \), the following holds,
\[
...
If the new read event is included in the events set after flushing read (\defref{def:transaction-event}), \ie \( (\etR, \addr, \val) \in ( \evset \flushR (\etR, \addr, \val) ) \), this means their is no other read and write to the same address before, so that the value \( \val  \) associate with the address \( \addr \) remains unchanged.
By the \( \obsstateName \) function (\figref{fig:thread_semantics}), where \( \h(\addr) = \val \iff \respo(\aexec, \txidset, \addr) \), the following holds,
\[
    (\etR, \addr, \val) \in \evset \implies \respo(\aexec, \txidset, \addr)
\]
Since the \( \newaexecName \) function only extends abstract execution, \ie \( \aexec \ordAEX \aexec' \), so that,
\[
    (\etR, \addr, \val) \in \evset \implies \respo(\aexec', \txidset, \addr)
\]
By the \( \obsstateName \) function, we also know \( \evset = \aexec'\prjT(\txid)\) and \( \txidset = \aexec'\prjV(\txid)\) therefore we have a proof for \equref{equ:res_policy}.

Second, we need to prove \equref{equ:con_guarantee}, that the visibility relation of the new abstract execution \( \aexec'\prjV \) contains the minimum relation by consistency guarantee.
Similarly, we only need considering those visibility relation related to the new transaction \( \txid \).
Note that for any transaction \( \txid'' \in \dom(\aexec'\prjT) \), we have \( (\tsid, \tsid'') \notin \conguar(\aexec') \).
Because by the \( \newaexecName \) function, so \( (\tsid'', \tsid) \in \aexec'\prjA \), and then by the definition of consistency guarantee (\defref{def:consistency-models}), the consistency guarantee cannot violate arbitration order, this is, \( \conguar(\aexec') \subseteq \aexec'\prjA\).
So assume a transaction \( \tsid' \in \dom(\aexec'\prjT) \) such that \( (\tsid', \tsid) \in \conguar(\aexec') \).
By the \( \predn{consis}\) predicate, we have \( (\tsid', \tsid) \in \conguar(\aexec') \implies (\tsid', \tsid) \in \aexec'\prjV\) so that we have proof for \equref{equ:con_guarantee}.
...
For sanity check and also proving the completeness of this semantics, we first prove the semantics is monotonic (\lemref{lem:semantics-monotonicity}), which means that for any reduction can happen in the stronger consistency model, it can also happen in the weaker one.

\begin{lem}[Semantics monotonicity]
\label{lem:semantics-monotonicity}
For any runtime abstract execution \( \aexecrun' \) can be produced by the semantics under stronger consistency model \( \como_{2}\), it can also be produced by a weaker one \( \como_{1} \).
\[
\begin{array}{@{}l@{}}
...
Given an initial runtime abstract execution \( \aexecrun \), a set of observable transactions \( \txidset \) a new transaction identifier \( \txid \) and a thread identifier \( \thid \), by the \rl{PCommit} rule (\figref{fig:thread_pool_semantics}), it is sufficient to prove, first, all the observable states under the stronger consistency model, they can be observed under the weaker one,
\begin{equation}
    \label{equ:obs-state-included}
    \obsstate{\aexecrun, \txidset, \respo_{2}} \subseteq  \obsstate{\aexecrun, \txidset, \respo_{1}} 
\end{equation}
and second If the new runtime abstract execution, \ie \( \aexecrun' \), exists under the stronger concurrency model, it should also exist under weaker one,
\begin{equation}
    \label{equ:consis-both-exist}
    \consis{\aexecrun', \txid, \thid, \conguar_{2}} \implies \consis{\aexecrun', \txid, \thid, \conguar_{1}}
\end{equation}
To prove \equref{equ:obs-state-included}, assume an observable heap \( \h \) under stronger consistency model \( \como_{2}\), this is, \( \h \in \obsstate{\aexecrun, \txidset, \respo_{2}} \).
Assume an address \( \addr\) and the corresponding value \( \val \) such that \(  \h(\addr) = \val \).
By the definitions of \( \obsstateName \) function  (\figref{fig:thread_semantics}) and resolution policy (\defref{def:consistency-models}), it is known that \( \val \in \respo_{2}(\eraseAEX{\aexecrun}, \txidset, \addr)  \).
Since \( \respo_{2}(\eraseAEX{\aexecrun}, \txidset, \addr) \subseteq \respo_{1}(\eraseAEX{\aexecrun}, \txidset, \addr) \) (\defref{def:consistency-models}), therefore \(  \val \in \respo_{1}(\eraseAEX{\aexecrun}, \txidset, \addr) \), so that \( \h \in \obsstate{\aexecrun, \txidset, \respo_{1}} \).
For \equref{equ:consis-both-exist}, assume the \( \consis{\aexecrun, \txid, \thid, \conguar_{2}} \) predicate holds.
...
thus the \equref{equ:consis-both-exist} holds.
Combining \equref{equ:obs-state-included} and \equref{equ:consis-both-exist}, we have the proof for \rl{PCommit}.
...
It means that from \equref{equ:not-in-vis} or \equref{equ:not-observable}, 
\[
...
The completeness means that if an abstract execution \( \aexecrun \) satisfies a consistency model, it is possible to produce such execution through the semantics under the corresponding consistency model.
...
The \thmref{thm:semantics-completeness} says given some initial configuration, if one is ``lucky'' that it ends up with a runtime abstract execution \( \aexecrun'\) that satisfies a consistency model \( \como \) under the anarchic semantics, it is possible to get the same result under the semantics specifically for the consistency model \( \como \).
...
\end{thm}
\begin{proof}
For a final runtime abstract execution that \( \eraseAEX{\aexecrun'} \in \evalCOM{\como} \), by the \lemref{lem:preserve-of-consistency}, it is known that the initial configuration also satisfies the consistency model, this is, \( \eraseAEX{\aexecrun} \in \evalCOM{\como} \).
