...
We define the operational semantics of transactions \(\trans \in \Transactions\) with respect to a configuration of the form $(\txstk, \h, \evset)$ comprising a transaction stack, a (local) heap and a set of operations that might affect other transactions.
...
A partial binary operation \( \addO \) that adds a new operation to a set,
\[
\begin{rclarray}
    \evset \addO (\etR, \addr, \val) & \defeq & 
    \begin{cases}
        \evset \uplus \Set{(\etR, \addr, \val)} & (\stub, \addr, \stub) \notin \evset \\
        \evset &  \text{otherwise} \\
    \end{cases} \\
    \evset \addO (\etW, \addr, \val) & \defeq & ( \evset \setminus \Set{(\etW, \addr, \stub)} ) \uplus \Set{(\etW, \addr, \val)} \\
\end{rclarray}
\]
This operation \( \addO \) includes the new read event if there is no other read or write event to the same address.
While it deletes other write events from the set and includes the new one.
...
\[
    \infer[\rl{TRead}]{
        \thstk \vdash (\txstk, \h, \evset) , \pderef{\txvar}{\expr} \ \toL \  (\txstk\remapsto{\txvar}{\val}, \h, \evset \addO (\etR, \addr, \val)), \pskip
    }{
...
\[
    \infer[\rl{TMutate}]{%
        \thstk \vdash (\txstk, \h, \evset), \pmutate{\expr_{1}}{\expr_{2}} \ \toL \  (\txstk, \h \remapsto{\addr}{\val}, \evset \addO (\etW, \addr, \val)) , \pskip
    }{%
