...
\begin{defn}[Program values]\label{def:prgram_values}
...
%
%
\begin{defn}[Stacks]\label{def:stacks}
...
\begin{defn}[Programming language]\label{def:language}
...
\begin{defn}[Timestamp heaps]\label{def:timestamp_heaps}
...
\begin{defn}[Fingerprint heaps]\label{def:fingerprint_heaps}
...
%\[
%    \begin{rclarray}
%        ( \dontcare, \dontcare ) \ \toL \  ( \dontcare, \dontcare ) & \defeq &  ( \Localstate \times \cmd ) \times ( \Localstate \times \cmd ) \\
%    \end{rclarray}
%\]
\hrule\vspace{5pt}
%\[
%	\toL : ((\Stack \times \FPHeaps) \times \Transactions) \times ((\Stack \times \FPHeaps) \times \Transactions)
%\]	
%
%
\[	
...
\[
    \infer[\rl{TWrite}]{%
        ( \stk, \fph), \pmutate{\expr_{1}}{\expr_{2}} \ \toL \  ( \stk, \hp \remapsto{\loc}{(\val, \addWFP{\fp})} , \pskip
    }{%
...
\[
    \infer[\rl{TRead}]{
        ( \stk, \fph) , \pderef{\var}{\expr} \ \toL \  (\stk \remapsto{\var}{\val}, \fph \remapsto{\loc}{(\val, \addRFP{\fp})}, \pskip
    }{
        \eval{\expr}_{\stk} = \loc && 
        \fph(\loc) {=} (\val, \fp) 
    }
...
\[
    \infer[\rl{TChoiseL}]{%
        (\stk, \fph) , \cmd_{1} + \cmd_{2} \ \toL \  (\stk, \fph) , \cmd_{1}
    }{}
\]

\[
    \infer[\rl{TChoiseR}]{%
        (\stk, \fph) , \cmd_{1} + \cmd_{2} \ \toL \  (\stk, \fph), \cmd_{2}
    }{%
    }
\]
...
\[
    \infer[\rl{TLoop}]{%
        (\stk, \fph),  \cmd^{*} \ \toL \  (\stk, \fph), \pskip + (\cmd ; \cmd^{*})
    }{%
    }
\]
...
\[
    \infer[\rl{TSeqSkip}]{%
        (\stk, \fph), \pskip ; \cmd_{2} \ \toL \  (\stk, \fph), \cmd_{2}
    }{%
    }
\]
...
\[
    \infer[\rl{TSeq}]{%
        (\stk, \fph), \cmd_{1} ; \cmd_{2} \ \toL \  (\stk', \fph'), \cmd_{1}' ; \cmd_{2}
    }{%
        (\stk, \fph), \cmd_{1} \ \toL \  (\stk', \fph'), \cmd_{1}'
    }
\]
\hrule\vspace{5pt}
...
\begin{defn}[Thread semantics]\label{def:thread_semantics}
...
%
\[
    \iprog \in \IntermediatePrograms ::= \prog \mid \iprog; \pwait{\thid}
\]
%
...
\[
    \infer[\rl{Commit}]{%
        ( \stk, \tshp, \ts ) , \ptrans{\cmd} \ \toT{\lbC{\tsid}} \ ( \stk', \tshp', \ts_{e} ) , \pskip
    }{%
        \begin{array}{c}
            \ts \leq \ts_s < \ts_e 
            \qquad \fph_s = \snapshot{\tshp}{\ts_s}
            \qquad ( \stk, \fph_s) , \cmd \toL^{*} ( \stk', \fph_e) , \pskip \\
...
\[
    \infer[\rl{PChoiseL}]{%
        ( \stk, \tshp, \ts ) , \prog_{1} + \prog_{2} \ \toT{\lbID} \  ( \stk, \tshp, \ts ) , \prog_{1}
    }{%
    }
\]
%
...
\[
    \infer[\rl{PChoiseR}]{%
        ( \stk, \tshp, \ts ) , \prog_{1} + \prog_{2} \ \toT{\lbID} \  ( \stk, \tshp, \ts ) , \prog_{2}
    }{%
    }
\]
%
...
\[
    \infer[\rl{PLoop}]{%
        ( \stk, \tshp, \ts ) , \prog^{*} \ \toT{\lbID} \  ( \stk, \tshp, \ts ) , \pskip + (\prog ; \prog^{*})
    }{%
    }
\]
%
...
\[
    \infer[\rl{PSeqSkip}]{%
        ( \stk, \tshp, \ts ) , \pskip ; \iprog \ \toT{\lbID} \  ( \stk, \tshp, \ts ) , \iprog
    }{%
    }
\]
%
...
\[
    \infer[\rl{PSeq}]{%
        ( \stk, \tshp, \ts ) , \iprog_{1} ; \iprog_{2} \ \toT{\lb} \ ( \stk', \tshp', \ts' ) , {\iprog_{1}}' ; \iprog_{2}
    }{%
...
\[
    \infer[\rl{PPar}]{%
        ( \stk, \tshp, \ts ) , \prog_{1} \ppar \prog_{2} \ \toT{\lbF{\thid, \prog_{2}}} \  ( \stk, \tshp, \ts ) , \prog_{1} ; \pwait{\thid}
    }{%
    }
\]
%
...
\begin{defn}[Thread pools]\label{def:thread_pools}
...
%
%
\begin{defn}[Thread pool semantics]\label{def:thread_pool_semantics}
...
%
\[
	\toG{} : (\Timestampheap \times \Threadpool) \times \Translabel \times (\Timestampheap \times \Threadpool) 
\]
%
...
\[
    \infer[\rl{PSingle}]{%
        ( \tshp, \tdpl \uplus \Set{ \thid \mapsto (\stk, \ts, \iprog) } ) \ \toG{\lb} \  ( \tshp', \tdpl \uplus \Set{ \thid \mapsto (\stk', \ts', {\iprog}') } ) 
    }{%
        ( \stk, \tshp, \ts ) , \iprog \ \toT{\lb} \  ( \stk', \tshp', \ts' ) , {\iprog}' 
        \quad \lb \in \Set{ \lbID, \lbC{\dontcare} }
    }
\]

\[
    \infer[\rl{PFork}]{%
        ( \tshp, \tdpl \uplus \Set{ \thid \mapsto (\stk, \ts, \iprog) } ) \ \toG{\lbF{\thid', \prog''}} \  ( \tshp', \tdpl \uplus \Set{ \thid \mapsto (\stk', \ts', {\iprog}'), \thid' \mapsto (\lambda \var \ldotp 0, \ts', \prog'') } )
    }{%
...
\[
    \infer[\rl{PJoin}]{%
        ( \tshp, \tdpl \uplus \Set{ \thid \mapsto (\stk, \ts, \iprog), \thid' \mapsto (\stk', \ts'', \pskip) } )  \ \toG{\lbJ{\thid',\ts''}} \ ( \tshp', \tdpl \uplus \Set{ \thid \mapsto (\stk', \ts', {\iprog}')} )
    }{%
