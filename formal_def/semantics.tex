...
\[
    \infer[\rl{Commit}]{%
        ( \stk, \aexecrun ), \ptrans{\trans} \ \toT{\lbC{\tsid}} \ ( \stk\rmto{\ret}{\txstk(\lret)}, \aexecrun' ) , \pskip
    }{%
        \begin{array}{c}
            \ts_s < \ts_e 
...
\[
    \infer[\rl{PAssign}]{%
        ( \thstk, \aexecrun ) , \passign{\thvar}{\expr} \ \toT{\lbID} \  ( \stk\rmto{\thvar}{\evalE{\expr}}, \aexecrun  ) , \pskip
    }{}
\]

\[
    \infer[\rl{PIncTime}]{%
        ( \thstk, \aexecrun ) , \passume{\expr} \ \toT{\lbID} \  ( \thstk, \aexecrun ) , \pskip
    }{%
        \evalE{\expr} = 0
    }
\]
...
\[
    \infer[\rl{PChoise}]{%
        ( \thstk, \aexecrun ) , \prog_{1} \pchoice \prog_{2} \ \toT{\lbID} \  ( \thstk, \aexecrun ) , \prog'
    }{
...
\[
    \infer[\rl{PLoop}]{%
        ( \thstk, \aexecrun ) , \prog\prepeat \ \toT{\lbID} \  ( \thstk, \aexecrun ) , \pskip \pchoice (\prog \pseq \prog\prepeat)
    }{}
\]

\[
    \infer[\rl{PSeqSkip}]{%
        ( \thstk, \aexecrun ) , \pskip \pseq \iprog \ \toT{\lbID} \  ( \thstk, \aexecrun ) , \iprog
    }{}
\]

\[
    \infer[\rl{PSeq}]{%
        ( \thstk, \aexecrun ) , \iprog_{1} \pseq \iprog_{2} \ \toT{\lb} \ ( \thstk', \aexecrun' ) , {\iprog_{1}}' \pseq \iprog_{2}
    }{%
        ( \thstk, \aexecrun ) , \iprog_{1} \ \toT{\lb} \  ( \thstk', \aexecrun' ) , {\iprog_{1}}' 
    }
\]

\[
    \infer[\rl{PPar}]{%
        ( \thstk, \aexecrun ) , \prog_{1} \ppar \prog_{2} \ \toT{\lbF{\thid, \prog_{2}}} \  ( \thstk, \aexecrun ) , \prog_{1} \pseq \pwait{\thid}
    }{}
\]

\[
    \infer[\rl{PWait}]{%
        ( \thstk, \aexecrun ) , \pwait{\thid} \ \toT{\lbJ{\thid, \thstk'}} \  (  \thstk_{1} \uplus \thstk_{2} \uplus \thstk_{f}, \aexecrun ) , \pskip 
    }{
...
Given the sets of stacks $\Stacks$ (\defref{def:stacks}), timestamps $\Timestamp$ (\defref{def:timestamp_heaps}) and programs $\Programs$ (\defref{def:language}), the set of \emph{thread pools} is: $\thpl \in \TPool \eqdef \ThreadID \parfinfun \Stacks \times \Timestamp \times \Programs$.
\end{defn}
