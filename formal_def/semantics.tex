...
\end{array}
\]
We can rewrite \equref{equ:res_policy} to the following form,
\[
...
In these base cases, the runtime abstract execution remains the same, \ie \( \aexecrun = \aexecrun' \), so it trivially holds because of the hypothesis.
...
\begin{proof}
We prove it by induction on the derivations.
The only interesting case is the  \rl{PCommit} rule, especially given \( \aexecrun, \txidset, \thid \), we need to prove the follows,
\begin{itemize}
...
Assume a transaction \( \txid' \) such that \( (\txid', \txid) \in \conguar_{2}(\eraseAEX{\aexec}) \) therefore by the definition, we have \( (\txid', \txid) \in \aexecrun\prjV \).
Since \( \conguar_{1}(\eraseAEX{\aexec}) \subseteq \conguar_{2}(\eraseAEX{\aexec})\), so that \( (\txid', \txid) \in \conguar_{1}(\eraseAEX{\aexec}) \implies (\txid', \txid) \in \aexecrun\prjV \) also holds.
