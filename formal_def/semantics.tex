...
\[
    \begin{rclarray}
        ...
        \evalE{\expr_{1} * \expr_{2}} & \defeq & \evalE{\expr_{1}} * \eval{\expr_{2}} 
    \end{rclarray}
\]
%
\end{defn}
...
\[
    \infer[\rl{TWrite}]{%
        ( \stk, \fph), \pmutate{\expr_{1}}{\expr_{2}} \ \toL \  ( \stk, \hp \remapsto{\addr}{(\val, \addFPW{\fp})}) , \pskip
    }{%
        \eval{\expr_{1}}_{\stk} = \addr && 
...
%	
	...
	\lambda \addr \ldotp
	\begin{cases}
		\tshp(\addr)\remapsto{\ts_s}{(\fphVal[\hp_s](\addr),\etS,\tsid)}\remapsto{\ts_e}{(\fphVal[\hp_e](\addr),\etW,\tsid)} 
		& \text{if \;} \fphFP[\fph_e](\addr) {=} \{\fpW\} \\
		\tshp(\addr)\remapsto{\ts_s}{(\fphVal[\hp_s](\addr),\etR,\tsid)}\remapsto{\ts_e}{(\fphVal[\hp_e](\addr),\etE,\tsid)} 
		& \text{if \;} \fphFP[\fph_e](\addr) {=} \{\fpR\} \\
		\tshp(\addr)\remapsto{\ts_s}{(\fphVal[\hp_s](\addr),\etR,\tsid)}\remapsto{\ts_e}{(\fphVal[\hp_e](\addr),\etW,\tsid)} & \text{if \;} \fphFP[\fph_e](\addr) {=} \{\fpR, \fpW\} \\
...
%        
	...
	&& \quad \land\ \for{\ts_m} \ts_m {=} \min(\Set{\ts'' \ \middle| \ \ts'' > \ts_{e} \land \tshp(\addr)(\ts'')\isdef}) \Rightarrow  \tshp(\addr)(\ts_{min}) \neq (\stub, \etR, \stub) 
	\vspace{3pt}\\
%
%        
