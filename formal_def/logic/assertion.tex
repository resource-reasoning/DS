...
Assuming a global function, \( \funcn{init} : \RegionID \to \powerset{ \HisHeaps \times \powerset{\Views} } \) that returns initial history heaps and sets of views for regions, the invariant of a region, namely \( \func{inv}{\rid, \intf} \), is the labelled transition system with respect to \( \intf \), where the initial states \( \func{init}{\rid}\), the configuration is a pair of history heap \( \hh \in \hhset \) and a set of views \( \viset \in \visetset \) and labels are client-specified capabilities \( \kap \).
\[
\begin{rclarray}
    \func{inv}{\rid, \intf} & \defeq & (\hhset \times \visetset, \toLTS{}, \func{init}{\rid}, \kapset) \\
\end{rclarray}
\]
The transition system is the least fix point from the relation \( {} \toLTS{.} {} : (\HisHeaps \times \powerset{\Views}) \times \Kaps \times (\HisHeaps \times \powerset{\Views}) \) given the set of initial configurations \( \func{init}{\rid} \subseteq \HisHeaps \times \powerset{\Views} \).
The relation \( \toLTS{.} \) is defined as the follows,
\[
\begin{rclarray}
    (\hh, \viset \uplus \Set{\vi} ) \toLTS{\kap} (\hh',\viset \uplus \Set{\vi'}) & \defeq &
    \begin{array}[t]{@{}l}
        \exsts{\txid, \opset} 
        \txid \in \func{fresh}{\hh} 
        \land \opset \in \intf(\kap) \\
        \quad {} \land \hh' = \func{commit}{\hh, \vi, \txid, \opset} 
        \land \cu' = \func{update}{\hh', \vi, \opset} \\
    \end{array}
\end{rclarray}
\]
\end{defn}
...
For brevity, \( (\hh,\vi) \in \func{inv}{\rid, \intf} \), where \( \vi \) is a single view, denotes \( \exsts{\viset } \vi \in \viset \land (\hh,\viset) \in \func{inv}{\rid,\intf}\projection{1} \), and similarly \( (\hh,\vi) \toLTS{\kap} (\hh',\vi') \in \func{inv}{\rid, \intf} \) for the second projection.
...
\begin{rclarray}
    ...
    \func{clpsSS}{\Set{\rid \mapsto (\hh, \vi, \ca, \intf)} \uplus \gs } & \defeq & 
        \Setcon{ 
            ...
            \dom(\hh) = \dom(\cu) \land (\hh,\cu) \in \func{inv}{\rid, \intf} \\ \quad {} \land (\hh', \vi', \ca') \in \eraseS{\gs} }\\
\end{rclarray}
\] 
% 
