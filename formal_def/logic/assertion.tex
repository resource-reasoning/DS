...
\begin{array}{@{}l@{}}
    ...
    \intass: \perm{INC} : \vx \pt n \oassto \vx \pt n \mat \Set{(\etR, \vx, k), (\etW, \vx, k + 1)}\\
    \intass: \perm{READ} : \vx \pt n \oassto \vx \pt m \land m \leq n \mat \Set{(\etR, \vx, k)}\\
\end{array}
\]
...
It also say if one want to increase the counter, it will have local state the same as the global state, specified by the thing before the action.
For example in the case we show here, it will have \( \vx \pt 1\) as the initial local state if one increases the counter.
The second line says one can always read the value of \( \vx \) which will be an arbitrary \( k \).
If one only want to read, the initial local state will be any value smaller than the global value.
...
\sx{depend on the action model, the reachable predicate might be slightly different}
\[
\begin{rclarray}
