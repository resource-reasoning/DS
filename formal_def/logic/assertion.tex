...
\[
\begin{rclarray}
	\lpre, \lpost  & ::= & \False \mid \True \mid \lpre \land \lpost \mid \lpre \lor \lpost \mid \exsts{\lvar} \lpre \mid \Emp \mid \lexpr \pt \lexpr \mid \lpre \sep \lpost  \\
\end{rclarray}	 
\]
Given a logical environment $\lenv \in \LEnv$, the \emph{local interpretation function}, $\evalLS[(.,.)]{.}: \LAst \times \LEnv \parfun \Heaps$, is defined over the structure of local assertions as follows,
\sx{there is variable clash for event tags and logical expression, leave it for now}
\[
\begin{rclarray}
	\evalLS{\assfalse} & \eqdef & \emptyset   \\
	\evalLS{\asstrue} & \defeq & \Heaps   \\
	...
	\evalLS{\assemp} & \defeq & \Set{\unitH}  \\
	\evalLS{\lexpr_{1} \pt \lexpr_2 } & \defeq & \Set{ \evalLE{\lexpr_1} \pt \evalLE{\lexpr_2} } \\
	\evalLS{\lpre \sep \lpost} & \defeq & 
    \Setcon{ \h_1 \composeH \h_2 }{ \h_{1} \in \evalLS{\lpre} \land \h_{2} \in \evalLS{\lpost} } 
\end{rclarray}
\]
\end{definition}
...
\begin{rclarray}
	\intass & \eqdef  &
	\emptyset \mid \Set{ \perm{\kap} : \exsts{\vec{\lvar}} (\lpre \mid \fp_{p}) \transfersto (\lpost | \fp_{q} ) } \cup \intass 
\end{rclarray}
\]
...
%
\sx{ at some place it should have semantics check for the well-form }
\[
\begin{rclarray}
	\evalI{\emptyset}(\kap) & \eqdef & \emptyset \\
	\evalI{\Set{ \perm{\kap} : \exsts{\vec{\lvar}} (\lpre \mid \fp_{p}) \transfersto (\lpost \mid \fp_{q} ) } \cup \intass }(\kap) & \eqdef &
	\Setcon{
		...
		\quad \h_\lpre \in \evalLS[\lenv', \stk]{\lpre} \land \h_\lpost \in \evalLS[\lenv', \stk]{\lpost} \\
		\quad \evset_\lpre \in \evalF[\lenv', \stk]{\fp_{p}} \land \evset_\lpost \in \evalF[\lenv', \stk]{\fp_{q}}
	}
	\cup 
	\evalI{\intass}(\kap)
\end{rclarray}
...
\begin{rclarray}
    ...
    \Setcon{
        ...
        \ca \in \unitC 
        \land \gs(\lrid)\projection{1} \in \evalLS{\lpre}
...
\begin{defn}[Heap-to-Execution]
\label{def:heap2aexec}
Given a consistency model \( \como \) (\defref{def:consistency-models}), a bijective function from heaps to sets of abstract executions, \( \funcn{h2e} : \ConsisModels \to \Heaps \to \powerset{\Aexecs}\) is defined as the follows,       
\[
    \begin{rclarray}
        \func{h2e_{\como}}{\h} & \defeq & \Setcon{\aexec}{\exsts{\respo}\como = (\stub, \respo) \land \obsstate{\aexec,\aexec\prjT,\respo}}
    \end{rclarray}
\]
\end{defn}
