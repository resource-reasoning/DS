...
\sx{ We might be able to eliminate all the heap assertions by combining them to fingerprints.
Extend \( \lexpr \fpI\) by replacing \( \backslash \) with the initial value.  }
...
\azalea{The definition of $\evalLS{ \bar{\fp} }$ looks cyclic as you use the same notation for interpreting local assertions and fingerprint assertions.}
    	...
    	\begin{cases}
    		\begin{array}{@{} l @{}}
	    		\Setcon{
	    			...
	    			\exsts{\vec{\val},\txid, \opset, \extopset} \\
	    				\quad \extopset {=} \evalF[\lenv\rmto{\vec{\lvar}}{\vec{\val}},\stk]{\fp} 
		         	\land \txid \in \func{fresh}{\hh}  \\
		         	\quad \land \extopset = (\opset, \ca_a, \ca_r)\\
...
\azalea{Why is the $(\hh, \vi) \in \func{inv}{\lrid, \intf}$ conjunct needed in the definition of $\evalW{ \boxass{\bar{\lpre}}{\lrid}{\intass} }$?}