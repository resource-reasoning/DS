...
\begin{rclarray}
    ...
    \begin{array}[t]{@{}l}
        \exsts{\vis, \po, \ar, \evset \in ( \dom(\intf( \kap )) { \color{blue}\uplus \Set{\emptyset} } ) } { \color{blue} \text{ allow to append an empty `box'} }\\
...
\[
\begin{rclarray}
    \aexec \in \clpsW{( \ca, \emptyset )} & \iffdef & \for{ \txid } \aexec\prjT(\txid) = \emptyset \\
    ( \aexec \composeAEX \aexec' ) \in \clpsW{(\ca, \Set{\rid \mapsto (\h, \intf)} \uplus \gs)} & \iffdef & 
            \aexec \in \func{s2e}{\rid, \h, \intf} \land \pred{wf}{\rid, \intf} \land \aexec' \in \clpsW{(\ca, \gs)} \\
\end{rclarray}
\] 
% 
