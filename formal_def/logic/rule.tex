...
A world transition is allowed by a consistency model \( (\w, \w') \in \como \), if there exist a operation set \( \opset \) that update the corresponding machine states from \( (\hh, \vi) \)  to \( (\hh', \vi') \),
\[
\begin{rclarray}
    (\w, \w') \in \como & \defeq & 
    \begin{array}[t]{@{}l}
    \for{\hh, \vi} \exsts{\txid, \hh', \vi', \opset} \\
