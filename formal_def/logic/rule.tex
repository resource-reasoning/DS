...
Regions in a world have disjointed history heaps and views, so we allow a transaction to update several regions together, but each region can be updated at most once.
Given that, the guarantee is a set of world transitions that are allowed by the local capabilities.
The rely is a set of transitions that the history heaps are changed changes against the residual of the local capabilities.
...
\[
    ...
    \end{rclarray}
\]
\azalea{\sx{Done, constraint this at the type of world.}What is $\pred{wfView}{\vi',\hh}$?}
...
\begin{rclarray}
    ...
    \begin{array}[t]{@{}l@{}}
        ...
        \qquad {} \land
        \exsts{{\color{blue}???\stk'???} } 
        \for{\txid, \opset} 
        \exsts{ \w', \hh', \vi'} \\
        \qqquad \txid \in \func{fresh}{\hh} 
