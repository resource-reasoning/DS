...
\sx{To allow a transaction update multiple regions}
...
\begin{rclarray}
    ...
    \land \opset \in \intf(\kap)\projection{2}
...
\[
    \begin{rclarray}
        \stable{\setworld} & \eqdef & \for{\w_{p}, \w_{q}}  \w_{p} \in \setworld \land (\w_{p}, \w_{q}) \in \Rely \implies \w_{q} \in \setworld
    \end{rclarray}
\]
...
\begin{rclarray}
    ...
    \begin{array}[t]{@{}l}
        \for{ \w, \h, \hh, \cu }
        \w \in \evalW{\gpre} 
        \land (\hh, \cu) \in \clpsS{\w}  \\
        \quad {} \land \h = \func{clps}{\hh, \cu} \implies \h \in \evalLS{\lpre}\\
    \end{array} \\
    ...
    \begin{array}[t]{@{}l@{}}
        ...
        \land \exsts{\txid} \hh' = \func{commit}{\hh, \cu, \txid, \evalF{\fp}}  \\
        \quad {} \land (\hh',\cu') \in \clpsW{\w'}
        \land (\w, \w') \in \Guar 
...
\begin{rclarray}
    ...
    \end{array} \\
    \pred{unboxleft}{\w, \lpre} & \defeq & \for{\hh} (\hh, \stub) \in \clpsW{\w} \land \exsts{\cu_{max}} \for{\addr} \cu(\addr) = \left| \hh(\addr) \right|  \implies \func{clps}{\hh, \cu_{max}} \in \evalLS{\lpre}
\end{rclarray}
\]

\hrule\vspace{5pt}
\caption{Syntactic repartition for SI and SER and Causal}
\label{fig:rule-prog}
\end{figure}
