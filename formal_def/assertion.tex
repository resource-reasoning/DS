...
\] 
\sx{
    Change the write part to \( \subseteq \), because the post condition could be write and read.
    A more constrained way is to say \( \fp = \emptyset \) to enforce the pre-conditions has no fingerprints.
    }
%
...
Then given the partial commutative monoid of capabilities ($\Caps$, \composeC, \unitC) in \defin\ref{def:capabilities}, the set of \emph{logical states} is: \(\lgs \in \LGStates \eqdef \Heaps \times \Caps\).
...
\]
%
where $\V x, \V r \in \LVars$, $\lexpr_1, \lexpr_2 \in \LExpressions$ (\defin\ref{def:local_assertions}), $\kap \in \Kaps$ (\defin\ref{def:capabilities}) and $\intass \in \IAssertions$ (\defin\ref{def:intf}).
%
...
%\[
    %\begin{rclarray}
        %\lstate \in \Logicalstate & \defeq & \TSHeaps \times \powerset{\Capability} \\
...
%\[
    %\begin{rclarray}
        %\lstate_{1} \compose_{\lstate} \lstate_{2} & \defeq & (\lstate_{1}\projection{1} \ \uplus \ \lstate_{2}\projection{1}, \lstate_{1}\projection{2} \ \compose_{\cstate} \ \lstate_{2}\projection{2}) \\
...
%\[
    %\begin{rclarray}
        %\world_{1} \compose_{\world} \world_{2} & \defeq & 
        %\begin{cases}
            %(\sstate, \lstate_{1} \compose_{\lstate} \lstate_{2}) & \world_{1} = (\sstate, \lstate_{1}) \land \world_{2} = (\sstate, \lstate_{2}) \\
            %\texttt{undefined} & o.w. \\
        %\end{cases} \\
    ...
    %\begin{rclarray}
        ...
        %\eval{\expr_{1} \pt \expr_{2}}_{\lenv, \stk} & \defeq & \Set{ ( ( \emptyset, ( \tshp, \emptyset ) ), \ts ) \middle| 
            %\begin{array}{@{}l@{}}
                ...
                %\tshp(\loc)(\ts') = (\val,\stub, \stub) \land \forall \ts'' \in ( \ts', \ts) \ldotp \tshp(\loc)(\ts'')\undef \\
            %\end{array}
        %} \\
        %\eval{\boxass{\gpre}{\intass}{\rid}}_{\lenv, \stk} & \defeq & \Set{ ( ( \sstate, \lstate ), \ts ) \middle| \exists \sstate', \lstate' \ldotp ( ( \sstate', \lstate \compose_{\lstate} \lstate' ), \ts ) \in \eval{\gpre}_{\lenv, \stk, \ts} \land \sstate = \sstate' \uplus \Set{\rid \mapsto ( \lstate', \eval{\intass}_{\lenv,\stk} ) } } \\
        ...
        %\eval{ \gpre \sep \gpost }_{\lenv, \stk} & \defeq & \Set{ ( \world_{1} \compose_{\world} \world_{2}, \ts ) \middle| ( \world_{1}, \ts ) \in \eval{\gpre}_{\lenv, \stk, \ts} \land ( \world_{2}, \ts ) \in \eval{\gpost}_{\lenv, \stk, \ts} } \\
...
%The first element, also pre-condition \( \lpreext \), only contains empty label, \( \stub \pt_{\emptyset} \stub \), because the semantics requires the pre-condition interpreted as a heap with empty write set and empty read set.
...
%%%
%%\[
%%	x \pt_{\emptyset} 1 \transfersto x \pt_{\Set{\fpR}} 2
%%\]
%%%
