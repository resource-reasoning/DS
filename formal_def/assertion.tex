...
%\[ 
%    \begin{rclarray}
%        \fp \in \Fingerprint & \defeq & \powerset{\Set{\wfp,\rfp}} \\
...
\[
	...
	\begin{cases}
		...
		\ca_2(\rid) & \text{if } \rid \in \dom(\ca_2) \text{ and } \rid \not\in \dom(\ca_1) \\
...
Given a stack $\stk \in \Stack$ (\defin\ref{def:stacks}) and a logical environment $\lenv: \LEnv$, the \emph{logical expression evaluation} function, $\leval{.}{(., .)}:\Expressions \times \Stack \times \LEnv\rightharpoonup \Val$, is defined inductively over the structure of logical expressions as follows: 
%
\[
    \begin{rclarray}
...
%
	\aeval{\exsts{\V x} \ldotp \lpre}{\lenv, \stk} \defeq & \bigcup\limits_{\val \in \Val}\aeval{\lpre}{\lenv\remapsto{\V x}{\val}, \stk}  \\
%
...
%
	...
	& \Set{
        ...
        \begin{array}{@{}l@{}}
			\exsts{\loc, \val} 
			\leval{\expr_1}{\lenv, \stk} = \loc 
			\land \leval{\expr_2}{\lenv, \stk} = \val  
			\land \dom(\fph) {=} \loc 
			\land \fph(\loc) {=} (\val, \fp)
		\end{array}
	} \\
...
\]
%
where $\subseteq$ denotes the standard subset relation. Or write:
%
\[
	\sqsubseteq \defeq \{(\emptyset, f), (\Set{\rfp}, \Set{\rfp, \wfp})\}^{*}
\]
%
where $*$ denotes the reflexive transitive closure of the relation.
%
The diagram is intuitive. If you want to keep it, then write:
%
\[
	\sqsubseteq \defeq \rightarrow^{*}
\]
%
...
\[
    \begin{rclarray}
        ...
        \eval{\exists \lvar \ldotp \lpre}_{\lenv, \stk} & \defeq & \bigcup\limits_{\val \in \Val} \eval{\lpre}_{\lenv\remapsto{\lvar}{\val}, \stk}  \\
    \end{rclarray}
\]
...
Capability assertions are interpreted over the \emph{capability} PCM $\textsc{CAP} \defeq (\mathbb{K}, \compos_{\kappa}, \emptyset)$ where $\kappa \in \mathbb{K} \defeq (\Regionid \parfinfun \Gamma)$, the $\emptyset$ denotes the function with empty domain and 
%
\[
	(\kappa_1 \compos_{\kappa} \kappa_2)(\rid) \defeq 
	\begin{cases}
		\kappa_1(\rid) \compos_{\Gamma} \kappa_2(\rid) & \text{if } \rid \in \dom(\kappa_1) \text{ and } \rid \in \dom(\kappa_2) \\
		\kappa_1(\rid) & \text{if } \rid \in \dom(\kappa_1) \text{ and } \rid \not\in \dom(\kappa_2) \\
		\kappa_2(\rid) & \text{if } \rid \in \dom(\kappa_2) \text{ and } \rid \in \dom(\kappa_1) \\
		\text{undefined} & \text{otherwise}
	\end{cases}
\]

Also $\capass$ is a bad name for capability assertions because i) you use $t$ for time stamps; and ii) it conveys no information.
}
...
%
\azalea{
%	For now, I recommend disallowing nested boxes. In CAP it was necessary as we were building up layers of abstraction and it was necessary to nest the resources of one module in another. Here, you do not have abstraction so disallowing nested boxes is not a big loss. So instead of $\boxass{\gpre}{\intass}{\rid}$ in your grammar write $\boxass{\lpre}{\intass}{\rid}$ where $\lpre$ is a local assertion (extended with capability assertions as described earlier).\\
	For the sake of consistency with CAP and CoLoSL, I'd write $\boxass{\gpre}{\rid}{\intass}$ instead of $\boxass{\gpre}{\intass}{\rid}$. 
}
\sx{ The nested box is interpreted as flatten box assertion.
Yes, \( \rid \) and \( \intf \) should change position.
}
%
...
\sx{ The domain it the first projection's domain, which is the raw heap.
    \( \wmrg \) take the write effect from the right-hand side to the left-hand side, but only those resource that are in the left-hand side.
    For example, if the left-hand side is a state \( \fphp \) satisfying \( \vx \pt_{\emptyset} \_\) and the right-hand side is a state \( \fphp' \)  satisfying \( \vx \pt_{\wfp} 1 \sep \vy\pt_{\wfp} \_ \), the merging result will be a state satisfying \( \vx \pt_{\wfp} 1\).
    }
