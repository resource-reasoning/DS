...
The \emph{merge of actions}, specifically merges the right-hand side to the left-hand side, is defined as follows:
\[
    \begin{rclarray}
...
Given the merge of action (\defin\ref{def:merge-action}), the \emph{merge of interference} is defined as follows:
\[
    \begin{rclarray}
        \funcn{merge\_interference} & : & \Actions \times \Interference \parfun \powerset{\Actions} \\
    ...
    \end{rclarray}
\]
It merges all possible actions from interference (right-hand side) to an  action (left-hand side), including self-merge which yields the same action.
\end{defn}
