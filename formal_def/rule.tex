...
\begin{defn}[Chain of actions]
    First, the \emph{chain of actions}, \( \predn{chain}  : \Actions \times \Actions \parfun \Actions \) is defined as follows:
    \[
        \begin{rclarray}
            \pred{chain}{((\fph_{l}^{p}, \ca_{l}^{p}), (\fph_{l}^{q}, \ca_{l}^{q}) ), ((\fph_{r}^{p}, \ca_{r}^{p}), (\fph_{r}^{q}, \ca_{r}^{q}) )} & \defeq & ((\fph^{p}, \ca^{p}), (\fph^{q}, \ca^{q}) )\\
        \end{rclarray}
    \]
\end{defn}
