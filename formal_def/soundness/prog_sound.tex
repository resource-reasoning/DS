...
By the \( \consistenName \) predicate (\figref{fig:thread_semantics}), the transaction \( \tsid' \) cannot write to any address read by \( \thid \),
\[
...
\caseB{\(i = 1 \)}
\caseB{\(j = 1 \)}
Since nothing happens before  time \( \ts_{1}^{s} \), we know that \( \for{\addr} \snapshot{\tsh_{1}}{ \ts_{1}^{s}}(\addr) =  \flattenFW{\fpw_{s}}\projection{2}(\addr) \), meaning the value for each address is the same.
...
Given \( \tsh_{i} \) is a series that is ordered by transactions' commit times, we know \( \ts_{e} = \ts_{j-1} \).
