...
\[
    \begin{array}{@{}l@{}}
    \for{i,j}
    \exsts{\fpw_{s}, \fpw_{e}, \ts_{s} = \min\Set{\ts_{i}^{s}, \dots, \ts_{j - 1}^{s}}, \ts_{e} = \max\Set{\ts_{i}^{e}, \dots, \ts_{j-1}^{e}}} 
    \ts_{i} < \ts_{j} \\
    \quad {} \land (\tsh_{i}, \ts_{s}) \in \func{x2tsh}{\fpw_{s}} 
    \land (\tsh_{j}, \ts_{e}) \in \func{x2tsh}{\fpw_{e}}
    %\land (\tsh_{1}, \tsh_{i}) \in \func{xx2tsh}{\fpw_{s}, \fpw_{e}}
...
\caseB{\(i = 1 \) and \( j = 2 \)}
By \defref{def:trans-run-within-env}, we have \( ( \tsh_{1}, \tsh_{2}) \in \Rely \), therefore \( (\fpw_{s}, \fpw_{e}) \in \rely(\intf) \subseteq \RelyI\)
\caseI{\( i = 1\) and \(j > 2\)}
