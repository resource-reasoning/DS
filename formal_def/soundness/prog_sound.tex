...
\end{array}
\end{equation}
\sx{ (NEED A LEMMA and NEED TO DEFINE \( \w, \w' \in \como\) ) }
Since \( (\w, \w') \in \como \), this means the transition from \( (\hh,\vi) \) to \( (\hh',\vi') \) satisfies the consistency model, hence we prove \equref{equ:commit-transaction}, 
\[
\begin{array}{@{}l}
    \for{\w,  \hh, \hh', \vi, \vi', \lenv, \stk, \stk', \txid, \opset} \exsts{\w'} \\
    \quad \w \in \evalW{\gpre}
