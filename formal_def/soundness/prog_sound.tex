...
Give the \ih and the soundness result per thread, we need to prove for any possible traces from the first thread, there exist a trace from the second thread ending up with the same state, and vice versa.
\sx{Need proper term for this?}
We prove this by constructing the traces.
Initially, because \( \gpre_{1} \sep \gpre_{2} \) is defined, we pick any state \( \w_{1}^{0} \) such that \( \w_{1}^{0} \in \evalW{\gpre_{1}}\), \( \w_{2}^{0} \in \evalW{\gpre_{2}}\) and \( (\w_{1}^{0} \composeW \w_{2}^{0})\isdef\).
We now prove for any step in the first trace that has side effect on the history heap, there exists a corresponding step in the second trace and vice versa.
\[
\begin{array}{@{}l}
    \fora{\tau_{1}, \tau_{2}, \w_{1}, \w_{1}', \w_{2}, \w_{2}'}
\end{array}
\]
Assuming after certain steps the global states in both traces end up to be compatible, says \( \w_{1}^{i} \) and \( \w_{2}^{j} \) such that \( (\w_{1}^{i} \composeW \w_{2}^{j})\isdef\).
There are three possibilities: (a) the first trace commits a transaction and the second trace takes a rely step; (b) the first takes a rely step; and the second thread either commit a truncation or take a rely step.
The first two cases are symmetric, so let us assume the first trace commits a transaction.
This means there is a world transfer is allowed  by the guarantee \( (\w_{1}^{i}, \w_{1}^{i+1}) \in \Guar \).
\end{proof}

\begin{lem}[Locality of update]
When a thread commit a transaction that is allowed by guarantee, the effect to the history heap is included in the rely of other threads.
\label{lem:locality-update}
\[
\begin{array}{@{}l}
    \fora{\w, \w', \w''} (\w \composeW \w')\isdef \land (\w, \w'') \in \Guar  \\
    \quad \implies \exsts{\w'''} (\w', \w''') \in \Rely \land \clpsW{\w''}\projection{1} = \clps{\w'''}\projection{1}
\end{array} 
\]
\end{lem}
\begin{proof}
...
Assuming \( ((\ca, \gs), (\ca', \gs') ) \in \Guar \), by the \defref{def:rely-guarantee}, we know that for any region \( \rid \) that has been updated, there exists a \( \kap \) such that \( \kap \sqsubseteq \ca(\rid) \) which gives the permission, this is, 
\[
\begin{array}{@{}l}
    \fora{\hh, \hh', \vi, \vi', \ca'', \ca''', \intf, \rid} \exsts{\kap} \\
...
\end{array} 
\]
We can construct a world \( \w'''\) that satisfies the rely \( (\w', \w''') \in \Rely \).
First, we take the same local capabilities as the \( \w' \).
Also we take the same states for regions that remain the same between \( \w \) and \( \w'' \).
For those regions being updated, we propagate the update for history heaps to \( \w''' \) but keep the views as the same in the \( \w' \).
This is allowed by \( ( \ca \composeC \ca' )\isdef \).
It is easy to see \( ( \w', \w''' ) \in \relyU \) by \defref{def:rely-guarantee}, therefore \( ( \w', \w''' ) \in \Rely \)
\end{proof}
