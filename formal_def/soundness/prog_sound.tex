...
\end{array}
\]
by reordering some reduction steps by \lemref{lem:reorder-concurrency} and \lemref{lem:reorder-the-future-and-past}, we can also assume the transactions from the environment and the current transactions  are ordered by the commit times.
\[
\begin{array}{@{}l@{}}
...
Given a transaction \( \trans \) and a possible environment with it corresponding states \( \tsh_{0} \dots \tsh_{\nat} \), and rely-guarantee \( \Rely, \Guar \) (\defref{def:trans-run-within-env}), the \emph{merge soundness} is as the follows.
...
\sx{ Need massive change, similar to merge, but much easy to prove}
\begin{lem}[Stabilisation soundness]
\label{lem:stable-sound}

Given a transaction \( \trans \) and a possible environment with it corresponding states \( \tsh_{0} \dots \tsh_{\nat} \), and rely-guarantee \( \Rely, \Guar \) (\defref{def:trans-run-within-env}), the \emph{stabilisation soundness} is as the follows.
\[
\begin{array}{@{}l@{}}
