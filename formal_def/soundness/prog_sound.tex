...
Assume the start and commit time for each transactions are \( \ts_{i}^{s} \) and \( \ts_{i}^{e}\), \ie,
\[
\begin{array}{@{}l@{}}
    ...
    \land \tsh_{i + 1}(\addr)(\ts_{s}) = (\stub,\etag_{s}, \stub) 
    \land \tsh_{i + 1}(\addr)(\ts_{e}) = (\stub,\etag_{e}, \stub) 
\end{array}
\]
...
\[
    ...
    \exsts{\fpw_{e}}
    i \leq j \\
    \quad {} \land (\tsh_{i}, \ts_{i}^{s}) \in \func{x2tsh}{\fpw_{s}} 
    \land (\tsh_{j}, \ts_{j}^{e}) \in \func{x2tsh}{\fpw_{e}}
    \land (\tsh_{i}, \tsh_{j}) \in \func{xx2tsh}{\fpw_{s}, \fpw_{e}}
