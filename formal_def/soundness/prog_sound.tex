...
\sx{The rely part is not very precise, as should include the consistency model.}
\[
\begin{array}{l}
    \begin{B}
        \stable{\gpre, \como} 
...
The \( \stable{\gpre, \como} \) predicate asserts any world \( \w \) that satisfies the pre-condition \( \gpre \), if the world can transfer to another world \( \w' \) through rely \( \Rely \), and if the transfer also satisfies the consistency model, the new world \( \w' \) also satisfies the pre-condition, which implies \equref{equ:stable-pre-condition}. 
\\
