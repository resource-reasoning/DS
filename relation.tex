...
We pick a invariant \( I \) that is always empty since \( \vshift[\SER]{\mkvs}{\vi}{\mkvs'}{\vi} \) is always true.
When a client commits a transaction $\txid$ 
...
This means that all the abstract executions in $\extend[\aexec, \txid, \Tx[\hh, \vi], \fp]$ are such that there is a visibility 
edge $\txid' \xrightarrow{\VIS} \txid$ where $\txid'$ is a writer transaction in $\hh_{\aexec}$. 
It is easy to see there exists an singleton set \( \Set{\aexec'} \) that is subset of $\extend[\aexec, \txid, \Tx[\hh, \vi], \fp]$ and \( \aexec' \models \Ax_\SER \), in particular, 
there is an edge $\txid' \xrightarrow{\VIS} \txid$ for any transaction $\txid' \in \T_{\aexec'}$.
For example, in \cref{fig:et-sound-to-aexec}, the possible abstract executions are $\aexec_1, \aexec_2$.
The singleton set abstract execution is $\Set{\aexec_2}$, because \( \aexec_2 \) that admits 
the visibility edge $\txid_{2} \xrightarrow{\VIS} \txid_{3}$ satisfies \(  \aexec_2 \models \Ax_\SER \). 
Because $\aexec \models \Ax_{\SER}$, and because 
