...
Consistency models can be specified by constraining the shape of dependency graph; typically, such constraints mandate the absence of particular cycles. 
For example, strict serialisability is defined as those dependency graphs where $(\SO \cup \WW \cup \WR \cup \RW)^+$ is acyclic. 
Dependency graph-based specifications can be converted into execution-test-based ones: such execution 
tests prevent committing transactions that result in kv-stores whose associated dependency graph does not 
have the shape required by the consistency model.
...
An abstract execution is a triple $\aexec = (\TtoOp{T}, \VIS, \AR)$, where 
 $\TtoOp{T}: \TxID_{0} \parfinfun \pset{\Ops}$ is a partial, 
 $\TtoOp{T}: \TxID_{0} \parfinfun \pset{\Ops}$ is a partial, 
...
Formally, an axiom is a function $\A : \aeset \to \pset{\TxID} \times \pset{\TxID}$, 
...
$\aeset$. 
\sx{There is a clash on label. Will relabel this later}
\begin{theorem}
...
consider the $i$-th $\ET_{\top}$-reduction of $\tau$, $(\hh_{i-1}, \vienv_{i-1}) \xrightarrow{(\cl_{i}, \lambda_{i})}_{\ET_{\top}} (\hh_i, \vienv_{i})$. 
...
view of client $\cl_{i}$ in $\vienv_{i-1}$, but it may also contain any other read-only transaction 
that is already present in the abstract execution. Formally, given a kv-store $\hh$ and a view $\vi$, we let 
$\Tx[\hh, \vi] \defeq \Setcon{ \WTx(\hh(\key, i)) }{ \key \in \Keys \wedge i \in \vi(\key) }$. Then, for each 
...
In practice, our proof techniques also takes into account the possibility of defining an invariant over the set $\Tx(\stub, \stub)$, which 
