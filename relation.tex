...
transactions read the  from their also visible write in the arbitration order: $\forall \txid \in \T_{\aexec}.\; (\otR,\key,\val) \in_{\aexec} \txid 
...
%consider the associated kv-store $\hh_{\aexec}$. 
%Consider also a tuple of the form , 
%where $\vi, \opset, \mkvs', \vi'$ are arbitrary. 
%The user constructs a non-empty subset of $\extend[\aexec', \txid_{3}, \Tx[\hh, \vi], \fp]$ 
%that satisfies the axiomatic specification $\Ax$..
...
%The user constructs a non-empty subset of $\extend[\aexec', \txid_{3}, \Tx[\hh, \vi], \fp]$ 
%that satisfies the axiomatic specification $\Ax$..
%We construct an abstract execution 
