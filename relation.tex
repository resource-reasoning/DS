...
%, for all $\ET_1,  \ET_2$, 
%the consistency model induced by the composite execution test 
...
However, the other direction of \cref{prop:composite_stronger} does not hold for arbitrary consistency models.
...
%\sx{\cref{sec:comm-counter-cp-si}}
%}
...
%\begin{enumerate*}
%	\item a \emph{read dependency} edge, $\txid_1 \xrightarrow{\WR(\ke)} \txid_2$, denotes
%that transaction $\txid_2$ reads a version of $\ke$ written by $\txid_1$;
%	\item a \emph{write dependency} edge, $\txid_1 \xrightarrow{\WW(\ke)} \txid_2$, denotes that $\txid_2$ overwrites a version of $\ke$ written by $\txid_1$; and 
%	\item an \emph{anti-dependency} edge, $\txid_1 \xrightarrow{\RW(\ke)} \txid_2$, denotes that $\txid_2$ overwrites a version of $\ke$ read by $\txid_2$. 
%\end{enumerate*}
\begin{enumerate*}
...
\azalea{Why do we have $\WR(\ke)$ etc. in \cref{fig:dependency-graph}? We don't have keys in the text; we have $\WR$, etc. If Keys are needed, the text needs to be corrected, but keys don't seem to be needed. }
%
...
%if we apply the construction outlined above to the kv-store 
...
%acyclic dependency graphs (where a pair of the form $(t, t') \in \PO$ is also considered to be an edge of the graph). 
...
%a transaction into a kv-store,  no cycle disallowed by the (dependency-graph based) consistency model specification 
...
As with dependency graphs, consistency models using abstract executions are specified by constraining the shape of abstract execution graphs via a set of \emph{axioms} $\Ax$, \eg mandating the absence of certain cycles.
...
%We next define what it means for an execution test $\ET$ to be \emph{sound} against an axiomatic specification $\Ax$.
...
The formal definition of execution test soundness is given in \cref{?}.
\azalea{Now, we need to give an intuitive definition of this soundness here, with the technical details moved to the appendix.}


%We then define what it means for an execution test $\ET$ to be \emph{complete} against an axiomatic specification $\Ax$.
