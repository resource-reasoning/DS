...
to~\ref{giveappendix} for the full details.
...
that the consistency models for kv-stores given in~\ref{fig4} are in fact equivalent
...
to results in~\cite{.} which show equivalence of  consistency models for dependency
...
Abstract executions provide another another formalism for specifying
...
and {read atomic} given by  the set of abstract
...
In~\ref{appexdixref}, 
we prove that all the constistency models
for kv-stores given in~\ref{fig4} have equivalent 
 consistency models for abstract
executions, as  defined
in~\cite{principle-eventual-consistency,surech-session-guarantee,framework-concur,laws}.
...
in~\ref{appendixE.3,sorrylostit}. 
...
\end{theorem}
\noindent The proof of theorem \ref{thm:aexec2kvtrace} is give
in~\cref{sec:aexectrace2kv}. Together, theorems \ref{thm:kvtrace2aexec} and \ref{thm:aexec2kvtrace} establish that $\CMs(\ET_{\top})$ is 
sound and complete with respect to the set of abstract executions
allowed by the read atomic consistency model. 

\mypar{Equivalence of axiomatic specifications and execution tests}
