...
and let $\SO$ as defined in \cref{subsec:kvstores}, and $ \WR, \WW, \AD$  
...
\node(t0wx) at (-1,2) {$(\otW, \key_1, \val_0)$}; 
\path (t0wx.east) + (1,0) node (t0wy) {$(\otW, \key_2, \val'_0)$};
\path (t0wx.north east) + (2.5,0.5) node[anchor = west] (t1ry) {$(\otR, \key_2, \val'_0)$}; 
\path (t1ry.east) + (0.2,0) node[anchor = west] (t1wx) {$(\otW, \key_1, \val_1)$};
\path (t0wx.south east) + (2.5,-0.5) node[anchor = west] (t2rx) {$(\otR, \key_1, \val_0)$};
\path (t2rx.east) + (0.2,0) node[anchor = west] (t2wy) {$(\otW, \key_2, \val'_1$)};
...
Given a set of axioms $\Ax$, we define $\CMs(\Ax) \defeq \{\mkvs_\aexec \mid \aexec \text{ sats.\ } \Ax \}$.
...
all $i \in \{1 \cdots n\}$, views $\vi_i, \vi'_i$, transactions $\txid'$, and fingerprints $\fp_i$,
whenever
\begin{enumerate*}
...
\(
    \CMs(\ET) \subseteq \{ \mkvs_\aexec \mid \aexec \in \CMs(\Ax)\}
\).
For all $\ET, \Ax$, if $\ET$ is complete against $\Ax$, then:
\(
    \{ \mkvs_\aexec \mid \aexec \in \CMs(\Ax)\}  \subseteq \CMs(\ET)
\).
\end{theorem} 
