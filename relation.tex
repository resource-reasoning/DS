...
that the consistency models for kv-stores given in~\ref{fig:execution-tests} are in fact equivalent
...
An {\em axiom} is a function $\A : \aeset \to \pset{\TxID \times
...
In~\ref{app:et_sound_complete}, 
we prove that all the consistency models
for kv-stores given in~\ref{fig:execution-tests} are equivalent to
...
abstract execution $\aexec_{\mathsf{partial}} \in \execs_{i}^{\mathsf{partial}}(\tau)$ we determine a set of sets of visible transactions for $\txid_{i}$:  
