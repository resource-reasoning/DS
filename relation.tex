...
In this Section, we compare our specifications of transactional consistency 
models with an alternative, axiomatic specification style based on abstract 
executions \cite{framework-concur}. We give the formal definitions of abstract 
executions and axiomatic specifications shortly. The main contribution of this 
section is the development of a general proof technique for proving the equivalence 
of execution-test-based and axiomatic specifications of consistency models. 
As it will become clear later in this section, our proof technique keeps the burden 
in terms of proof obligations that must be satisfied by its user to a minimum. 
\textbf{Don't like this sentence: What I am trying to say in English is: the user just 
takes care of some details, i.e. that the constraints on views in execution tests 
relate to the constraints on visibility edges in abstract executions; then we 
have an automatic way to map an $\ET$-trace terminating into a kv-store 
$\mkvs$ into a set of abstract executions that satisfy the axiomatic specification, 
and whose underlying kv-store is also $\mkvs$.}
We show how our proof technique can be applied to prove that the execution 
tests for monotonic reads and serialisability are sound with respect to their 
]axiomatic specifications, respectively. In \cref{?}, we apply our proof technique 
to show that all the execution tests from \cref{fig:execution.tests} are equivalent 
to their respective axiomatic specification. 
...
Like dependency graphs, abstract executions are labelled graph 
whose nodes consist of transaction identifiers and their associated fingerprints. 
These nodes may be connected either by a \emph{visibility edge} $\txid \xrightarrow{\VIS} 
\txid'$, meaning that $\txid'$ sees the updates of $\txid$, or $\txid \xrightarrow{\AR} 
\txid'$, meaning that updates from $\txid'$ overwrite updates from $\txid$. 
Figure \ref{fig:abstract_execution} gives an example of abstract execution.
...
fingerprints, with $\TtoOp{T}(\txid_{0} = \{ (\otW, \key, \val_{0}) \mid \key \in \Keys\}$, 
...
Given an abstract execution $\aexec$ and $\txid: (\otR, \key, \val) \in_{\aexec} \txid$, 
the \emph{visible writes} of $\txid$ in $\aexec$ are defined by 
$\visibleWrites_{\aexec}(\key, \txid) = \VIS^{-1}_{\aexec}(\txid) \cap 
\{\txid' \mid (\otW, \key, \stub) \in_{\aexec} \txid'\}$. 
An abstract execution $\aexec$ satisfies the \emph{last-writer-wins} policy if, for any 
transaction $\txid$ such that $(\otR,\key,\val) \in_{\aexec} \txid$, then 
$(\otW, \key, \val) \in \max_{\AR_{\aexec}}(\visibleWrites_{\aexec}(\key, \txid)$.
...
last-writer-wins policy and, henceforth, only consider abstract
executions in $\aeset$. 
...
Axiomatic specifications are a specification style of consistency models that 
impose constraints on the overall structure of abstract executions. 
For most consistency models \cite{laws,concur-2015,suresh-concur17}, these constraints  
specify the conditions under which a transaction \textbf{must} see the updates of 
another transaction. For example, monotonic reads can be specified by requiring 
that if a transaction $\txid$ follows in the session order another transaction $\txid'$, 
then $\txid$ must see any transaction that is also seen by $\txid'$.
Serialisability can be specified by requiring that 
in an abstract execution $\aexec$, a transaction $\txid$ must see all the transactions 
that precede itself in the arbitration order.
%form $\mathcal{R}_{\aexec} \subseteq \VIS_{\aexec}$.

Formally, an {\em axiom} is a function $\A : \aeset \to \pset{\TxID \times \TxID}$ 
such that for any an  abstract execution $\aexec$, $\A(\aexec) \subseteq \AR_{\aexec}$. 
We say that $\aexec \models \A$ if and only
if $\A(\aexec) \subseteq \VIS_{\aexec}$ and, for technical reasons,
$\forall \aexec. \A(\aexec) \subseteq \AR_{\aexec}$. 
Finally, an {\em axiomatic specification}  $\Ax$  is a set of axioms.
Coming back to the example of monotonic reads, 
we let $\Ax_{\MRd} = \{\A_{\MRd}\}$, where $\A_{\MRd}(\aexec) = \VIS_{\aexec} ; \SO_{\VIS}$. 
Then, for any $\aexec \in \aeset$, $\aexec \models \A_{\MRd}$ if and only 
if $\VIS_{\aexec} ; \SO_{\aexec} \subseteq \VIS_{\aexec}$: that is, 
whenever $\txid'' \xrightarrow{\PO_{\aexec}} \txid \xrightarrow{\VIS_{\aexec}} \txid'$, 
then $\txid'' \xrightarrow{\VIS_{\aexec}} \txid$.
Similarly, for serialisability we let $\Ax_{\SER} = \{ \A_{\SER} \}$. The axiom $\A_{\SER}(\aexec) = \AR_{\aexec}$ 
captures the constraint that a transaction $\txid$ must see any other transaction 
that precede itself in the arbitration order.
...
into an equivalent dependency graph $\Gr_{\aexec}$, using 
the construction outlined in \cite{laws}, and hence into an
...
This construction is useful to define the consistency model $\CMs(\Ax)$ induced by the 
axiomatic specification $\Ax$: we let  $\CMs(\Ax) = \{ \hh_{\aexec} \mid \forall \A \in \Ax. \aexec \models \A \}$. 

Our aim in the rest of this section is to develop a proof techniques for showing  
that an execution test $\ET$ and an axiomatic specification $\Ax$ induce the same 
consistency model, that is $\CMs(\ET) = \CMs(\Ax)$. 
Due to space constraints, we focus only on soundness, i.e. on proving an inclusion 
of the form $\CMs(\ET) \subseteq \CMs(\Ax)$, and we leave the details 
about how to prove the opposite inclusion to \ref{app:?}. 
...
such that, for any $\aexec \in \execs(\tau) $ with $\hh_{\aexec} = \hh$, 
...
whenever in $\tau$ a client $\cl$ with view $\vi$ commits a transaction $\txid$, then in any 
...
abstract executions in $\execs(\tau)$ differ only for the set of read-only transactions (i.e. those with no write operations) 
that transactions see. While mapping an $\ET_{\top}$-trace into multiple abstract executions is 
not strictly necessary for the sole purpose of proving the soundness of $\ET_{\top}$ with respect 
to the weakest axiomatic specification, this choice plays a significant role when proving the soundness 
...
The definition of $\execs(\tau)$ is by induction on the length of 
$\tau$. For the $\ET_{\top}$-trace $\tau_{0}$ consisting of a single configuration 
$(\hh_{0}, \stub)$, we let $\execs(\tau_{0})$ contain a single abstract execution, with 
a single transaction $\txid_{0}$ that initialises all the keys to the initial value $\val_{0}$:
$\execs(\tau_{0}) = \{ ([\txid_{0} \mapsto \{ (\otW, \key, \val_{0} \mid \key \in \Keys)\}], \emptyset, \emptyset \}$. 
For a $\ET_{\top}$-trace $\tau = \tau' \toET{(\cl, \alpha)} (\hh', \vienv')$, let $(\hh, \vienv)$ be the last 
configuration appearing in $\tau'$. Suppose also that $\aexec(\tau')$ has been already defined. 
If $\alpha = \varepsilon$, then we let $\execs(\tau) = \execs(\tau')$. 
...
the latter are those transactions of $\hh'$ that never appear as writers of some version. 
Then, for any abstract execution $\aexec' \in \execs(\tau')$, we define a set of abstract 
executions $\extend[\aexec, \txid, \Tx[\mkvs',\vi'], \txidset_{\rd}, \fp]$ as the largest set 
such that, whenever $\aexec \in \extend[\aexec', \txid', \Tx[\mkvs',\vi', \fp], \txidset_{\rd}, \fp]$, 
then \textbf{(i)} $\TtoOp{T}_{\aexec} = \TtoOp{T}_{\aexec'}\rmto{\txid'}{\fp}$, \textbf{(ii)} for any $\txid' \in \T_{\aexec}$, 
$\txid' \xrightarrow{\AR_{\aexec'}} \txid$, and \textbf{(iii)} whenever $\txid' \xrightarrow{\VIS_{\aexec}} \txid$, 
then either $\txid \in \Tx[\hh', \vi']$, or $\txid \in \txidset_{\rd}$.  
Finally, we let $\execs(\tau) = \bigcup_{\aexec in \execs(\tau')} \extend[\aexec, \txid, \Tx[\mkvs',\vi'], \txidset_{\rd}, \fp]$. 

In \cref{app:?} we prove that the reverse construction is possible.
%first, for any 
...
Finally, we prove that the resulting abstract execution $\aexec'$ satisfies $\Ax$. 

\sx{The following paragraph can move to between the end of definition and the beginning of the theorem, since it is the intuition of why the theorem holds}
