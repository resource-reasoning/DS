...
In particular, with the exception of $\UA$, $\CP$ and $\SI$, all execution tests defined in \cref{fig:execution_tests} satisfy the four conditions in \cref{def:et_properties}. 
...
A dependency graph is a directed graph with its 
...
In \cref{???} We demonstrate that the consistency models induced by the $\CP$ and $\SI$ execution tests in \cref{fig:execution_tests} are \emph{equivalent} to their dependency graph counterparts. 
