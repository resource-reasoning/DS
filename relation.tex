...
In this paper, we are interested mainly in abstract executions that satisfy the \emph{last-write-wins} resolution policy: 
...
Furthermore, the constructions used to convert an abstract execution 
into a $\ET_{\top}$-trace and vice-versa are universal, meaning that they can be applied to show the 
...
above to establish the soundness of an execution test with respect to an axiomatic 
specification. In \cref{sec:aexectrace2kv} we propose a similar proof technique 
aimed at proving completeness of execution tests with respect to axiomatic specifications. 
