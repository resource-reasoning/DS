...
We then provide an automatic mapping from $\ET$-trace terminating in a kv-store 
$\mkvs$, to a set of abstract executions that satisfy the axiomatic specification, 
...
axiomatic specifications. In \cref{?} we apply our proof technique 
...
As with dependency graphs, abstract executions are labelled graphs 
...
Figure \ref{fig:abstract_execution} depicts an example abstract execution.
...
Given an abstract execution $\aexec$ and a transaction $\txid$, 
%$\txid: (\otR, \key, \val) \in_{\aexec} \txid$, \azalea{I don't know what this notation is saying}
the \emph{visible writes} of $\txid$ in $\aexec$ are:
$\visibleWrites_{\aexec}(\key, \txid) \defeq \VIS^{-1}_{\aexec}(\txid) \cap 
\{\txid' \mid (\otW, \key, \stub) \in_{\aexec} \txid'\}$. 
An abstract execution $\aexec$ satisfies the \emph{last-writer-wins} policy if, for all 
transactions $\txid$, if $(\otR,\key,\val) \in_{\aexec} \txid$, then 
...
last-writer-wins policy and, henceforth, only consider such abstract
executions.
...
abstract-execution-based specifications are defined by constraining the overall structure of abstract executions. 
...
another transaction. For example, monotonic reads is specified by requiring 
...
such that for all abstract executions $\aexec$, $\A(\aexec) \subseteq \AR_{\aexec}$. 
...
We then show that for all $\aexec \in \aeset$: $\aexec \models \A_{\MRd}$ if and only 
...
An abstract executions $\aexec$ can be mapped 
into an equivalent dependency graph $\Gr_{\aexec}$, using 
the results of \citet{laws}, and hence into an
...
We can then use this construction to define the consistency model $\CMs(\Ax)$ induced by the $\Ax$ axioms: $\CMs(\Ax) \defeq \{ \hh_{\aexec} \mid \forall \A \in \Ax. \aexec \models \A \}$. 
...
we describe the other direction in full in \ref{app:?}. 
...
Then, for all abstract execution $\aexec' \in \execs(\tau')$, we define $\extend[\aexec, \txid, \Tx[\mkvs',\vi'], \txidset_{\rd}, \fp]$ as the largest set 
such that, whenever $\aexec \in \extend[\aexec', \txid', \Tx[\mkvs',\vi', \fp], \txidset_{\rd}, \fp]$, then 
\textbf{(i)} $\TtoOp{T}_{\aexec} = \TtoOp{T}_{\aexec'}\rmto{\txid'}{\fp}$;
\textbf{(ii)} for all $\txid' \in \T_{\aexec}$, 
$\txid' \xrightarrow{\AR_{\aexec'}} \txid$; and 
\textbf{(iii)} if $\txid' \xrightarrow{\VIS_{\aexec}} \txid$, 
...
In \cref{app:?} we describe the reverse construction.
%first, for any 
...
equivalence of abstract-execution-based axiomatic specifications and
our execution tests over kv-stores. 
As discussed above, here we present a proof technique that uses the construction outlined 
above to establish that an execution test $\ET$ can induce  a consistency model that is \emph{sound} with respect to an axiomatic specification $\Ax$. Completeness is handled with a
...
In \cref{app:et_sound_complete} we use our techniques to show the equivalence of our execution tests in  \cref{fig:execution_tests} 
and their corresponding axiomatic specifications using abstract executions.
...
By definition, if $\hh$ is included in $\CMs(\ET)$, then 
we can find an $\ET$-trace $\tau$ that terminates in $(\hh, \stub)$. 
From \cref{thm:kvtrace2aexec} it thus suffices to show that $\execs(\tau)$
%Because 
...
Our proof technique can be summarised as follows.
Let $\aexec \in \aeset$ satisfy the $\Ax$ axioms, and consider the associated kv-store $\hh_{\aexec}$. 
Consider also a tuple of the form $\ET \vdash (\mkvs_{\aexec}, \vi) \csat \opset: (\mkvs', \vi')$, 
...
is crucial for proving the soundness of several execution-test-based specifications (see \cref{app:et_sound_complete}).
