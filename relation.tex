...
We let $\Aexecs$ be the set of abstract executions that satisfy the
...
For most consistency models \cite{laws,framework-concur,sureshConcur}, these constraints  
...
Formally, an {\em axiom} is a function, $\A : \Aexecs \to \pset{\TxID \times \TxID}$,
...
We then show that for all $\aexec \in \Aexecs$: $\aexec \models \A_{\MRd}$ if and only 
...
above to establish that an execution test $\ET$ can induce  a consistency model that is \emph{sound} with respect to an axiomatic specification $\Ax$. \emph{Completeness} is handled with a
similar proof technique, which we give in \cref{sec:kv2aexec-sound-complete}.
...
%for any $\aexec \in \Aexecs(\tau)$, $\hh_{\aexec} = \hh$. Therefore, it suffices to prove 
%$\execs(\tau)$ 
contains at least one abstract execution that satisfies $\Ax$, to conclude 
that $\hh \in \CMs(\Ax)$. 

Our proof technique can be summarised as follows.
Let $\aexec \in \Aexecs$ satisfy the $\Ax$ axioms, and consider the associated kv-store $\hh_{\aexec}$. 
