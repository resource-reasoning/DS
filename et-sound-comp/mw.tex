...
\begin{itemize}
    ...
    \item if \( \txid' \) has at least one write, it is easy to see \( \txid' \in \Tx(\mkvs, \vi) \) since \( j \in \vi(\ke) \wedge \WTx(\mkvs(\ke', i)) \xrightarrow{\PO?} \WTx(\mkvs(\ke, j)) \implies i \in \vi(\ke') \).
\end{itemize}
