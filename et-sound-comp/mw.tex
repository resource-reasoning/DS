...
\begin{itemize}
    ...
    \item if \( \txid' \) has at least one write, it is easy to see \( \txid' \in \Tx(\mkvs, \vi) \) since \( j \in \vi(\key) \wedge \WTx(\mkvs(\key', i)) \toEDGE{\PO?} \WTx(\mkvs(\key, j)) \implies i \in \vi(\key') \).
\end{itemize}
...
\begin{array}{@{}l@{}}
    ...
    \wedge \WTx(\mkvs_{\cut(\aexec, i-1)}(\key', m)) \toEDGE{\PO?} \WTx(\mkvs_{\cut(\aexec, i-1)}(\key, j))  
    \implies m \in \vi(\key')
\end{array}
\end{equation}
