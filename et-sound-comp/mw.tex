...
\begin{itemize}
    ...
    \item if \( \txid' \) has at least one write, it is easy to see \( \txid' \in \Tx(\mkvs, \vi) \) since \( j \in \vi(\key) \wedge \wtOf(\mkvs(\key', i)) \toEDGE{\SO\rflx} \wtOf(\mkvs(\key, j)) \implies i \in \vi(\key') \).
\end{itemize}
...
\begin{array}{@{}l@{}}
    ...
    \wedge \wtOf(\mkvs_{\cut(\aexec, i-1)}(\key', m)) \toEDGE{\SO\rflx} \wtOf(\mkvs_{\cut(\aexec, i-1)}(\key, j))  
    \implies m \in \vi(\key')
\end{array}
\end{equation}
