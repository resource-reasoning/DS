...
%\fora{\ke, \ke', i, j}
        %i \in \vi(\ke)
        %\wedge \WTx(\mkvs(\ke', j))\toEdge{((\PO \cup \RF_{\mkvs} \cup \VO_{\mkvs}) ; \AD_{\mkvs}?)^{+}} \WTx(\mkvs(\ke, i))
  %\implies j \in \vi(\ke')    
%\]
...
\begin{gather}
    ...
    \begin{array}{@{}l@{}}
        ...
        \land m \in \vi(\ke)  \\
        \quad {} \wedge \WTx(\mkvs(\ke', j)) \toEdge{((\PO \cup \RF_{\mkvs} \cup \VO_{\mkvs}) ; \AD_{\mkvs}?)^{+}} \WTx(\mkvs(\ke, m)) \\
