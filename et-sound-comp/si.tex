...
%\fora{\key, \key', i, j}
        %i \in \vi(\key)
        %\wedge \WTx(\mkvs(\key', j))\toEDGE{((\PO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \AD_{\mkvs}?)^{+}} \WTx(\mkvs(\key, i))
  %\implies j \in \vi(\key')    
%\]
...
\begin{gather}
    ...
    \begin{array}{@{}l@{}}
        ...
        \land m \in \vi(\key)  \\
        \quad {} \wedge \WTx(\mkvs(\key', j)) \toEDGE{((\PO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \AD_{\mkvs}?)^{+}} \WTx(\mkvs(\key, m)) \\
