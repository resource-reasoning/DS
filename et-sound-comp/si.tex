...
\begin{rclarray}
    ...
    I_2(\aexec, \cl) & = & \left( \bigcup_{\Setcon{\txid_{\cl}^{i} \in \txidset_{\aexec} }{ i \in \Nat }} (\SO_{\aexec}^{-1})?(\txid^i_\cl) \right) \setminus \txidset_\rd
\end{rclarray}
\]
...
and \( \left( \bigcup_{\Setcon{\txid_{\cl}^{i} \in \txidset_{\aexec} }{ i \in \Nat }} (\SO_{\aexec}^{-1})?(\txid^i_\cl) \right) \).
...
%\fora{\key, \key', i, j}
        %i \in \vi(\key)
        %\wedge \wtOf(\mkvs(\key', j))\toEDGE{((\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \RW_{\mkvs}?)^{+}} \wtOf(\mkvs(\key, i))
  %\implies j \in \vi(\key')    
%\]
...
\item If \( \txid' \) is a read-only transaction, we know \( \txid' \in \txidset_\rd \) or \( \txid' \in \txidset'_\rd \).
    More specifically we have three cases: \textbf{(i)} \( \txid' \in \bigcup_{\Setcon{\txid_{\cl}^{i} \in \txidset_{\aexec} }{ i \in \Nat }} \VIS_{\aexec}^{-1}(\txid^i_\cl) \), \textbf{(ii)} \( \txid' \in \bigcup_{\Setcon{\txid_{\cl}^{i} \in \txidset_{\aexec} }{ i \in \Nat }} (\SO_{\aexec}^{-1})?(\txid^i_\cl) \) or \textbf{(iii)} \( \txid' \in \txidset'_\rd\).
    \begin{itemize}
    ...
    \item Assume \( \txid' \in \bigcup_{\Setcon{\txid_{\cl}^{i} \in \txidset_{\aexec} }{ i \in \Nat }} (\SO_{\aexec}^{-1})?(\txid^i_\cl) \).
...
\begin{gather}
    ...
    \begin{array}{@{}l@{}}
        ...
        \land m \in \vi(\key)  \\
        \quad {} \wedge \wtOf(\mkvs(\key', j)) \toEDGE{((\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \RW_{\mkvs}?)^{+}} \wtOf(\mkvs(\key, m)) \\
...
%\[
    %\begin{array}{@{}l@{}}
        ...
        %\quad {} \land \txid \toEDGE{\RW_{\aexec}?} \wtOf(\mkvs(\key,i))  \land {} \\
        %\quad \left(
        %\begin{array}{@{}l @{}}
...
%\begin{gather}
    ...
    %\begin{array}{@{}l@{}}
        ...
        %i \in \vi(\key) 
        %\land \wtOf(\mkvs(\key,i)) \in \VIS^{-1}_{\aexec}(\txid_{i})  \\
        %{} \land \txid \toEDGE{(\SO_\aexec \cup (\WR_\aexec ; \SO_\aexec ) \cup \WR_\aexec \cup \WW_\aexec ) ; \RW_{\aexec}?} \wtOf(\mkvs(\key,i))  \\
