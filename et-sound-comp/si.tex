...
%\fora{\key, \key', i, j}
        %i \in \vi(\key)
        %\wedge \WTx(\mkvs(\key', j))\toEDGE{((\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \RW_{\mkvs}?)^{+}} \WTx(\mkvs(\key, i))
  %\implies j \in \vi(\key')    
%\]
...
\begin{gather}
    ...
    \begin{array}{@{}l@{}}
        ...
        \land m \in \vi(\key)  \\
        \quad {} \wedge \WTx(\mkvs(\key', j)) \toEDGE{((\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \RW_{\mkvs}?)^{+}} \WTx(\mkvs(\key, m)) \\
...
%\[
    %\begin{array}{@{}l@{}}
        ...
        %\left( \begin{array}{@{}l@{}}
        %i \in \vi(\key) 
        %\land \WTx(\mkvs(\key,i)) \in \VIS^{-1}_{\aexec}(\txid_{i}) 
        %\land \txid' = \WTx(\mkvs(\key',j)) \\
        %\quad {} \land \txid \toEDGE{\RW_{\aexec}?} \WTx(\mkvs(\key,i))  \land {} \\
        %\quad \left(
        %\begin{array}{@{}l @{}}
...
%\begin{gather}
    ...
    %\begin{array}{@{}l@{}}
        ...
        %i \in \vi(\key) 
        %\land \WTx(\mkvs(\key,i)) \in \VIS^{-1}_{\aexec}(\txid_{i})  \\
        %{} \land \txid \toEDGE{(\SO_\aexec \cup (\WR_\aexec ; \SO_\aexec ) \cup \WR_\aexec \cup \WW_\aexec ) ; \RW_{\aexec}?} \WTx(\mkvs(\key,i))  \\
