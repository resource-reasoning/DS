...
\begin{itemize}
    ...
    %\[
        %\begin{rclarray}
            %\func{RW^{-1}}{\mkvs, \key, i} & \defeq & \Setcon{\txid}{\exsts{ j \leq i } \txid \in \RTx(\mkvs(\key,j))} \\
            %\dagger & \equiv &
            %\begin{array}[t]{@{}l@{}}
                %\fora{\key, \key', i, j, m, \txid, \txid', \txid''} \\
                %\left( \begin{array}{@{}l@{}}
                %i \in \vi(\key) 
                %\land \txid \in \Set{\WTx(\mkvs(\key,i))} \cup \func{RW^{-1}}{\mkvs, \key, i} \land {} \\
                %\quad \left(
                    %\begin{array}{@{}l @{}}
                        %\left( \begin{array}{@{}l@{}}
                                %\txid' \in \func{SO^{-1}}{\txid} \land {} \\
                                %\txid' \in \Set{\WTx(\mkvs(\key',j))} \cup  \RTx(\mkvs(\key',j))
                        %\end{array} \right)  \lor {} \\
                        %\left( \begin{array}{@{}l@{}}
                                %\txid \in \RTx(\mkvs(\key',j)) \land \txid' = \WTx(\mkvs(\key',j))
                        %\end{array} \right)
                        %\end{array} \right) 
                    %\end{array}
                    %\right)  \\
    ...
    %\begin{itemize}
        %\item \( \RW_\aexec\). Assume a key \( \key \),  an index \( i \) and the writer \( \txid  = \WTx(\mkvs(\key,i))\),
    %then \( \txid' \in \RW^{-1}(\mkvs, \key, i)\) if and only if \( \txid' \toEdge{\RW_\aexec} \txid\).
        ...
        %\item  \( \WR_\aexec \). It is easy to see \( \txid \in \RTx(\mkvs(\key',j)) \land \txid' = \WTx(\mkvs(\key',j)) \) if and only if \( \txid' \toEdge{\WR_\aexec} \txid \).
        ...
        %That is, \( \txid' = \WTx(\mkvs(\key,j)) \land \txid = \WTx(\mkvs(\key,i)) \land j < i\) if and only if
        ...
        %\begin{array}{@{}l@{}}
            ...
            %\quad \left(
                %\begin{array}{@{}l @{}}
                    ...
                    %{} \lor 
                    %\left( \begin{array}{@{}l@{}}
                            %\txid \toEdge{\WR_{\aexec'}} \txid'' \land \txid = \WTx(\mkvs(\key',j))
                    %\end{array} \right)
                    %\end{array} \right) 
                %\end{array}
                %\right)  \\
                %{} \lor \left( \begin{array}{@{}l@{}}
                        %\txid \toEdge{\WW_{\aexec'}} \txid'' \land \txid = \WTx(\mkvs(\key',j))
                %\end{array} \right) \\
                %\qquad \implies j \in \vi(\key') 
        %\end{array} 
        %\label{equ:cp-dagger}
    ...
    \begin{itemize}
        ...
        \dagger  \equiv 
            \fora{\key, \key', i, j}
                i \in \vi(\key)  \wedge \WTx(\mkvs(\key', j)) \toEdge{(((\PO \cup \RF_{\mkvs}) ; \AD_{\mkvs}?) \cup \VO_{\mkvs})^{+}} \WTx(\mkvs(\key, i))
            \implies j \in \vi(\key')  
        \end{equation}
...
\dagger  \equiv 
        \fora{\key, \key', m, j}
             m \in \vi(\key)  \wedge \WTx(\mkvs(\key', j)) \toEdge{(((\PO \cup \RF_{\mkvs}) ; \AD_{\mkvs}?) \cup \VO_{\mkvs})^{+}} \WTx(\mkvs(\key, m))
         \implies j \in \vi(\key')  
\]
...
\begin{gather}
    ...
    \begin{array}{@{}l@{}}
        ...
        \quad {} \land \WTx(\mkvs(\key', j)) \toEdge{(((\PO \cup \RF_{\mkvs}) ; \AD_{\mkvs}?) \cup \VO_{\mkvs})^{+}} \WTx(\mkvs(\key, m)) \\
            \qquad \implies \WTx(\mkvs(\key',j)) \in \VIS_\aexec^{-1}(\txid_i)
    \end{array}
\end{gather}
%Note that \( \txid \in \Set{\WTx(\mkvs(\key,i))} \cup \func{RW^{-1}}{\mkvs, \key, i} \) 
%means \( \txid \toEdge{\RW_{\aexec}?} \WTx(\mkvs(\key,i)) \),
%the formulae \(\left( \begin{array}{@{}l@{}} \txid \in \RTx(\mkvs(\key',j)) \land \txid' = \WTx(\mkvs(\key',j)) \end{array} \right) \) 
%means \( \txid \toEdge{\WR_\aexec} \txid' \),
%and \( \left( \begin{array}{@{}l@{}} \txid = \WTx(\mkvs(\key',m)) \land \txid' = \WTx(\mkvs(\key',j)) \land m > j \end{array} \right) \) 
...
%\[
    %\begin{rclarray}
        %\begin{array}[t]{@{}l@{}}
            ...
            %\left( \begin{array}{@{}l@{}}
            %i \in \vi(\key) 
            %\land \WTx(\mkvs(\key,i)) \in \VIS_\aexec^{-1}(\txid_i) \\
            %{} \land \txid' = \WTx(\mkvs(\key',j))
            %\land \txid \toEdge{\RW_{\aexec}?} \WTx(\mkvs(\key,i)) \land {} \\
            %\left(
                %\begin{array}{@{}l @{}}
                ...
                %\end{array}
                %\right)  \\
                %{} \lor \txid' \toEdge{\WW_\aexec} \WTx(\mkvs(\key,i)) \\
...
%\begin{gather}
    ...
    %\begin{rclarray}
        %\begin{array}[t]{@{}l@{}}
            ...
            %i \in \vi(\key) 
            %\land \WTx(\mkvs(\key,i)) \in \VIS_\aexec^{-1}(\txid_i) \\
            %{} \land \txid \toEdge{( (\WR_\aexec; \SO_\aexec) \cup \SO_\aexec \cup \WR_\aexec) ; \RW_{\aexec}? \cup \WW_\aexec} \WTx(\mkvs(\key,i)) 
                %\end{array}
                %\right)  \\
                %\qquad \implies \txid \in \VIS_\aexec^{-1}(\txid_i)
        %\end{array} \\
