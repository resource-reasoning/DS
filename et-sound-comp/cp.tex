...
\begin{itemize}
    ...
    %Given above and 
    %\[ \txid \toEDGE{(\SO_{\aexec'} \cup \WR_{\aexec'} ) ; \RW_{\aexec'}\rflx \cup \WW_{\aexec'}} \txid' \toEDGE{\VIS_{\aexec'}}  \txid_\cl^n \] we can substitute and rewrite the \( \dagger \) as the following:
    %\begin{gather}
        %\begin{array}{@{}l@{}}
            %\fora{\txid'',\key',j}
            %\wtOf(\mkvs(\key,i)) = \txid' \land {} \\
            %\left( \begin{array}{@{}l@{}}
            %\txid'' \toEDGE{\RW_{\aexec'}\rflx} \txid' \land {} \\
            %\quad \left(
                %\begin{array}{@{}l @{}}
                    %\left( \begin{array}{@{}l@{}}
    ...
    %\[
        %\txid \toEDGE{(\SO_{\aexec'} \cup \WR_{\aexec'} )} \txid' \toEDGE{\SO_{\aexec'} ; \RW_{\aexec'}\rflx} \txid'' \toEDGE{\VIS_{\aexec'}}  \txid_\cl^n
    %\]
...
%\[
    %\begin{rclarray}
        %\begin{array}[t]{@{}l@{}}
            ...
            %\land \txid \toEDGE{\RW_{\aexec}\rflx} \wtOf(\mkvs(\key,i)) \land {} \\
            %\left(
                %\begin{array}{@{}l @{}}
...
%\begin{gather}
    ...
    %\begin{rclarray}
        %\begin{array}[t]{@{}l@{}}
            ...
            %i \in \vi(\key) 
            %\land \wtOf(\mkvs(\key,i)) \in \VIS_\aexec^{-1}(\txid_i) \\
            %{} \land \txid \toEDGE{( (\WR_\aexec; \SO_\aexec) \cup \SO_\aexec \cup \WR_\aexec) ; \RW_{\aexec}\rflx \cup \WW_\aexec} \wtOf(\mkvs(\key,i)) 
                %\end{array}
                %\right)  \\
                %\qquad \implies \txid \in \VIS_\aexec^{-1}(\txid_i)
        %\end{array} \\
