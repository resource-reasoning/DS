...
\[
    \begin{array}{@{}l@{}}
        \fora{ \txid' } 
        (\txid' ,\txid)  \in \WR(\aexec',\key) ; (\PO_{\aexec'})? ; \VIS_{\aexec'} 
        \implies \txid' \in \Tx(\mkvs, \vi) 
    \end{array}
\]
Suppose \( (\txid' ,\txid)  \in \WR(\aexec', \key) ; (\PO_{\aexec'})? ; \VIS_{\aexec'} \) for some key \( \key \),
...
\( (\RP_{\LWW}, \Set{\lambda \aexec. \WR(\aexec', \key) ; (\PO_{\aexec'})? ; \VIS_{\aexec'} })\).
...
\[
    ...
    \land \txid' \in \RTx(\hh_{\cut(\aexec, i-1)}(\key', m)) \land \txid' {\xrightarrow{\PO?}} \WTx(\hh_{\cut(\aexec, i-1)}(\key, j)) ) 
    \implies m \in \vi(\key')
    \end{array}
\]
Given a key \( \key \) and an index \( j \) such that \( j \in \vi(\key) \), 
it means that the writer \( \txid \) of the version \( \hh_{\cut(\aexec, i-1)}(\key, j) \) is visible, \ie \( \txid \in \VIS_{\aexec'}^{-1}(\txid_i) \).
Assume some \( \txid' \) such that \( (\txid', \txid) \in (\PO_{\aexec'})? \) and reads a version of some key \( \mkvs(\key',m) \).
