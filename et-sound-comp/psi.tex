...
such that $\ET_{\PSI} \vdash (\mkvs, \vi) \triangleright \fp: (\mkvs',\vi')$. 
...
\begin{gather}
    ...
    \fora{\txid} (\txid, \txid_\cl^n) \in (\WW_{\aexec'} \cup \WR_{\aexec'} \cup \SO ) ; \VIS_{\aexec'} \implies \txid \in \Tx[\mkvs, \vi] \cup \txidset_\rd \cup \txidset'_\rd \label{equ:psi-sound-update-closure}\\
...
\item \cref{equ:psi-sound-update-closure} can be derived from \cref{thm:view-vis-relation}.
