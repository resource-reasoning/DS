\documentclass[runningheads]{llncs}


%%%%%%%%%%%%%%%%%%%%%% INCLUDING %%%%%%%%%%%%%%%%%%%%%%%%%
% to switch including packages in env.tex
\newif\ifNonESOPMode
\NonESOPModefalse
% all packages
\input{env.tex}
% all macros
\usepackage{hhmacros}
% all tikz setting and macros
\input{tikzsetting.tex}
\newcommand{\RootPath}{.}
%%%%%%%%%%%%%%%%%%%%%% END INCLUDING %%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%% EDIT OPTIONS %%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifTechReport
%\TechReporttrue
\TechReportfalse
\newif\ifCommentEdits
\CommentEditstrue
%\CommentEditsfalse
\input{comment-box.tex}
%%%%%%%%%%%%%%%%%%%%%% END EDIT OPTIONS %%%%%%%%%%%%%%%%%%%%%

\begin{document}

%% Title information
\title{
	Data Consistency in Transactional Storage Systems: A Centralised Approach. 
    %Operational Semantics and Logic for Weak Consistency in Transactional Systems%
    %: a Multi-version Based Operational Approach
    } 

\author{%
Shale Xiong\inst{1}
%\orcidID{0000-1111-2222-3333} \and
Andrea Cerone\inst{1}
%\orcidID{1111-2222-3333-4444} \and
Azalea Raad\inst{2}
%\orcidID{0000-0002-2319-3242} \and
Philippa Gardner\inst{1}
%\orcidID{1111-2222-3333-4444} 
}
%
\authorrunning{S. Xiong \and A. Cerone \and A. Raad \and P. Gardner}

%\authorrunning{X. Author et al.}

% First names are abbreviated in the running head.
% If there are more than two authors, 'et al.' is used.
%
\institute{Imperial College London  \\
\email{\{shale.xiong14, a.cerone, p.gardner\}@imperial.ac.uk} \and
Max Planck Institute for Software Systems  (MPI-SWS) \\
\email{azalea@mpi-sws.org}   
}

\maketitle

\begin{abstract}
    \input{\RootPath/abstract.tex}
\end{abstract}

\input{\RootPath/intro.tex}
\input{\RootPath/overview.tex}
\input{\RootPath/model.tex}
\input{\RootPath/semantics/op-semantics.tex}
\input{\RootPath/spec.tex}
\input{\RootPath/relation.tex}
\input{\RootPath/logic.tex}
%\input{example.tex}

\bibliographystyle{splncs04}
\bibliography{bibliography,bibliography2}

\newpage
\appendix
\ifTechReport
% tech report appendix
\else
% appendix for submission
\section{Operational Semantics on Key-value Stores}
\label{sec:full-semantics}
The full operational semantics is given in \cref{fig:full-semantics}.
\input{\RootPath/semantics/semantics-fig.tex}
% composition of kv
\input{\RootPath/composition.tex}
% depenedent graph relates kv
\input{\RootPath/dgraph.tex}
% abstract execution semantics
\input{\RootPath/aexec.tex}
% abstract execution relates kv
\input{\RootPath/kv2aexec.tex}
% adequate between kv and abstract execution
\input{\RootPath/adequate.tex}
\section{Model of the Logic}
\label{sec:model-of-logic}

Before presenting the model, it is safe to weaken snapshots \( \ss \), key-value stores \( \mkvs \) and views \( \vi \) to partial functions,
since we do not model the allocation and deallocation.
We can extend, for example,  a partial key-value store \( \mkvs^{p} \) into total \( \mkvs^{t} \), 
such that \( \mkvs^{p}(\ke) = \mkvs^{t}(\ke) \) for keys \( \ke \in \dom(\mkvs^{p}) \).
We also define the PCM of any set of partial functions \( \mathbb{F}\) as \( (\mathbb{F}, \compose, \unitelem) \) where \( \compose = \uplus \) as the standard disjointed union, and \( \unitelem = \emptyset  \).

\input{\RootPath/logic/transaction.tex}
\input{\RootPath/logic/program.tex}
\input{\RootPath/logic/trans_sound.tex}
\input{\RootPath/logic/prog_sound.tex}
\fi
\input{\RootPath/cops.tex}
\end{document}
