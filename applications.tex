...
\pg{for arbitray E, can't rememebr or find what it's called.} 
...
we give the first robustness proofs that take client sessions into
account.

\pg{In above, is cannot too strong, do you really know that their
  methods cannot be extended? Just be gentler in the words.} 
...
\pg{Below, have I got $(cl, \bullet)$  right, I could not find it
  quickly in paper.}
...
because any local computation step $(cl, \bullet)$ does rely on kv-store,
...
\pg{Assume there is cycle in \( \kvs' \), which must involve in \(
  \txid \). Is this true? I believe it must involve t, but I'm not
  sure if the cycle can't be from ti to ti????? The relation (SO cup
  WR) for K' restricted to K can surely be bigger than the relation SO
  U WR for K???? Haven't had a chance to think too much about this.} 
...
which means \(\ToEdge{ \txid | \RW[\kvs'] -> \txid_1 } \) cannot
happen.
...
under $\WSI$,
\ie \( \conf_0, \prog \toPROG{}_{\ET[\WSI]} (\mkvs, \stub), \stub\), and for all $\txid, \key, i,i'$:
...
\noindent From \cref{thm:serialisable_nocycle} it suffices to prove that $(\SO \cup \WR[\mkvs] \cup \WW[\mkvs] \cup \RW[\mkvs])^{+}$ is acyclic.
