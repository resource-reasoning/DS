...
\end{enumerate*}
It is straightforward to see that the version $j$ read by \( \txid \) in \cref{equ:main-wsi-safe-write-must-read} must be written immediately before the version $i$ written by \( \txid \), \ie \( i {=} j + 1 \).
