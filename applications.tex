...
Pick an arbitrary key $\ke$ and $\mkvs \in \CMs(\ET_{\PSI}, \mathsf{Counter}(\ke))$, 
...
$\for{\txid, i > 0} \txid {=} \WTx(\mkvs(\ke, i)) \implies \txid \in \RTx(\mkvs(\ke, i{-}1))$. 
...
is consistent with the order of the index of such versions in $\mkvs(\ke)$:
%
%
{%
\begin{align*}
    \small
	\mkvs(\ke) & = (0, \txid_{0}, \T^{\mathsf{r}}_{0} \cup \{\txid^{\mathsf{w}}_1\}) 
...
As $\ET_{\SI} \subseteq \ET_{\PSI}$, the list of versions of $\mkvs(\ke_1)$ and $\mkvs(\ke_2)$ 
must both follow the same structure as that described above for a single counter under $\ET_{\PSI}$. 
Recall that this structure embeds a total order $\dashrightarrow$ over transactions in $\mkvs(\ke_1)$ and $\mkvs(\ke_2)$.
...
$\txid \twoheadrightarrow \txid'$ if $\txid \in \{\WTx(\mkvs(\ke_{i}, \_)\} \cup \RTx(\mkvs(\ke_{i}, \_))$ 
and $\txid' \in \{\WTx(\mkvs(\ke_{j}, \_)\} \cup \RTx(\mkvs(\ke_{j}, \_))$ with $i,j \in \Set{1,2} \land j \neq i$,  
