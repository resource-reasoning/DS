...
if the version \( (\txid,\repl) \) is included in the dependency set of the version \( (\txid',\repl')\).
...
and uses the \emph{re-fetch set} to \emph{re-fetch}
...
The COPS trace that preduces \cref{fig:cops-request-values,fig:cops-re-read-values} is depicts in \cref{fig:cops-trace}(top).
...
\item \( \cl \) receives retuned versions of the multi-read transaction (\(\mathtt{e}\)).
\end{enumerate*}
...
interfere the multi-read transaction from \( \cl \).
...
denote disjoint sets of transactions calling $\ctrinc(\key)$ and $\ctrread(\key)$ respectively:\\[5pt]
%
\noindent
\ 
