...
make no assumption on the structure of kv-stores in $\CM(\ET, L)$; rather, we use the definition of $\ET$ to reason 
...
Clients write to $\key$ by calling $\ctrinc(\key)$: 
since $\ET_{\PSI}$ enforces write conflict freedom, we have 
...
%
\noindent where $\{\txid^{\mathsf{w}}_{i}\}_{i=1}^{n}$ and $\bigcup_{i=0}^{n} \txidset^{\mathsf{r}}_{i}$ 
are the set of transactions calling $\ctrinc(\key)$ and $\ctrread(\key)$, respectively.
...
When a client $\cl$ requests to write key $\key$ to replica $r$, it sends a message containing the value to be written and its context to $r$; then  
it waits for a reply from the replica. 
Upon receiving the  message, replica $r$ produces a monotonically increasing time-stamp $t$, which it uses to install a new version for the key $\key$. 
The version carries the value received, and the context received as its set of dependencies: 
the version being written depends on any other version previously observed by the client $\cl$. 
The time-stamp $t$ is sent back to the client, which will incorporate the pair $(\key, t)$ in its local context: 
...
contained in the local state of a replica $r$ is closed with respect to causal dependencies.
...
When a client requests to read a set of keys \( \{\key_1, \dots, \key_n\} \) to replica $r$, it sends a message 
...
(comprising the state of each replica and client) into a configuration in the kv-store framework.
...
% Furthermore, the 
The local context of a (COPS) client identifies a set of dependencies, which are mapped into a set of (kv-store) versions 
in the kv-store built from the state of COPS replicas: the local context of a client determines a view over the kv-store.

Having encoded client-replica states in COPS into configurations, we map  
the execution of a transaction in COPS into an $\ET_{\CC}$ reduction between the encodings of the 
...
More details about the COPS protocol and our soundness proof are given in \cref{sec:cops}.
...
We have proved that Clock-SI is sound with respect to $\ET_\SI$ in \cref{sec:clock-si}.
