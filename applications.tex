...
\mypar{COPS Verification} To prove that COPS satisfies causal consistency,
...
the multi-counter library is robust against \( \SI \).
We show that the multi-counter library satisfies a pattern, named \emph{\( \WSI \)-safe}
which guarantees the robustness against \( \WSI \) 
and hence \( \SI \).
We then use this pattern to show a banking library from \cite{bank-example-wsi} is robust against \( \SI \)
...
We use these two specific acyclicity results in \( \WSI \)-safe.
...
Any \( \WW_{\mkvs} \) edge can be replace by \( \WR_{\mkvs}^{+} ]) \)
...
By \eqref{equ:main-wsi-safe-write-must-read} and \eqref{equ:main-wsi-safe-all-write} we know that 
an \( \RW \) edge starting from a writing transaction can be replaced by a \( \WW \) edge.
Moreover, \( \WW \) edges can be replaced by \( \WR^+ \).
We thus have:
\(
