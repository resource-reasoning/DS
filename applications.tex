...
In this section, we explain the intuition how we verify COPS protocol via trace refinement.
...
COPS is a fully replicated database, with each replica storing multiple versions of each key as depicted in \cref{fig:cops-after-write-transaction}. 
...
We demonstrate how a client \( \cl \) interacts with a replica through the example:
\begin{align}
...
first waits for all dependencies of $\ver$ to be appeared to $\repl'$, and then accept $\ver$.
As such, the set of versions contained in each replica is closed with respect to causal dependencies.
In the example above, when other replicas receive version $\ver$ from $\repl_1$, they can immediately accept $\ver$ as its dependency set is empty. 
...
Upon receiving this message, replica $\repl_1$ builds a causally consistent snapshot (a mapping from $\Set{\key_1, \key_2}$ to values) in two phases as follows. 
...
\begin{enumerate*}
	...
	\item then receives two synchronisation messages from \( \repl_2 \) (shaded), 
...
we show that any COPS traces can be refined to causal dependency traces in our semantics.
First we give a reference operational semantics for COPS.
The abstact machine state consists of the set of replicas, the set of client contexts and a program.
For each trisition, 
a transaction writes or reads a version from a replica, 
or a replica receives a synchronisation message.
A multple-read transaction has multple read operations in the trace, sepearted into two phases,
and interleaving may happen bewteen two reads.
We first move any re-fetch opertaion from the second phase 
to the right (delaying the read) 
until it is adjacent by another re-fecth operation,
without changing the result.
This is because the second phase cannot read any versions 
that are committed after the end of first phase.
We only need to show that the second phase is atomic, 
since the first phase is opaque to the client.
This yields a \emph{normal COPS trace}
where transactions are executed atommically.


we then show that normal COPS traces can be refined to causal consistency traces in our semantics.
First we encode the state of the COPS machine state  
as a configuration in our operational semantics. 
As each replica stores the replications of versions, 
we can project the state of COPS replicas into a kv-store 
by mapping all the replcations into one verion in a kv-store.
...
We next map a normal COPS trace to a kv-store trace:
each tansition in the kv-store trace corresponds 
to a write operation or the sequence of re-fecth operations. 
Last, we show every step in the kv-store trace satisfies \( \et[\CC] \).
...
We refer the reader to \cref{sec:cops} for all the details.
