...
We define the \( \DEP \) relation: \( \ToEdge{ (\txid,\repl) | \DEP -> (\txid',\repl') } \),
if the version \( (\txid,\repl) \) is included in the dependency set of \( (\txid',\repl')\).
COPS assumes a total order over replica identifiers. As such, versions can be
totally ordered lexicographically over their time-stamps. 
...
to \emph{re-fetch}, for each key \( \key \), the most recent version with the biggest time-stamp between the versions of \( \key \)
...
\citet{cops} informally argue that the snapshot sent back to the client are causally consistent in their specific setting.
...
and show that COPS traces can be refined to traces in our semantics using the execution test for causal consistency \( \ET[\CC] \) in three steps:
\begin{enumerate*}
\item every COPS trace can be transferred to normlaised COPS traces, 
in which there is no inteleaving for the reads from a transaction; and
\item the normlaised COPS traces can be refined to traces in our semantics, in which \item each step satisfies \( \ET[\CC] \).
\end{enumerate*}

The operational semantics for COPS 
describes transitions over abstract states \( \copsconf \) comprising a set of replicas,
a set of client contexts and a program.
The COPS trace that repreduces \cref{fig:cops-request-values,fig:cops-re-read-values} is depicts in \cref{fig:cops-trace}.
A transition in the semantic describes that:
a client (\(\cl\)) of a replica (\( \repl_1 \))
\begin{enumerate*}
\item writes a version to the replica \( (\opW,(\txid_1,\repl_1)) \);
\item reads a version from the replica \( (\opR,(\txid_1,\repl_1)) \);
\item or a replica receives synchronisation messages (\(\mathtt{sync}\)).
We also label \item the start (\( \mathtt{s} \)), \item the end (\( \mathtt{e} \)) and 
\item the phase change (\( \mathtt{p} \)) of any read-only transaction.
\end{enumerate*}
%Recall that COPS supports reading from multiple versions atomically. 
Recall that a multi-read transaction does not excuted atomically in the replca,
which is captured by multiple read transitions in the trace.
These read transitions are divided into two phases:
those of optimistic reads between \( \mathtt{s} \)  and \( \mathtt{p} \),
and those of re-fetches between \( \mathtt{p} \)  and \( \mathtt{e} \).
...
by moving all re-fetches for a transaction 
towards the end label \( \mathtt{e}\) for the transaction
so that the re-fetch phase are no longer interleaved by others.
Since the re-fetch phase cannot read a version committed after the phase change \( \mathtt{p}\),
normalising traces does not alter the returned versions to the client. 
Single-write and multi-read transactions in the resulting normalised trace are executed atomically. 


We next show that normalised COPS traces can be refined to traces in our semantics.
To do this, we encode the abstract COPS states \(\copsconf \) as a configuration
in our operational semantics (\cref{sec:model}). 
...
For example, the context of \( \cl \) after committing its first single-write transaction in \( \prog_{\CodeFont{COPS}} \)
is encoded as the client view depicted in \cref{fig:encode-view}.

\input{cops-encoded.tex}

we then show that every step in the kv-store trace satisfies \( \et[\CC] \).
...
In particular, we check the client views over the kv-store as follows.
...
Using our operational semantics, 
we are able to prove invariant properties of kv-stores, such as:
\begin{enumerate*}
...
\item the correctness of the standard lock paradigm under \( \UA \), although it is not robust.
\end{enumerate*}
...
we know that if a transaction $\txid$ updates $\key$ (by calling $\ctrinc(\key)$) and writes version $\ver$ to $\key$, then it must have read the version of $\key$ immediately preceding $\ver$:
...
As such, the invariant depicted and defined below, where  
...
\coordinate  (I) at ([xshift=17,yshift=0] H.center);
\coordinate  (J) at ([xshift=2,yshift=-17] I.center);
...
yield kv-stores that adhere to a particular pattern that guarantees robustness against \( \WSI \):
...
\begin{enumerate*} 
    ...
    \item if \( \txid \) writes to $\key$, then it must also read from it \eqref{equ:main-wsi-safe-write-must-read}, a property we call \emph{strictly-no-blind writes}; and
...
Since \( \mkvs \) is reachable under \( \WSI \) and thus \( \CC \):% also reachable under \( \CC \),
...
\noindent
This lock paradigm \eqref{equ:lock-program} is correct if executed under serialisability,
