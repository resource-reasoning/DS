...
To construct the correct dependency sets for new versions,
...
\]
In contrast, COPS uses the $\depOf[\ver]$ to ensure a multiple-read transaction fetches causally consistent versions,
that is, if a transaction reads a version \( \ver \) that depends on another version \( \ver' \in \depOf[\ver]\), 
...
A client view \( \vi =  \func{viewOf}[ctx] \) contains the context \( ctx \) and versions \( ctx \) depends on:
\[
