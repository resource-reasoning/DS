...
$\txid \twoheadrightarrow \txid'$ iff $\txid \xrightarrow{\SO} \txid' \wedge \left(\exists i,j \in \{1,2\}. i \neq j \wedge \txid \in \Set{\wtOf(\mkvs(\key_{i}, \stub)} \cup \rsOf(\mkvs(\key_{i}, \stub)) 
\wedge \txid' \in \Set{\wtOf(\mkvs(\key_{j}, \stub))} \cup \rsOf(\mkvs(\key_{j}, \stub)) \right)$.
...
The state of the system (comprising the state of each replica and of each client) can be encoded into 
...
in the kv-store built from the state of COPS replicas: that is, the local context of a client uniquely determines a view over the kv-store.
