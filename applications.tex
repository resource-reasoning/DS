...
This guarantees that for any version \( \ver \) in a replica \( \repl \), that is \( \ver \in \repl \),
the versions that  \( \ver \) depends on also are in the replica, \( \depOf[\ver] \subseteq \repl \).
...
A client can read some keys \( \key_1, \dots, \key_n \) in a multiple-read transaction through requesting a replica; the replica reads the most recent version \( \ver_{\key_i} \) per key \( \key_i \) (\( 0 < i \leq n\)) one by one, between which interleaving can happen, that is, other writes might arrive.
...
and a client context can be encoded to a COPS client view, that is, a version is contained in the client context, if and only if it is contained in the view.
