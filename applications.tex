...
\begin{definition}[\(\WSI\)-safe]
    ...
    that is, \( \ET_\WSI \vdash \conf_0, \prog \toPROG{} (\mkvs, \vienv), \prog' \),
...
The \cref{def:main-body-wsi-safe} captures that:
\begin{enumerate*} 
...
the version read by \( \txid \) must be the immediate predecessor of the one wrote by \( \txid \),
that is, \( i = j + 1 \).
