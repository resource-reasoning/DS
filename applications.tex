...
\{
    ...
    \right)
\}$
to a strict total order (that is,  an irreflexive and transitive
relation that is total). 
Note that $\dashrightarrow$ contains $\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs} \cup \RW_{\mkvs}$ and thus
$(\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs} \cup \RW_{\mkvs})^+$ is
irreflexive:
that is,  $\Counter(\key)$ is robust against $\PSI$.
In contrast, a multi-counter library is not robust against $\PSI$. A
mulit-counter library 
...
can be seen as invariant properties:  any reachable kv-store \( \kvs\) under \( \et[\top] \) does not contains cycle of the relation \( (\SO {\cup} \WR_{\mkvs}) \),
where \( \et[\top] \) is the most permissive execution test defined in
\cref{def:cm}; and 
any reachable kv-store \( \kvs\) under \( \et[\CP] \) does not contains cycle of the relation \( ((\SO {\cup} \WR_{\mkvs}); \RW_{\mkvs}^{-1})\).                  
...
consider 
\( \ToProg{\kvs | \vienv | \clenv | \prog | \lbTrans{\vi,\fp} -> \kvs' | \vienv' | \clenv' | \prog' } \),
where \( \kvs \) contains no cycle in  the relation \( ((\SO {\cup} \WR_{\mkvs}) ; \RW_{\kvs}^? )^{+} \) 
...
which means \(\ToEdge{ \txid | \RW_{\kvs'} -> \txid_1 } \) cannot
happen.
...
\noindent From \cref{thm:serialisable_nocycle} it suffices to prove
that the relation $(\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs} \cup \RW_{\mkvs})^{+}$ is acyclic.
...
By \eqref{equ:main-wsi-safe-write-must-read} and \( \UA \),
