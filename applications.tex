...
\end{enumerate*}
We have thus demonstrated that COPS satisfies causal consistency (the full detail is in \cref{sec:cops}).
...
\item the robustness of a multi-counter library and the well-known  banking library~\citet{bank-example-wsi} against \( \SI \)w; and
\item the correctness of a program pattern using locks against \( \UA \), even though it is not robust.
\end{enumerate*}
We prove item~2  by identifying a general  transaction pattern that
...
As such, the invariant illustrated below always holds, where  
...
\{
    ...
    \right)
\}$
to a strict total order (that is,  an irreflexive and transitive relation that is total). 
...
In contrast, a multi-counter library is not robust against $\PSI$. A mulit-counter library 
...
using which we can prove a general robustness result
against WSI for libraries with transactions satisfying this pattern. 
The proof of this general robustness result  uses the following two 
acyclicproperties of kv-stores. 



\mypar{Acyclic results on kv-stores}
We prove two general acyclic properties on kv-stores, which
can be seen as invariant properties:  any reachable kv-store \( \kvs\) under \( \et[\top] \) does not contains cycle of the relation \( (\SO {\cup} \WR[\mkvs]) \),
where \( \et[\top] \) is the most permissive execution test defined in
\cref{def:cm}; and 
any reachable kv-store \( \kvs\) under \( \et[\CP] \) does not contains cycle of the relation \( ((\SO {\cup} \WR[\mkvs]); \RW[\mkvs]^{-1})\).                  
...
let us focus on the case where the last tansaction step has the form:
\( \ToProg{\kvs | \vienv | \clenv | \prog | \lbTrans{\vi,\fp} -> \kvs' | \vienv' | \clenv' | \prog' } \),
where \( \kvs \) contains no cycle of the relation \( (\SO {\cup} \WR[\mkvs]) \) by the inductive hypothesis.
We prove \( \kvs' \) also has no cycle by contradiction.
Let \( \txid \) be the new transaction in \( \kvs' \).
Assume there is cycle in \( \kvs' \), which must involve  \( \txid \):
that is:
\(
\ToEdge{\txid | \rel -> \txid_1 | \rel -> \cdots | \rel -> \txid_n | \rel -> \txid}, 
\)
where \( \rel = \SO {\cup} \WR[\mkvs'] \), and \( \txid_1 \) to \( \txid_n \) are distinct transactions.
Consider \( \ToEdge{\txid | \SO {\cup} \WR[\mkvs'] -> \txid_1} \).
...
Thus, it cannot be \( \ToEdge{\txid | \WR[\mkvs'] -> \txid_1} \).
...
\Cref{thm:acyclic-cp} can be proven in the similar way to \cref{thm:acyclic-cc}.
For inductive case, consider 
\( \ToProg{\kvs | \vienv | \clenv | \prog | \lbTrans{\vi,\fp} -> \kvs' | \vienv' | \clenv' | \prog' } \),
where \( \kvs \) contains no cycle in the relation \( ((\SO {\cup} \WR[\mkvs]) ; \RW[\kvs]^? )^{+} \) 
by the inductive hypothesis.
Assume there is a cycle in \( \kvs' \), which must include the new transaction \( \txid \).
There are two possible cycles.
\begin{enumerate}
\item This first one is of the form
\(
\ToEdge{\txid | \rel -> \txid_1 | \rel -> \cdots | \rel -> \txid_n | \rel -> \txid}, 
\)
where \( \rel = (\SO {\cup} \WR[\mkvs']) ; \RW[\kvs']^?  \), and \( \txid_1 \) to \( \txid_n \) are distinct transactions.
This cycle cannot exist because \( \txid \) is the last transaction in \( \kvs' \).
Similar to  \cref{thm:acyclic-cc},
it cannot have \( \ToEdge{\txid | \SO -> \txid_1 } \) or \( \ToEdge{\txid | \WR[\kvs'] -> \txid_1 } \).
Also it cannot have
it cannot have \( \ToEdge{\txid | \SO -> \txid' | \RW[\kvs'] -> \txid_1 } \) or \( \ToEdge{\txid | \WR[\kvs'] -> \txid' | \RW[\kvs'] -> \txid_1 } \),
for some transaction \( \txid' \in \kvs \).

\item The second one is of the form
\(
\ToEdge{ \txid_1 | \rel -> \cdots | \rel -> \txid_n | (\SO {\cup} \WR[\mkvs']) -> \txid | \RW[\kvs'] -> \txid_1} .
\)
However by \( \et[\CP] \), the view \( \vi \) of \( \txid \) must contains all versions written by \( \txid_1 \) to \( \txid_n \).
This means \(\ToEdge{ \txid | \RW[\kvs'] -> \txid_1 } \) cannot happen,
because by \( \RW[\kvs'] \) the view \( \vi \) is behind versions written by \( \txid_1 \).
\end{enumerate}
...
yield kv-stores that have a particular pattern that guarantees 
robustness against \( \WSI \).
These examples are originally believed to be robust against \( \SI \), 
by individually checking all final results of each program of the libraries.
In contrast, we prove the robustness by showing the invairant satisfies a pattern step by step.
...
if any reachable kv-store \( \kvs \) by executing a program \( \prog \) of the library under $\WSI$,
\ie \( \ToProg[{\ET[\WSI]}]{\kvs_0 | \vienv_0 | \clenv | \prog | \stub | * -> \kvs | \stub | \stub | \stub }\),
satisfies the following conditions: for all $\txid, \key, i,i'$,
...
\end{enumerate*}
It is straightforward to see that, by UA,  the version $j$ read by \(
...
\begin{theorem}[\( \WSI \) robustness]
 \label{thm:main-wsi-robust}
    Any \(\WSI\)-safe library \( L \) is robust against \(\WSI\).   
\end{theorem}
\SpaceBelowDef
...
\noindent
Assume a program \( \prog \) of the library \( L \).
Assume a reachable kv-store \( \kvs \) by executing the program \( \prog \) under $\WSI$,
\ie \( \ToProg[{\ET[\WSI]}]{\kvs_0 | \vienv_0 | \clenv | \prog | \stub | * -> \kvs | \stub | \stub | \stub }\).
From \cref{thm:serialisable_nocycle} it suffices to prove that $(\SO \cup \WR[\mkvs] \cup \WW[\mkvs] \cup \RW[\mkvs])^{+}$ is acyclic.
We proceed by contradiction.
Assume that there exists $\txid_1$ such that $\txid_1 \toEDGE{(\SO \cup \WR[\mkvs] \cup \WW[\mkvs] \cup \RW[\mkvs])^{+}} \txid_1$. 
By \cref{thm:acyclic-cc}, \( (\SO \cup \WR[\mkvs])^{+} \) is acyclic.
Because of no no-blind-writes in \eqref{equ:main-wsi-safe-write-must-read} and \( \UA \),
any \( \WW[\mkvs](\key)\) edge on a key \( \key \) can be replace by \( \WR[\mkvs]^{+}(\key) ) \), 
illustrated by \cref{fig:wr-replace-ww}.
Therefore, \( (\SO \cup \WR[\mkvs])^{+} \cup \WW[\kvs]\) is acyclic.
...
Moreover, we can replace any new \( \WW \) edges by \( \WR^+ \).
We thus have
\(
...
This leads to a contradiction using \cref{thm:acyclic-cp}.
...
we are able to prove the robustness of the banking library in
...
\begin{enumerate*}
\item they identified an notion of dangerous dependency between transactions,
which, they argued, can lead to violation of robustness of \( \SI \); and
\item they argued this banking example contains no dangerous dependency.
\end{enumerate*}
...
The saving account must be non-negative but checking account can be negative.
...
%
In \citet{bank-example-wsi}, they argue that  to make the banking library robust against \( \SI \),
...
\pg{Above: now this can be strengthened. It should be pointed out that
  ...
  library is not safe, maybe transfering an example they discovered in
...
not robust against \( \UA \), we can still prove an invairant property that
states that only one client can hold the lock at any time. 
The distributed lock library has the operations \(\plock(k)\), \(\ptrylock(k)\) and \(\punlock(k)\) 
on the key \( \key \):
...
If the value  is zero, that is  the lock is available, 
...
Consider the program pattern \( \prog_{\CodeFont{LK}} \) where clients \( \cl\) and \( \cl'\) compete the lock \( \key \):
...
However, \( \prog_{\CodeFont{LK}} \) is not robust under \( \UA \) and \( \PSI \), 
...
Nevertheless, we show that \( \prog_{\CodeFont{LK}} \) is still correct under \( \UA \) and hence \( \PSI \).
We capture the correctness in the sense that 
only one client can hold the lock at a time,
by the following invariant: for all \( i \), if  \( i > 0\), then:
...
Assume a kv-store \( \kvs \) satisfies the conditions.
Given the lock pattern given by \( \prog_{\CodeFont{LK}} \),
if the lastest value of \( \key \) is 0, 
then all clients are competing the lock, and due to \( \UA \)
only a client \( \cl \) with full view of \( \key \)
can install a new version with value being its unique client ID.
This will stop other client to acquire the lock, 
because the lastest version is non-zero.
Now the next transaction of client \( \cl \) is a \( \punlock(\key) \),
which releases the lock, adding a version with value zero.
