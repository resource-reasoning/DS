\section{Applications: Robustness}
...
and saving and checking table map customer IDs to saving balances (\( \codeFont{Saving(\underline{CustomerID}, Balance )} \)) 
...
\begin{align*}
    ...
    \end{transaction}
\end{align*}
\( \codeFont{balance(n)} \) returns, through the spatial variable \( \ret \), total balance of customer \( n \).
...
read and write to any keys they touch, thus ensures strict no blind write.
