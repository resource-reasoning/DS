...
$\ke$ such that either $\txid = \WTx(\mkvs(\ke, i))$, or $\txid \in \RTx(\mkvs(\ke,i))$; furthermore, 
$(\otW, \ke, \val) \in \TtoOp{T}(\txid)$ if and only 
if $\txid = \WTx(\mkvs(\ke, i))$ for some $i$, and 
$(\otR, \ke, \val) \in \TtoOp{T}(\txid)$ if and only if $\txid \in \RTx(\mkvs(\ke, i))$ for some $i$, 
\item $\txid \xrightarrow{\RF(\ke)} \txid'$ if and only if there exists an index $i: 0 < i < \lvert \mkvs(\ke) \rvert$ 
such that $\txid = \WTx(\mkvs(\ke, i))$, and $\txid' \in \RTx(\mkvs(\ke, i))$, 
\item $\txid \xrightarrow{\VO(\ke)} \txid'$ if and only if there exist two indexes $i,j$: $0 < i < j < \lvert \mkvs(\ke) \rvert$ 
such that $\txid = \WTx(\ke, i)$, $\txid' = \WTx(\ke, j)$, 
\item $\txid \xrightarrow{\AD(\ke)} \txid'$ if and only if there exist two indexes $i,j$: $0 < i < j < \lvert \mkvs(\ke) \rvert$ 
...
\begin{itemize}
\item Let $\txid \in \dom(\TtoOp{T}_{\mkvs})$, and suppose that $(\otR, \ke, \val) \in \TtoOp{T}_{\mkvs}(\txid)$. 
We need to prove that either $\val = \val_{0}$, and there exists no $\txid' \in \dom(\TtoOp{T}_{\mkvs})$ such that 
$\txid' \xrightarrow{\RF_{\mkvs}(\ke)} \txid$, or $\txid' \xrightarrow{\RF_{\mkvs}(\ke)} \txid$ for some 
$\txid' \in \dom(\TtoOp{T}_{\mkvs})$ such that $(\otW, \ke, \val) \in \TtoOp{T}_{\mkvs}(\txid')$. 
...
$\txid \neq \txid_0$. Furthermore, because $(\otR, \ke, \val) \in \TtoOp{T}_{\mkvs}(\txid)$, there 
must exist an index $i: 0 \leq i < \lvert \mkvs(\ke) \rvert$ such that $\mkvs(\ke, i) = (\val, \txid', \Set{\txid } \cup \_ )$ 
for some $\txid' \in \TxID$. 
We have two possibilities: 
\begin{enumerate}
\item $i = 0$, in which case the hypothesis that $\mkvs$ is well-formed ensures that $\txid' = \txid_0$, 
and $\val = \val_0$. We also have that there exists no transaction $\txid''$ such that $\txid'' \xrightarrow{\RF_{\mkvs}(\ke)} \txid$: 
in fact, by \cref{def:kv2graph}, we have that $\txid'' \xrightarrow{\RF(\ke)} \txid$ if and only if there exists an index 
$j: 0 < j < \lvert \mkvs(\ke) \rvert$ such that $\mkvs(\ke, j) = (\_, \txid'', \Set{\txid} \cup \_)$. However, in this case we would 
have that $0 < j$, and $\txid \in \RTx(\mkvs(\ke, j)) \cap \RTx(\mkvs(\ke, 0))$, contradicting the constraint placed 
over well-formed kv-stores that a transaction never reads multiple versions for a key. Therefore, there exists 
no transaction $\txid''$ such that $\txid'' \xrightarrow{\RF_{\mkvs}(\ke)} \txid$, 
\item $i > 0$; in this case \cref{def:kv2graph} ensures that $\txid' \xrightarrow{\RF_{\mkvs}(\ke)} \txid$; also, 
because $\mkvs(\ke, i) = (\val, \txid', \_)$, it must be the case that $(\otW, \ke, \val) \in \TtoOp{T}_{\mkvs}(\txid')$.
\end{enumerate}
\item Let $\txid \in \dom(\TtoOp{T}_{\mkvs})$, and suppose that there exist $\txid_1, \txid_2$ such that 
$\txid_{1} \xrightarrow{\RF_{\ke}(\mkvs)} \txid$, $\txid_{2} \xrightarrow{\RF_{\ke}(\mkvs)} \txid$. 
By \cref{def:kv2graph}, there exist two indexes $i, j: 0 < i, j < \lvert \mkvs(\ke) \rvert$, such that 
$\mkvs(\ke, i) = (\_, \txid_1, \Set{\txid} \cup \_)$, $\mkvs(\ke, j) = (\_, \txid_2, \Set{\txid} \cup \_)$. 
We have that $\txid \in \RTx(\mkvs(\ke, i)) \cap \RTx(\mkvs(\ke, j))$, i.e. 
...
$\txid_{\cl}^{n} \xrightarrow{\RF_{\mkvs}(\ke)} \txid_{\cl}^{m}$.  We prove that 
$n < m$. By \cref{def:kv2graph}, it must be the case that 
there exists an index $i : 0 \leq i < \lvert \mkvs(\ke) \rvert$ such that $\mkvs(\ke, i) = 
...
We prove this fact by contradiction: suppose that $\txid \xrightarrow{\VO_{\mkvs}(\ke)} \txid$ for some key $\ke$. By \cref{def:kv2graph}, 
there must exist two indexes $i,j: 0 < i < j < \lvert \mkvs(\ke) \rvert$ such that $\txid = \WTx(\mkvs(\ke,i))$ and 
$\txid = \WTx(\mkvs(\ke, j))$. Because we are assuming that $\mkvs$ is well-formed, then it must be the 
case that $i = j$, contradicting the statement that $i < j$. 
\item Let $\txid, \txid'$ be such that $\txid' \xrightarrow{\VO_{\ke}(\mkvs)} \txid$. 
We must show that  $(\otW, \ke, \_) \in \TtoOp{T}_{\mkvs}(\txid')$, and $(\otW, \ke, \_) \in \TtoOp{T}_{\mkvs}(\txid)$.
By \cref{def:kv2graph}, there exist $i, j: 0 < i,j < \lvert \mkvs(\ke) \rvert$ such that 
$\mkvs(\ke, i) = (\val', \txid', \_)$ and $\mkvs(\ke, j) = (\val, \txid, \_)$, for some 
$\val, \val' \in \Val$. \cref{def:kv2graph} also ensures that $(\otW, \ke, \val') \in 
\TtoOp{T}_{\mkvs}(\txid')$, and $(\otW, \ke, \val) \in \TtoOp{T}_{\mkvs}(\txid)$.
\item Let $\txid, \txid'$ be such that $(\otW, \ke, \_) \in \TtoOp{T}_{\mkvs}(\txid)$ 
and $(\otW, \ke, \_) \in \TtoOp{T}_{\mkvs}(\txid')$. We need to prove that 
either $\txid = \txid', \txid \xrightarrow{\VO_{\mkvs}(\ke)} \txid'$, or $\txid' \xrightarrow{\VO_{\mkvs}(\ke)} \txid$. 
By \cref{def:kv2graph} there exist two indexes $i, j: 0 < i,j< \lvert \mkvs(\ke) \rvert$ such that 
$\mkvs(\ke, i) = (\_, \txid, \_)$ and $\mkvs(\ke, j) = (\_, \txid', \_)$. If $i = j$, then $\txid = \txid'$ 
and there is nothing left to prove. Otherwise, suppose without loss of generality that 
$i < j$. Then \cref{def:kv2graph} ensures that $\txid \xrightarrow{\VO_{\mkvs}(\ke)} \txid'$. 
\item Suppose that $\txid_{\cl}^{m} \xrightarrow{\VO_{\mkvs}(\ke)} \txid_{\cl}^{n}$ for 
some $\cl \in \Clients$ and $m, n \in \Nat$. We need to show that $m < n$. 
By \cref{def:kv2graph}, because  $\txid_{\cl}^{m} \xrightarrow{\VO_{\mkvs}(\ke)} \txid_{\cl}^{n}$ 
there exist two indexes $i,j: 0 < i,j < \lvert \mkvs(\ke) \rvert$ such that $\WTx(\mkvs(\ke,i)) = \txid_{\cl}^{m}$ 
and $\WTx(\mkvs(\ke, j)) = \txid_{\cl}^{n}$. From the assumption that $\mkvs$ is well-formed, it 
follows that $n < m$.
\end{itemize}
\end{proof}
...
\[
\begin{array}{l}
\fora{ \ke \in \dom(\mkvs), \cl \in \Clients} \fora{ i,j; 0 \leq i < j < \lvert \hh_{\Gr}(\ke) \rvert}
\fora{ n, m \geq 0}\\
...
It is sufficient to fix a key \( \ke \) and to prove \( \hh_{\Gr_{\mkvs}}(\ke) = \mkvs(\ke) \).
We prove \( \hh_{\Gr_{\mkvs}}(\ke) = \mkvs(\ke) \) by induction on the length of \( \mkvs(\ke) \).

\begin{itemize}
    \item \caseB{\abs{\mkvs(\ke)} = 1}
Let \( \mkvs(\ke) = (\val_0, \txid_0, \txidset_0 ) \) for some transactions \( \txidset_0 \) that read the initial value \( \val_0 \).
Given the definition of \( \Gr_\mkvs \) (\cref{def:kv2graph}), we know that \( (\otR, \ke, \val_0 ) \in \TtoOp{T}(\txid) \) for all \( \txid \in \txidset_0 \) and \( \VO(\ke) = \RF(\ke) = \AD(\ke) = \emptyset  \).
Given the definition of \( \hh_{\Gr_\mkvs}\) (\cref{def:dependency-to-kv-store}), it is easy to see \( \hh_{\Gr_\mkvs}(\ke) = \mkvs(\ke) \).

    \item \caseI{\abs{\mkvs(\ke)}= m + 1 }
Suppose \( \hh_{\Gr_\mkvs}(\ke) = \mkvs(\ke) \) when \( |\mkvs(\ke)| = m \) and let consider  \( |\mkvs(\ke)| = m + 1 \).
Let 
\[
    \mkvs(\ke) = \List{(\val_0, \txid_0, \txidset_0 ), \dots, (\val_m, \txid_m, \txidset_m ), (\val_{m+1}, \txid_{m+1}, \txidset_{m+1} ) } 
\]
We now discuss the \( \VO(\ke) \), \( \RF(\ke) \) and \( \AD(\ke) \) relations in \( \Gr_\mkvs(\ke) \) and the corresponding versions in \( \hh_{\Gr_\mkvs}(\ke) \).
\begin{itemize}
    ...
    By the \ih, we have \( \WTx(\hh_{\Gr_\mkvs}(\ke,i))  = \txid_i \) and \( \WTx(\hh_{\Gr_\mkvs}(\ke,i))  = \txid_j \).
    If \( \txid' = \txid_{m + 1} \), then \( \txid = \txid_i \) for some \( i \) such that \( 0 \leq i < m + 1 \).
    By the definition of  \( \hh_{\Gr_\mkvs}(\ke) \) (\cref{def:dependency-to-kv-store}), the order of versions is the same as the order of \( \AD(\ke) \).
    That means the version \( (\val_{m+1}, \txid_{m+1}, \stub ) \) is the last one, \ie (m + 1)-\emph{th}, in the \( \hh_{\Gr_\mkvs}(\ke) \).
    Combine the two cases above, we know:
    \begin{equation}
        \label{equ:ww-back-to-ww}
        \fora{i : 0 \leq i \leq m + 1} \exsts{\txidset} \hh_{\Gr_\mkvs}(\ke, i) = (\val_i, \txid_i, \txidset)
    \end{equation}
    ...
    By the definition of  \( \hh_{\Gr_\mkvs}(\ke) \) and \cref{equ:ww-back-to-ww}, it follows:
    \begin{equation}
        \label{equ:k-to-kgk}
        \fora{i : 0 \leq i \leq m + 1} \hh_{\Gr_\mkvs}(\ke, i) = (\val_i, \txid_i, \txidset_i)
    \end{equation}
\end{itemize}
\end{itemize}
The \cref{equ:k-to-kgk} implies \( \mkvs(\ke) = \hh_{\Gr_\mkvs}(\ke) \) and then \( \mkvs = \hh_{\Gr_\mkvs} \).
\end{proof}
