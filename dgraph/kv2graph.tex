...
\begin{definition}
\label{def:dgraph}
A \emph{dependency graph} is a quadruple $\Gr = (\TtoOp{T}, \WR, \WW, \AD)$, where
\begin{itemize}
\item 
...
Given a dependency graph $\Gr = (\TtoOp{T}, \WR, \WW, \AD)$, we often 
commit an abuse of notation and use $\WR$ to denote the relation 
$\bigcup_{\key \in \Keys} \WR(\key)$; a similar notation is adopted for $\WW, \AD$. 
...
\WW_{\mkvs}, \AD_{\mkvs})$ is defined as follows: 
\begin{itemize}
...
Given a dependency graph $\Gr = (\TtoOp{T}, \WR, \WW, \AD)$, we define the kv-store $\mkvs_{\Gr}$ as follows: 
\begin{enumerate}
...
$\Gr_{\mkvs} = (\TtoOp{T}_{\mkvs}, \WR_{\mkvs}, \WW_{\mkvs}, \AD_{\mkvs})$ is a dependency graph. 
...
For any dependency graph $\Gr = (\TtoOp{T}, \WW, \WR, \AD)$, $\mkvs_{\Gr}$ is a well-formed kv-store.
\end{proposition}
...
Given the definition of \( \Gr_\mkvs \) (\cref{def:kv2graph}), we know that \( (\otR, \key, \val_0 ) \in \TtoOp{T}(\txid) \) for all \( \txid \in \txidset_0 \) and \( \WW(\key) = \WR(\key) = \AD(\key) = \emptyset  \).
...
\]
We now discuss the \( \WW(\key) \), \( \WR(\key) \) and \( \AD(\key) \) relations in \( \Gr_\mkvs(\key) \) and the corresponding versions in \( \mkvs_{\Gr_\mkvs}(\key) \).
\begin{itemize}
    ...
    By the definition of  \( \mkvs_{\Gr_\mkvs}(\key) \) (\cref{def:dependency-to-kv-store}), the order of versions is the same as the order of \( \AD(\key) \).
