...
and such that for any $i, j: 1 \leq i < j \leq n$, $\WTx(\ver_{\key}^{i}) \toEDGE{\WW(\key)} \WTx(\ver_{\key}^{j})$. 
...
Because $i < j$, it must be the case that $\txid_{\cl}^{n} = \WTx(\ver(\txid_{\cl}^{n}, \key) \toEDGE{\WW(\key)} 
