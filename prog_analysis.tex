...
We only consider 
the case where the key-value store consists of two counters $\mathsf{Counter}(\ke_1)$ and $\mathsf{Counter}(\ke_2)$, 
although our line of reasoning can be generalised to an arbitrary number of counters. 
Therefore, let $\mathsf{Counter} = \mathsf{Counter}(\ke_1) \cup \mathsf{Counter}(\ke_2)$. 

Let also $\hh$ be in $\mathsf{KVStores}(\ET_{\SI}, \mathsf{Counter})$. Because 
...
Let $\hh$ be in $\mathsf{KVStores}(\ET_{\PSI}, \mathsf{Counter})$. 
...
follows from the fact that each of the operations in $\mathsf{Counter}$ only access a single key. 
\end{proof}
...
$\hh \in \mathsf{KVStores}(\ET_{\SI}, \mathsf{Counter})$, and show that such a 
...
Let $\hh \in \mathsf{KVStores}(\ET_{\SI}, \mathsf{Counter})$, and let 
...
let $\rightarrow$ be the smallest relation such that 
\[
\begin{array}{ll}
\txid \in \T^{\ke_{h}}_{i} &\implies \txid^{\ke_{h}}_{i} \rightarrow \txid,\\
i < j &\implies \txid_{i} \rightarrow \txid_{j},\\
\txid \in \T^{\ke_{h}}_{i} \wedge i < j & \implies \txid \rightarrow \txid^{\ke_{h}}_{j}\\
\txid, \txid' \in \T^{\ke_{h}}_{i} \wedge \txid \xrightarrow{\PO} \txid' &\implies \txid \rightarrow \txid'\\
\txid \in (\T^{\ke_{h}}_{I} \cup \{\txid^{\ke_{h}}_{l}\}) \wedge \txid' \in (\T^{\ke_{l}}{j} \cup \{\txid^{\ke_l}_{j}\} \wedge \txid \xrightarrow{\PO} \txid' &\implies \txid \rightarrow \txid'\\
\txid \rightarrow \txid' \rightarrow \txid'' &\implies \txid \rightarrow \txid''
\end{array}
\]
...
\txid^{i}
\]
which contradicts Proposition \ref{prop:si_cycles}.
