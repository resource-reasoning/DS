...
such that whenever $\vi(\ke) \neq \vi'(\ke)$ then either $(\otW, \ke, \stub) \in \fp$ or $(\otR, \ke, \stub) \in \fp$.
...
\item \caseI{$n > 0$} In this case, we have that $\tr = \tr' \xrightarrowtriangle{(\cl, \mu)} (\mkvs, \viewFun)$ 
...
By definition, we have that $(\otR, \ke, \val) \in \fp$. Recall that $\tau = \tau' \xrightarrow{(\cl, \fp)}_{\ET_{\top}} (\mkvs, \viewFun)$, 
...
By definition of execution test, and because $(\otR, \ke, \val) \in \fp$, then it must be the case that 
...
must be the case that $\WTx(\mkvs(\ke, r)) = \txid''$. Also, because $\WTx(\hh_{\aexec_{\tau'}}(\ke, r)) = \txid''$, 
...
By \cref{lem:graph.update} we have that either $\txid' \xrightarrow{\RF_{\Gr_\mkvs}(\ke)} \txid''$, 
or $\txid'' = \txid$, $(\otR, \ke, \stub) \in \fp$, $\txid' = \max_{\VO_{\Gr_\mkvs}(\ke)}\Setcon{\WTx(\ke, i) }{ i \in \vi(\ke)}$. 

\begin{itemize}
\item If $\txid' \xrightarrow{\RF_{\Gr_\mkvs}(\ke)} \txid''$, then because 
...
\item If $\txid'' = \txid$, $(\otR, \ke, \stub) \in \fp$, and $\txid' = \max_{\VO_{\Gr_\mkvs}(\ke)} \Setcon{ \WTx(\hh_{\aexec}(\ke, i)) }{ i \in \vi(\ke)}$, 
...
$\txid' \xrightarrow{\RF_{\Gr}(\ke)_{\Gr}} \txid''$, or $(\otR, \ke, \stub) \in \fp$, $\txid'' = \txid$ and 
$\txid' = \max_{\VO_{\Gr}(\ke)}(\T)$, 
\item $\txid' \xrightarrow{\VO_{\Gr'}(\ke)} \txid''$ if and only if 
either $\txid' \xrightarrow{\VO_{\Gr}(\ke)} \txid''$, or $(\otW, \ke, \stub) \in \fp$, $\txid'' = \txid$, 
and $(\otW, \ke, \stub) \in_{\Gr} \txid'$.
\end{enumerate}
\end{lemma}

\begin{proof}
Fix a key $\ke$. Let $\aexec' = \extend(\aexec, \txid, \T, \fp)$. Recall that $\Gr' = \graphOf(\aexec')$.
...
\item Suppose now that $(\otR,\ke, \stub) \in \fp$, and $\txid' = \max_{\VO(\ke)_{\Gr}}(\T)$. 
...
\item Suppose that $(\otW, \ke, \stub) \in_{\aexec} \txid'$, $(\otW, \ke, \stub) \in \fp$. Because $\txid' \in \T_{\aexec}$, 
we have that $\txid' \neq \txid$, hence $(\otW, \ke, \stub) \in_{\aexec' }\txid'$. By definition, 
$\TtoOp{T}_{\aexec'}(\txid) = \fp$, hence $(\otW, \ke, \stub) \in_{\aexec'} \txid$. Finally, 
...
Let $\Gr = \Gr_{\mkvs}$, $\Gr' = \Gr_{\mkvs'}$; then for all $\txid', \txid'' \in \T_{\Gr'}$ and keys $\ke$, 
\begin{itemize}
\item $\TtoOp{T}_{\Gr'} = \TtoOp{T}_{\Gr}\rmto{\txid}{\fp}$, 
\item $\txid' \xrightarrow{\RF_{\Gr'}(\ke)} \txid''$ if and only if either 
$\txid' \xrightarrow{\RF_{\Gr}(\ke)} \txid''$, or $(\otR, \ke, \stub) \in \fp$ and 
$\txid' = \max_{\VO_{\Gr}(\ke)}(\Set{\WTx(\mkvs(\ke, i)) }{ i \in \vi(\ke)})$, 
\item $\txid' \xrightarrow{\VO_{\Gr'}(\ke)} \txid''$ if and only if either 
$\txid' \xrightarrow{\VO_{\Gr}(\ke)} \txid''$, or $(\otW, \ke, \stub) \in \fp$ 
and $\txid' = \WTx(\mkvs(\ke, \stub))$. 
\end{itemize}
\end{lemma}
...
that $\mkvs(\ke, i) = (\val, \stub, \Set{\txid'} \cup \stub)$. Because $\mkvs' = \updateKV(\mkvs, \vi, \fp, \txid)$, 
it is immediate to observe that $\mkvs'(\ke, i) = (\val, \stub, \Set{\txid'} \cup \stub)$, and therefore 
$(\otR,\ke, \val) \in_{\Gr'} \txid'$. Conversely, note that if $(\otR, \ke, \val) \in_{\Gr'} \txid$, 
then there exists an index $i = 0,\cdots, \lvert \mkvs'(\ke) \rvert - 1$ such that 
$\mkvs'(\ke, i) = (\val, \stub, \Set{\txid'} \cup \stub)$. As a simple consequence of \cref{cor:updatekv.singlecell} 
it follows that it must be the case that $i \leq \lvert \mkvs(\ke) \rvert - 1$, and because 
$\txid' \neq \txid$, we have that $\mkvs(\ke, i) = (\val, \stub, \Set{\txid'} \cup \stub)$. Therefore 
$(\otR, \ke, \val) \in_{\Gr} \txid'$. 

Similarly, if $(\otW, \ke, \val) \in_{\Gr} \txid'$, 
then there exists an index $i=0,\cdots, \lvert \mkvs(\ke) \rvert - 1$ such that 
$\mkvs(\ke, i) = (\val, \txid', \val)$. It follows that $\mkvs'(\ke, i) = (\val, \txid', \stub)$, hence 
$(\otW, \ke, \val) \in_{\Gr'} \txid'$. If $(\otW, \ke, \val) \in \fp$, then we 
have from \cref{cor:updatekv.singlecell} that $\mkvs'(\ke, \lvert \mkvs'(\ke) \rvert - 1) = (\val, \txid', \stub)$, 
hence $(\otW, \ke, \val) \in_{\Gr'} \txid'$. 
Conversely, if $(\otW, \ke, \val) \in_{\Gr'} \txid'$, then there exists an index 
$i = 0, \cdots, \lvert \mkvs'(\ke) \rvert - 1$ such that $\mkvs(\ke, i) = (\val, \txid', \stub)$. 
We have two possible cases: either $i < \lvert \mkvs'(\ke, i) \rvert - 1$, leading to  
$\txid' \neq \txid$ and $\mkvs(\ke, i) = (\val, \txid', \stub)$, or equivalently 
$(\otR,\ke, \val) \in_{\Gr} \txid'$; or $i = \lvert \mkvs'(\ke, i) \rvert - 1$, 
leading to $\txid' = \txid$, and $\mkvs(\ke, i) = (\val, \txid, \emptyset)$ 
for some $\val$ such that $(\otW, \ke, \val) \in \fp$. 
...
By definition, there exists an index $i = 0,\cdots, \lvert \mkvs(\ke) \rvert - 1$ 
such that $\mkvs(\ke, i) = (\stub, \txid', \Set{\txid''} \cup \stub)$. It is immediate 
to observer, from the definition of $\updateKV$, that $\mkvs'(\ke, i) = (\stub, \txid', \Set{\txid''} \cup \stub)$, 
and therefore $\txid' \xrightarrow{\RF_{\Gr'}(\ke)} \txid''$. 

\item Next, suppose that $(\otR, \ke, \stub) \in \fp$, and $\txid' = \max_{\VO_{\Gr}(\ke)}(\Setcon{\WTx(\mkvs(\ke, i)) }{ i \in \vi(\ke)}$. 
By Definition, $\mkvs(\ke, i) = (\stub, \txid', \stub)$, where $i = \max(\vi(\ke))$. This is because 
$\txid' \rightarrow{\VO_{\Gr}(\ke)} \txid''$ if and only if $\txid' = \WTx(\mkvs(\ke, j_1)), \txid'' = 
\WTx(\mkvs(\ke, j_2))$ for some $j_1, j_2$ such that $j_1 < j_2$. 
The definition of $\updateKV$ now ensures that $\mkvs'(\ke, i) = (\stub, \txid', \Set{\txid } \cup \stub)$, 
from which it follows that $\txid' \xrightarrow{\RF_{\Gr'}(\ke)} \txid$.
...
\begin{itemize}
\item If $\txid'' = \txid$, then it must be the case that there exists an index $i = 0,\cdots, \lvert \mkvs'(\ke) \rvert - 1$ 
such that $\mkvs'(\ke, i) = (\stub, \txid', \Set{\txid } \cup \stub)$. Note that if $\mkvs'(\ke, \lvert \mkvs'(\ke) \rvert -1)$ is 
defined, then it must be the case that $\mkvs'(\ke, \lvert \mkvs'(\ke) \rvert -1) = (\stub, \txid, \emptyset)$, 
hence it must be the case that $i < \lvert \mkvs'(\ke) \rvert - 1$. Because $\txid \notin \mkvs$, 
then by the definition of $\updateKV$ it must be the case that $(\otR, \ke, \stub) \in \fp$, 
$\mkvs(\ke, i) = (\stub, \txid', \stub)$ and $i = \max(\vi(\ke))$; this also implies that $\txid' = 
\max_{\VO(\ke)}\Setcon{\WTx(\mkvs(\ke, i)) }{ i \in \vi(\ke)}$. 

\item If $\txid'' \in \T_{\Gr}$, then  it must be the case that $\txid'' \neq \txid$. 
In this case, it also must exist an index $i = 0,\cdots, \lvert \mkvs'(\ke) \rvert - 1$ 
such that $\mkvs'(\ke, i) = (\stub, \txid', \Set{\txid''} \cup \stub)$. As in the previous 
case, we note that $i < \lvert \mkvs'(\ke) \rvert - 1$, which together 
with the fact that $\txid'' \neq \txid$ leads to $\mkvs(\ke, i) = (\stub, \txid', \Set{\txid''} \cup \stub)$. 
...
$\mkvs(\ke, i) = (\stub, \txid', \stub)$, $\mkvs(\ke, j) = (\stub, \txid'', \stub)$ 
and $i < j$. The definition of $\updateKV$ ensures that 
$\mkvs'(\ke, i) = (\stub, \txid', \stub)$, $\mkvs'(\ke, j) = (\stub, \txid'', \stub)$, 
and because $i < j$ we obtain that $\txid' \xrightarrow{\VO_{\Gr'}(\ke)} \txid''$. 

\item Suppose that $(\otW, \ke, \stub) \in \fp$. Then $\mkvs'(\ke, \lvert \mkvs(\ke) \rvert) = (\stub, \txid, \stub)$.
Let $\txid' \in \T_{\Gr}$; by definition there exists an index $i = 0,\cdots, \lvert \mkvs(\ke) \rvert$ 
such that $\mkvs(\ke, i) = (\stub, \txid', \stub)$. It follows that $\mkvs'(\ke, i) = (\stub, \txid', \stub)$, and 
because $i < \lvert \mkvs(\ke) \rvert$, then we have that $\txid' \xrightarrow{\VO_{\Gr'}(\ke)} \txid$. 
...
or equivalently there exists an index $i=0,\cdots, \lvert \mkvs'(\ke) \rvert -1 $ such that 
$\mkvs'(\ke, i) = (\stub, \txid, \stub)$. Because $\txid \notin \mkvs$, and because for any 
$i = 0, \cdots, \lvert \mkvs(\ke) \rvert - 1$, $\mkvs'(\ke, i) = (\stub, \txid, \stub) \implies 
\mkvs(\ke, i) = (\stub, \txid, \stub)$, then it necessarily has to be $i = \mkvs'(\ke) \rvert - 1$. 
According to the definition of $\updateKV$, this is possible only if $(\otW,\ke, \stub) \in \fp$. 
Finally, note that because $\txid' \xrightarrow{\VO_{\Gr'}(\ke)} \txid$, then 
there exists an index $j < \lvert \mkvs'(\ke, i) \rvert - 1$ such that 
$\mkvs'(\ke, j) = (\stub, \txid' ,\stub)$. The fact that $j < \lvert \mkvs'(\ke, i) \rvert - 1$ 
From \cref{cor:updatekv.singlecell} we obtain that $\mkvs(\ke, j) = (\stub, \txid', \stub)$, 
or equivalently $\txid' = \WTx(\mkvs(\ke, \stub))$. 

\item If $\txid'' \in \T_{\Gr}$, then there exist two indexes $i,j$ such that 
$j < \lvert \mkvs'(\ke, j) \rvert - 1$, $\mkvs'(\ke, j) = (\stub, \txid'', \stub)$, 
$i < j$, and $\mkvs'(\ke, i) = (\stub, \txid', \stub)$. It is immediate to observe 
that $\mkvs(\ke, i) = (\stub, \txid', \stub)$, $\mkvs(\ke, j) = (\stub, \txid'', \stub)$, 
