...
\begin{array}[t]{@{}l}
\txid = \txid_{\cl}^{n}
\wedge \VIS' = \VIS_{\aexec} \uplus \Setcon{(\txid', \txid) }{ \txid \in \txidset}  \\
{} \wedge \AR' = \AR_{\aexec} \uplus \Setcon{(\txid', \txid) }{ \txid' \in \txidset_{\aexec}}
\end{array}
\end{rclarray}
\]
...
\item 
    ...
    then $\extend(\aexec, \txid, \txidset, \fp) \in \aeset\left(\tr \xrightarrowtriangle{(\cl, \fp)}_{\ET_{\top}} (\mkvs, \vienv) \right)$.
\end{itemize}
\end{definition}
...
\Setcon{\wtOf(\mkvs_{\aexec_{\tau'}}(\key',  i)) }{ \key' \in \Keys \wedge  i \in \vienv'(\cl)(\key')}
\end{array}
\]
...
or $\txid'' = \txid$, $(\otR, \key, \stub) \in \fp$, $\txid' = \max_{\WW_{\Gr_\mkvs}(\key)}\Setcon{\wtOf(\key, i) }{ i \in \vi(\key)}$. 
...
\item If $\txid'' = \txid$, $(\otR, \key, \stub) \in \fp$, and $\txid' = \max_{\WW_{\Gr_\mkvs}(\key)} \Setcon{ \wtOf(\mkvs_{\aexec}(\key, i)) }{ i \in \vi(\key)}$, 
    then we also have that $\txid' = \max_{\WW_{\graphOf(\aexec)}(\key)} (\txidset \cap \Setcon{ \txid''' }{ (\otW, \key, \stub) \in_{\aexec} \txid'''}) $. 
This is because of the assumption that 
\[
\begin{array}{l}
    \Setcon{\wtOf(\mkvs_{\aexec}(\key, i)) }{ i \in \vi(\key)}
    \begin{array}[t]{l}
    = \Setcon{\wtOf(\mkvs_{\aexec}(\key', i)) }{ \key' \in \Keys \wedge i \in \vi(\key')} \cap \Set{\wtOf(\mkvs_{\aexec}(\key, \stub)} \\
= \Tx(\mkvs_{\aexec}, \vi) \cap \Set{\wtOf(\mkvs_{\aexec}(\key, \stub)}  \\
= \txidset \cap \Setcon{(\txid''' }{ (\otW, \key, \stub) \in_{\aexec} \txid''')}
\end{array}
\end{array}
\]
...
\item Suppose now that $(\otR,\key, \stub) \in \fp$, and $\txid' = \max_{\WW(\key)_{\Gr}}(\txidset)$. 
    By Definition, $\txid' = \max_{\AR_{\aexec}}(\txidset) \cap \Setcon{ \txid''' }{ (\otW, \key, \stub) \in_{\aexec} \txid'''}$, 
...
\cap \Setcon{\txid''' }{ (\otW, \key, \stub) \in_{\aexec} \txid'''}) = \max_{\WW_{\Gr}(\key)}(\txidset)$. 
...
\Setcon{\txid' }{ \txid' \in \mkvs} \cup \Set{\txid}$. It follows that $\txidset_{\Gr'} = \txidset_{\Gr} \uplus \Set{\txid }$.
...
\item Next, suppose that $(\otR, \key, \stub) \in \fp$, and $\txid' = \max_{\WW_{\Gr}(\key)}(\Setcon{\wtOf(\mkvs(\key, i)) }{ i \in \vi(\key)}$. 
...
\max_{\WW(\key)}\Setcon{\wtOf(\mkvs(\key, i)) }{ i \in \vi(\key)}$. 
