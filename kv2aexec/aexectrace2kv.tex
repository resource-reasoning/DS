...
\xrightarrowtriangle{(\cl, \fp)}_{\ET_{\top}} (\mkvs, \viewFun_{\tr}\rmto{\cl}{\vi'}) 
\end{array}
...
where $\viewFun_{0} = \lambda \cl \in \Clients(\aexec). \lambda \key.\Set{0}$. 
...
\[
\tr = \tr' \xrightarrowtriangle{(\cl, \varepsilon)} (\hh_{\tr'}, \viewFun_{\tr'}\rmto{\cl}{\vi}) \xrightarrowtriangle{(\cl, \fp)} 
(\mkvs,  \viewFun_{\tr'}\rmto{\cl}{\vi'})
\]
where $(\hh_{\tr'}, \viewFun_{\tr'}) = \lastConf(\tr')$, and $\mkvs = \updateKV(\hh_{\tr'}, \vi, \fp, \txid_{i})$, 
...
It suffices to prove that $\viewFun_{\tr'}(\cl) \viewleq \vi$ for any key $\key$.
...
that  $\viewFun_{\tr'}(\cl) = \lambda \key. \Set{0}$. Because $\vi = \getView(\aexec, \VIS^{-1}_{\aexec}(\txid_{i}))$, 
...
It follows that the last transition in $\tr'$ of the form $(\stub, \stub) \xrightarrow{(\cl, \fp_{j})} (\stub, \viewFun_{\mathsf{pre}})$ 
...
has the form $(\stub, \stub) \xrightarrowtriangle{(\cl, \fp_{j})}_{\ET_{\top}} (\stub, \viewFun_{\mathsf{pre}})$, 
...
\item $(\hh_{\tr'}, \viewFun_{\tr'}\rmto{\cl}{\vi}) \xrightarrowtriangle{(\cl, \fp)}_{\ET_{\top}} (\mkvs,  \viewFun_{\tr'}\rmto{\cl}{\vi'})$. 
