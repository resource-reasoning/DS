...
\[
\Tx(\mkvs, \vi) \defeq \Setcon{ \WTx(\mkvs(\ke, i)) }{ \ke \in \Keys \wedge i \in \vi(\ke) }
\]
...
\begin{proof}
Fix $\T \subseteq \aexec$, and let $\{\mkvs\} = \RP_{\LWW}(\aexec, \T)$. We prove that, for any $\ke \in \Keys$, 
$\mkvs(\ke) = \snapshot(\getView(\aexec, \T))(\ke)$. There are two different cases: 
\begin{enumerate}
\item $\T \cap \{ \txid \mid (\otW, \ke, \stub) \in_{\aexec} \txid \} = \emptyset$. 
In this case $\mkvs(\ke) = \val_0$. 
...
$\mkvs(\ke', j) = (\stub, \txid, \stub)$; because kv-stores only allow a transaction to write at most one version 
