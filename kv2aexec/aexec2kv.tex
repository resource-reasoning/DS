...
\begin{itemize}
\item $\txid \xrightarrow{\WR_{\aexec}(\key)} \txid'$ if and only if 
$\txid = \max_{\AR_{\aexec}}(\visibleWrites_{\aexec}(\key, \txid'))$, 
\item $\txid \xrightarrow{\WW_{\aexec}(\key)} \txid'$ if and only 
$\txid, \txid' \in_{\aexec} (\otW, \;\key, \stub)$ 
and $\txid \xrightarrow{\AR_{\aexec}} \txid'$,
\item $\txid \xrightarrow{\RW_{\aexec}(\key)} \txid'$ if and only if either 
$(\otR, \key, \stub) \in_{\aexec} \txid, (\otW, \key, \stub) \in_{\aexec} \txid'$ and 
whenever $\txid'' \xrightarrow{\WR_{\aexec}(\key)} \txid$, 
then $\txid'' \xrightarrow{\WW_{\aexec}(\key)} \txid'$.
\end{itemize}
\end{definition}
...
$\graphOf(\aexec)$ gives that $\txid' \xrightarrow{\WW_{\aexec}(\key)} \txid$. 
...
be the case that $\txid \xrightarrow{\WW_{\aexec}(\key)} \txid'$, and because 
$\WW_{\aexec}(\key)$ is transitive and irreflexive, it must be that  
$\neg( \txid' \xrightarrow{\WW_{\aexec}(\key)} \txid)$ and $\txid \neq \txid'$: this implies that 
