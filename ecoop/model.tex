...
by the view in the C11 operational semantics in \cite{promises}.
...
(1) it constructs an initial \emph{snapshot} \(\sn\) of \(\mkvs\) using its view \(\vienv(\cl)\)as defined in \cref{def:snapshot};  
(2) it executes \(\trans\) in isolation over \(\sn\)
accumulating the effects (the reads and writes) of executing \(\trans\); and
(3) it commits \(\trans\) by incorporating these effects into \(\mkvs\).
...
We are ready for client \(\cl\) to commit the transaction resulting 
in the kv-store \(\mkvs'\) with the client view \(\vi''\) \emph{shifting} to a new view \(\vi'\): 
(1) pick a fresh transaction identifier \(\txid \in \nextTxid[\cl, \mkvs]\);
(2) compute the new kv-store \(\mkvs' = \mathtt{update}(\mkvs, {\allowbreak \vi''},\)
\(\fp, \txid)\); and 
(3) check if the \emph{view shift} is permitted under execution test \(\ET\) using \(\vshift{\mkvs}{\vi''}{\mkvs'}{\vi'}\). 
...
then \(\vilist\rmto{i}{\ver} \defeq \ver_0 \lcat \cdots \lcat \ver_{i-1} \lcat \ver \lcat \ver_{i+1} \lcat \cdots \lcat \ver_{n}\).
