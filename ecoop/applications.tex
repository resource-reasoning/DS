...
(1) \( \pcopsput(\key,\val) \) for writing to a \emph{single}
key $\key$; and 
(2) \( \pcopsread(\keyset) \) for atomically reading from \emph{a set} of keys  $\keyset$.  
...
(1) first reads \( (\key_1,\val_1,(\txid_1,\repl_1), \emptyset) \) for $\key_1$ (highlighted); %(highlighted in \cref{fig:cops-request-values})
(2) then receives two synchronisation messages from \( \repl_2 \), 
containing versions \( ( \key_1, \val'_1, (\txid_1,\repl_2),\emptyset ) \) and \( ( \key_2, \val'_2, (t_2,\repl_2),\Set{(\key_1,\txid_1,\repl_2)} ) \); and
(3) finally reads \( ( \key_2, \val'_2, (t_2,\repl_2),\Set{(\key_1,\txid_1,\repl_2)} ) \) for $\key_2$ (highlighted).
...
(1) every COPS trace can be transferred to a normalised COPS trace, 
in which multiple reads of a transaction are not interleaved by other transactions; and
(2) the normalised COPS trace can be refined to traces in our semantics, 
in which (3) each step satisfies \( \ET[\CC] \).
...
(1) \( \cl \) writes version \( (\opW,\key_1, (\txid_1,\repl_1)) \) to $\repl_1$;
(2) \( \cl \) starts a multi-read transaction (\( \mathtt{s} \));
(3) \( \cl \) reads \( (\opR,\key_1, (\txid_1,\repl_1)) \) from $\repl_1$;
(4) \( \repl_1 \) receives synchronisation messages (\(\mathtt{sync}\));
(5) \( \cl \) reads \( (\opR,\key_2, (\txid_2,\repl_2)) \) from $\repl_1$;
(6) \( \cl \) enters the second phase of the multi-read transaction (\(\mathtt{p}\));
(7) an arbitrary step \( \lb \) interferes;
(8) \( \cl \) re-fetches the version \( (\opR,\key_1, (\txid_1,\repl_2)) \) from \( \repl_2 \) and puts the version in the snapshot;
(9) an arbitrary step \( \lb' \) interferes;
(10) \( \cl \) puts the version \( (\opR,\key_2, (\txid_2,\repl_2)) \) in the snapshot; and
(11) \( \cl \) reads the values in the snapshot and commits the transaction (\(\mathtt{e}\)).
...
(1) the $\cl$ context grows, and thus we obtain a more up-to-date view of the associated kv-store; \ie $\vshiftname_{\MR}$ holds;
(2) the $\cl$ context always includes the time-stamp of the versions written by itself, and thus the 
corresponding client view always includes the versions $\cl$ has written; \ie $\vshiftname_{\RYW}$ holds; and
(3) the $\cl$ context is always closed to the relation \( \DEP \), 
...
correctness of a program pattern using a lock library  against \( \UA \) and
...
client sessions. \polish{HAs anyone done anything like locks?} 
\polish{No, I dont think so. }

A {\em transactional
library}  is a set of transactional operations: for example, the counter
library, 
...
\polish{Belowis a result, not a definition! Now say something about
  this proof, where is it given, is it easy,....} 
\polish{we adopt a well-known result.} 
...
\mypar{Single-counter Library: Robustness}..... \polish{This title can be moved above; we have explained the single-counter library and robustness above.}
...
\polish{not read.} In the single-counter library, $\Counter(\key)$, 
...
\begin{proof}[Proof (sketch)]
\polish{Not read.} From the definition of \( \CMs \) (\cref{def:cm})
...
\end{theorem}
\begin{proof}[Proof (sketch)]
\polish{NOt read.} We proceed as in the proof of \cref{thm:acyclic-cc}.
...
\polish{Have we any way of explaining why these conditions, except
  that they work?} 
\polish{In \( \SI \) and \( \WSI \), the read only transacions (\cref{equ:main-wsi-safe-read-only}) can be reorder, moving backward or forward without caurse programs.
The rest transactions, that are transacions satisfies strictly {no-blind writes} (\cref{equ:main-wsi-safe-write-must-read,equ:main-wsi-safe-all-write}), 
must be sequentialised, satisfying serialisability. }
  That is, 
...
\eqref{equ:main-wsi-safe-write-must-read}  if \( \txid \) writes to $\key$, then it must also read from it, a property known as \emph{no-blind writes}~\footnote{From
       \(\UA\), it is
  immediate that $j = i-1$.}; and
\eqref{equ:main-wsi-safe-all-write}  if \( \txid \) writes to $\key$,
then it must also write to all keys it reads from.
It is straightforward to see that the  multi-counter
library given in \cref{sec:overview} is \(\WSI\)-safe. \polish{Are any
  of the other libraries from references 7 and 9 WSI-safe? Do we have
  an example of a well-known libary satisfying 1,2 and 3.}
\polish{Not sure if the well-known benchmark satisfies this property.
But examples in \cite{bank-example-wsi,giovanni_concur16,snapshot-isolation-robust-tool}. 
I cannot say that they are well-known.}
...
%
\begin{proof}[Proof (sketch)]
\polish{Not read.} Pick a \(\WSI\)-safe  library \( L \), a client program \( \prog \) of  $L$ and a kv-store \( \kvs \) obtained from executing \( \prog \) under $\WSI$;
...
\polish{Hardly touched from here.} 
...
(1) they identified a notion of dangerous dependency between transactions,
which, they argued, can lead to violation of robustness of \( \SI \); and
(2) they argued this banking example contains no such dangerous dependencies.
