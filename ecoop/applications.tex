...
client sessions.
\polish{HAs anyone done anything like locks?} 
\polish{No, I dont think so. }

\mypar{Single-counter Library: Robustness}
A {\em transactional library}  is a set of transactional operations: for example, the counter
library, 
...
%\polish{Have we any way of explaining why these conditions, except
  %that they work?} 
\polish{INTUITIVE EXPLANATION: In \( \SI \) and \( \WSI \), the read only transactions \eqref{equ:main-wsi-safe-read-only} can be reordered, moving backward or forward without modifying values.
The rest transactions, that are transactions satisfying strictly no-blind writes (\eqref{equ:main-wsi-safe-write-must-read} and \eqref{equ:main-wsi-safe-all-write}), 
must be sequentialised, hence having serialisable behaviours. }
  That is, 
