...
can be \polish{I don't know what this means, they can't be reordered
  psat a write....reordered so that they have serialisable behaviour.}

\polish{You have previously said that 2
  corresponds to no-blind writes. Now are saying that 2 and 3 satisfy
  no-blind writes.......Please take more care. I cannot read
  everything that you are doing. Either remove or find a way that
  Azalea can help. I do not understand the explanation.
Under \( \WSI \) and  \( \SI \), transactions that satisfy strictly no-blind writes (\eqref{equ:main-wsi-safe-write-must-read} and \eqref{equ:main-wsi-safe-all-write}), 
...
\polish{I'm sorry but I did not understand your answer Shale. Are the
  libraries in 7 and 9 WSI-safe: if so you should say so; if not say nothing.} 
