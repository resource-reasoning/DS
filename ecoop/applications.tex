...
Under \( \SI \) and \( \WSI \), transactions that satisfy strictly no-blind writes (\eqref{equ:main-wsi-safe-write-must-read} and \eqref{equ:main-wsi-safe-all-write}), 
must be sequentialised, hence having serialisable behaviours;
and the read only transactions, satisfying \eqref{equ:main-wsi-safe-read-only}, 
can be reordered so that they have serialisable behaviours.
