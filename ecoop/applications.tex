...
behaviour is, thus, serialisable  in the sense that the write they are
reading is current.
Under \( \WSI \) and  \( \SI \), transactions satisfying  {\em
  strict no-blind writes}, that is  \eqref{equ:main-wsi-safe-write-must-read}
and \eqref{equ:main-wsi-safe-all-write},  enforce a total order between
transactions on a key, which is enough to obtain 
...
because execution graphs do not directly record the library \emph{state} and merely record the execution shape, thus making it harder to reason about such guarantees.
