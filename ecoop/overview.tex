...
is allowed under \emph{causal consistency} consistency model  (CC),
...
Our operational semantics provides   transitions over centralised
{abstract states},
...
are possible at the point where the transaction commits.  One way of
...
(2) if a transaction, such as \( \txid \),
...
(3) if a transaction, such as \( \txid_3 \), reads a older version of a \( \key_1 \),
...
with previous results which showed that these protocols satisfy specific
...
if $\kvs$ is obtained by executing \(\prog\) under a weak model, then $\kvs$ can also be obtained under the stronger model serialisability.
...
against \(\SI\).
We do the latter by proving general invariants that guarantee robustness against our new proposed model \( \WSI \) which is weaker than $\SI$; this implies robustness against stronger models such as  \( \SI \).
