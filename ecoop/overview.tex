...
is allowed under \emph{causal consistency} (CC) consistency model,
...
{abstract states},
comprising a centralised, multi-versioned {\em kv-store},
which is {global} in that 
it records  all the versions written by all its clients, a 
\emph{client view},  which is
{partial} in that it records  those 
 versions in the kv-store observed by a client, and a variable store
 local to the client. 
Each transition of our operational semantics either updates the
variable store using a 
a primitive command, or updates the kv-store using an 
atomic transaction. 
...
has been read by \(\txid_1 \) and a new version with value 1 installed at
the end of the list. 
...
This is where the partial \emph{client view} come into play.  Intuitively, a view of
...
\(\key\) has  a  view containing  {\em all} the versions of  \(\key\)
available in the
...
%It ensures that 
requiring that  a client view is a set of versions 
that is {closed} with respect to the commit order of transactions.
...
\polish{I'm not sure about this sentence, how is it
  over-approximating?: However, we have enough information to over-approximate the relevant commit order between transactions}:
(1) if a transaction, such as \( \txid_3 \) in \cref{fig:mult-counter},
reads a version written by another transaction, such as \( \txid_0 \),
then it must start  after the commit of the transaction that
wrote the version, \( \txid_3 \) must start after the commit of  \( \txid_0 \)
(\cref{fig:overview-dependencies-time-line});\polish{And t1 must be
  after the comit of t0, figure needs changing} 
(2) if a transaction
writes a newer version of a key, such as \( \txid_1 \) for \( \key_1 \), 
then  it must commit after transactions that wrote the previous
versions of the key, such as \( \txidinit \)  (\cref{fig:overview-dependencies-time-line}); and
(3) if a transaction reads a older version of
a key, such as \( \txid_3 \) for \( \key_1 \),
it must start before the commit of any transactions that write the
newer versions of \( \key \), such as \(
...
models and the declarative definitions  based on abstract executions \cite{framework-concur},
...
demonstrated  that these protocols satisfy specific
consistency models  defined for those particular implementations.
...
if $\kvs$ is obtained by executing \(\prog\) using a weak consistency
model, then $\kvs$ can also be obtained using \(\SER\): 
...
new  model, \( \WSI \),  proposed by ourselves that lies between \(\PSI\)
and  $\SI$. 
...
\polish{We need to be more precise with this last sentence: is this  about 18 and 24?}
  talking about This is because  existing techniques do not directly record the library \emph{state}; 
