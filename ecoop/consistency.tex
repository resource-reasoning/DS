...
\(\PSI\) can be formally defined as the composition of
 \(\CC\) and \(\UA\) \cite{framework-concur}. In contrast, it is not possible to define \(\cancommitname_{\PSI}\) as
...
written by a transaction, observes all previous transactions writing versions of \(\key\).
For example,
transaction \( \txid^{1}_{\cl'} \) that writes the third version of \( \key_1 \) in \cref{fig:cc-ua-allowed-but-psi} must observe \( \txid^{1}_{\cl} \). 
This relation can be captured by write-write dependencies (\(\WW[\mkvs]\)),
since \( (\txid^{1}_{\cl}, \txid^{1}_{\cl'} ) \in \WW[\mkvs] \),
which leads us to include \(\WW[\mkvs]\) in \(\rel[\PSI]\).
...
Under \( \PSI \), the client view of \( \txid \) should include versions written by \( \txid^{1}_{\cl} \),
