...
\polish{Another big long list, please make smaller and try to be
...
to \(\vi'\). \polish{Do we use this or is this historical: We adopt the notation \(\ET \vdash (\mkvs, \vi) \csat \fp: (\mkvs', \vi')\) to capture this intuition. }
...
\polish{Question: could have $\txid \in \mkvs \wedge \txid \neq
  \wtOf[\mkvs(\key,i)] $ rather than arb $t$?}
...
\polish{I don't think you need to say this here, you say at end and
  beginning of section: In \cite{shale-phd},  we show that our definitions correspond to
...
\polish{I've changed this paragraph, what do you think?} We now give the definitions of well-known consistency models in distributed databases,
...
into the conjunction of \( \UA \) and \( \CC \).\polish{Why separate
  PSI and SI?  
  citation? } Interestingly, this is not quite the case for the
...
\end{enumerate*}
With abstract executions, \polish{Is this comment correct, I've tried
  to clarify without really knowing what I am doing:using the fact that there is a total order
on transactions,} \(\PSI\) can be formally defined as the conjunction of
...
\(\UA\). 
This is for two reasons. \polish{Presumably, it's also because there's
  not a total order so something you would argue for abstract
  executions you can't argue here. I can't make this conenction....} 
...
but not \(\cancommitname_{\PSI}\). \polish{Goodness, this explanation
  of PSI is so technical, probably too late to do more now.} 


\polish{Not done from here.} 
