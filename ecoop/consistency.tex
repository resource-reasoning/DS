...
(1) \(\vi {\in}  \allowbreak \Views(\mkvs)\) and \(\vi' {\in} \Views(\mkvs')\); 
(2) \(\cancommit \mkvs \vi \fp\); 
(3) \(\vshift \mkvs {\allowbreak \vi} {\mkvs'} {\vi'}\); and 
(4) for all \(\key \!\in\! \mkvs\) and \(\val \!\in\! \Val\), if \((\otR, \key, \val) \!\in\! \fp \) then \(	\mkvs(\key, \max{}_{<}(\vi(\key))) {=} \val   \).
\end{definition}
\SpaceBelowDef
...
This consistency model requires: 
(1) if a client view includes 
a version \(\ver\) written by \( \txid \) prior to committing a transaction, 
then it must also include the versions which \(\txid\) observes; and
(2) there is no write-conflict.
...
The proof is given in \cite{shale-phd}. 
