...
\begin{lemma}
    ...
    For any abstract execution \( \aexec \) satisfies 
    \( \left( (\SO \cup \WR ) ; \RW? \cup \WW \right)^* ; \VIS_\aexec \subseteq \VIS_\aexec \) 
    and  \( \SO_\aexec \subseteq \VIS_\aexec \),
    it satisfies \( \AR_\aexec ; \VIS_\aexec \subseteq \VIS_\aexec \)
    and  \( \SO_\aexec \subseteq \VIS_\aexec \).
    The another way round also holds.
\end{lemma}
\begin{proof}
...
Then, by \cref{lem:cp-eauiv-spec} to prove soundness and completeness of \( \ET_\CP \), it is sufficient to use the specification:
\[
...
\subsubsection{Serialisibility \( \SER \)}
