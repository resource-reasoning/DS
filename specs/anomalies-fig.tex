...
\matrix(versionx) [version list,,column 2/.style={text width=8mm},column 4/.style={text width=7mm},ampersand replacement=\&]
    at ([xshift=\tikzkvspace]locx.east) {
...
%Location y
\path (versionx.east) + (0.8,0) node (locy) {$\ke_2 \mapsto$};
\matrix(versiony) [version list,column 2/.style={text width=8mm},column 4/.style={text width=7mm},ampersand replacement=\&]
   at ([xshift=\tikzkvspace]locy.east) {
...
%\sx{The example above is a counter example, why in our framework \( \SI = \CP \cap \UA \) does not hold, while it holds in abstract execution.
%The example satisfies our definition of \( \CP \) and \( \UA \), but does not satisfies our definition of \( \SI \).
%Because \( \SI = \CP \cap \UA \) relies on agreement on a total arbitration order, while in our framework we can only recover a minimum arbitration order per consistency model and the two minimum arbitration orders from \( \CP \) and \( \UA \) are not necessary agrees.}
