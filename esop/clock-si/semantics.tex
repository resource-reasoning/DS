...
\begin{centermultline}[equ:clock-si-no-interleaving]
        ...
        {} \implies \exsts{\ct', \mkvs_j, \cts_j, \cts'_j, \thdenv_j, \prog_j}  \\
...
\begin{proof}
    ...
    \begin{itemize}
        \item \caseB{\( \fp = \emptyset \)}
            It is easy to see that 
            \[ \func{commitKV}[\mkvs,\ct,\txid_\cl^{\ct'},\emptyset] = \mkvs =  \updateKV{\mkvs, \func{viewOf}[\mkvs, \ct],\fp, \txid_\cl^{\ct'}} \]
...
\begin{proof}
    ...
    By the \rl{ReadRemote}, it follows \[ n  = \max\Set{n'}[\exsts{j} \txid^{n'} = \wtOf(\mkvs(\key, j)) \land n' < \ct'] \] which implies \( n < \ct' \).
\end{proof}
