...
rhe well-formed conditions for a world are:
\textbf{(i)} the state of every region \( (\mkvs, \vi, \ca'') \) is disjointed with each others, and satisfies the invariant of the region \( (\mkvs, \vi, \ca'') \in \func{inv}{\rid, \intf}\); 
\textbf{(ii)} the domain of the view in a region is the same as the domain of the key-value store, \ie they have exactly the same keys; 
\textbf{(iii)} the view should not be out of the range of the key-value store;
\textbf{(iv)} capabilities from regions and local capabilities are compatible;
...
\(
\begin{rclarray}
    \eraseW{\w} & \defeq & (\hh, \vi)
\end{rclarray}
\),
where \( (\hh, \vi, \stub) \in \func{collapse}{\w\projection{2}} \).
...
\textbf{(i)} the transformation of key-value store is allowed by the execution tests \( (\mkvs, \mkvs') \in \et \), which means the environment executes a fingerprint \( \fp \) under its view and it passes the execute test;
