...
Before presenting the model, we conduct an abuse of notation by
making \( \mkvs \) and \( \vi \) partial functions.
We can extend a partial key-value store \( \mkvs\) into total \( \mkvs' \), 
such that for transaction in \( \mkvs \), it never appear in \( \mkvs' \) and vice versa.
It is safe to assume that, since we do not model the allocation and deallocation.
