...
The assertions inside regions \( \bar{\lpre} \) have capability assertions for shared capabilities, single key assertion \( \lexpr \pt \lexpr \) and other standard separation logic assertions.
The single key assertions \( \lexpr_1 \pt \lexpr_2 \) are interpreted to any kv-store and view where the view point to a version of key \( \evalLE{\lexpr_1 }\) in the kv-store carrying value \( \evalLE{\lexpr_2}\).
...
The \emph{rely} describes the allowed actions for the environment while the guarantee are for the current client (\cref{def:rely-guarantee}).
...
The \emph{rely} describes how the environment can change the state of the shared regions.
...
Separately, the \emph{view shift rely} \( \relyV \) allows to advanced the local view.
