...
\sx{and (b) the new view under the new key-value store is able to progress.
...
It is especially useful when the execution tests are for serialisibility, which means that the view can always shift to the end when the key-value store has been updated by the environment.
\[
    \vi' \geq \vi
    \quad {} \fora{opset}
    \land (\hh', \vi) \csat \opset : \stub \implies (\hh', \vi) \csat \opset : \stub \\
\]
}
