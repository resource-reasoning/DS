...
\begin{array}{@{}l}
    ...
    \quad {} \vdash (\stk, \clpsHH{\mkvs, \vi}, \unitO), \trans \toL (\stk', \h', \opset), \pskip  \\
...
\end{array}
\]
Because the \cref{lem:prgram-step-to-command-step} that the \( \predn{stepp} \) implies \( \predn{stepc}\), 
...
\begin{itemize}
    ...
    \begin{itemize}
        \item For the case that one client from \( \prog \) executes its commands, we unfold the definition of \( \predn{stepp} \) and assume that the \( \prog \) takes one step to a new \( \wenv_i\), \( \prog_i \) and \( \thdenv_i \):
        \[
        \begin{array}{@{}l}
...
Predicate \( \predn{stepp} \) implies  \( \predn{stepc} \).
\[
\begin{array}{@{}l}
