...
\end{rclarray}	 
\]
The \emph{transactional assertion interpretation function}, $\evalLS[(.,.)]{.}: \LAst \times \LEnv \times \LAst \parfun \powerset{\Snapshots \times \Fingerprints} $, is defined over the structure of local assertions, where the composition for snapshots \( \composeH \defeq \uplus \) is standard disjointed union on two functions and the composition for fingerprints \( \fp \composeO \fp' \defeq \fp \uplus \fp'\) when they contain different keys, \ie \( \fp\projection{2} \cap \fp'\projection{2} = \emptyset\):
\[
\begin{rclarray}
	...
	\evalLS{\lpre \implies \lpost} & \defeq & \Setcon{(\h, \fp)}{(\h , \fp) \in \evalLS{\lpre} \implies (\h , \fp) \in \evalLS{\lpost}}\\
    ...
    \Setcon{
        (\h_1 \composeH \h_2, \fp_{1} \composeE \fp_{2})
    }{ 
        (\h_{1},\fp_{1}) \in \evalLS{\lpre} 
        \land (\h_{2}, \fp_{2} ) \in \evalLS{\lpost} 
    } 
\end{rclarray}%
\]
\end{definition}
