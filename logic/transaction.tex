...
Recall that a transaction take a snapshot of the kv-store and commits the fingerprint by the end.
...
the \emph{transactional assertions} (\cref{local_assertions}) extend normal sequential separate logic assertions with read and/or write labels, 
\eg \( \vx \fpR 0 \) or \( \vy \fpW 1 \).
