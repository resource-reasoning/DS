...
\item The transaction $\txid_{1}$ executes with the initial view, which points to the initial (and only) version for each location; after this executing the transaction, a new version $( 1, \txid_1, \emptyset )$ is appended to \( \key_\vx\) (\figref{fig:cc-exec-b}).
\item The second thread $\thid_2$ updates its view as to see the last version of $\key_\vx$ installed by $\thid_1$ (\figref{fig:cc-exec-c}), after which it proceeds to execute transaction $\txid_{2}$. 
This results in a new version with value $1$ to be installed to the end of $\key_\vy$ (\figref{fig:cc-exec-d}). 
\item Finally, the thread $\thid_3$ updates its view so to observe the update of location $\key_{\vy}$, but not the update of 
location $\key_{\vx}$ (\figref{fig:cc-exec-e}).
...
intuitive after all...}
Let consider the history $\hh$ in \figref{fig:cc-view-a}, where $\pred{ddep}{\hh(\key_\vz)(1),\hh(\key_{\vy})(1)}$, and $\pred{ddep}{\hh(\key_{\vy})(1) ,\hh(\key_{\vx})(1)}$.
...
Therefore we can only update the view $V_0$ by including the second version of $\key_{\vx}$, \ie the new view \(\vi_1 = \vi_0\rmto{\key_\vx}{1} \) (\figref{fig:cc-view-b}).
The new view \( \vi_1\) is causally consistent as no version directly depend on the second version of \( \key_\vx\).
We can now update the view $\vi_1$ to include the latest version of $\key_{\vy}$, resulting in the view $\vi_2 = \vi_1\rmto{\key_{\vy}}{1}$ (\figref{fig:cc-view-c}). 
The view \( \vi_2\) is causally consistent as the second version of \( \key_\vx\) is already included in the view.
Finally, the view includes the latest version of \( \key_\vz \) (\figref{fig:cc-view-d}).
The view \( \vi_3\) is causally consistent as the second version of \( \key_\vy\) is already included in the view.

%Because the view $\vi_0$ is causally consistent, and because all the versions on which $\hh(\key_{\vy})(V_2(\key_{\vy}))$ directly depends
...
%%state where the view thread of $\thid_3$ observes the update to location $\key_{\vy}$, but not the 
%%update of $\key_{\vx}$ from which the latter causally depends. 
...
%\begin{definition}
%Let $\hh, V$ and $\key_{\vx}$ be a history heap, a view, and a location, respectively. 
%Given two transactions $\tsid_1, \tsid_2$, we say that $\tsid_2$ write-read depends 
%on $\tsid_1$ via $\key_{\vx}$, written $\tsid_1 \xrightarrow{\RF(\key_{\vx})_{\hh}} \tsid_2$, 
