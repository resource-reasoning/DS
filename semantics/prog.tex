...
We parametrise the execution test in the operational semantics of commands (\figref{fig:thread_semantics}), while we will give examples of different execution tests in \secref{sec:cmexamples}. 
...
Given the set of key-value stores \( \hh \in \HisHeaps \) (\defref{def:his_heap}), fingerprints \( \opset \in \Opsets \) (\defref{def:ops}) and views \( \vi, \vi' \in \Views \) (\defref{def:views}), \emph{execution tests} \( \como \in \Como \) is a set of quadruples in the form of \( ( \hh, \vi, \opset, \vi' ) \):
\[
    \begin{rclarray}
...
The transaction code \( \trans \) is executed locally given an initial snapshot \( \sn = \clpsHH{\hh, \vi''}\) (\defref{def:snapshot}) decided by the current state of key-value store \( \mkvs \) and the local view \( \vi'' \).
...
After local execution via the semantics for transactions (\figref{fig:transaction_semantics}), we propagate the stack \( \stk' \) and more importantly obtain the fingerprints \( \opset \), while the snapshot \( \sn' \) will be throw away.
...
\begin{mathpar}
    \inferrule[\rl{PCommit}]{%
        ...
        \\ \h = \clpsHH{\hh,\vi''}
        \\ (\stk, \h, \unitO), \trans \ \toL^{*} \  (\stk', \h', \opset) , \pskip
        \\\\ \hh' = \func{updateMKVS}{\hh, \vi'', \txid, \opset}  
        \\ \vi' \geq \func{updateView}{\hh', \vi'', \opset}
        \\ (\hh, \vi) \csat \opset : \vi'
    }{%
...
%Given the set of execution tests \( \ConsisModels \) (\defref{def:consistency-models}) and key-value stores \(\HisHeaps\) (\defref{def:mkvs}), 
