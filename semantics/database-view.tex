...
In this paper we focus on kv-stores under consistency models where transactions enjoy 
...
Thus, a \emph{well-formed} kv-store $\hh$ requires that:
\textbf{(i)} for each key $\ke \in \dom(\hh)$, $\hh(\ke, 0) = (\val_0, \txid_0, \stub)$, where $\val_0$ is a default value from $\Val$;
...
\textbf{(iv)} all the reads of a transaction precede the writes of the same transaction: $\fora{ \ke \in \dom(\hh)}
