...
Given a version $\ver = (\val, \txid, \txidset)$, we write $\valueOf(\ver)$ for its value ($n$), $\WTx(\ver)$ for its writer ($\txid$), and $\RTx(\ver)$ for its readers ($\txidset$). 
...
An MKVS is \emph{well-formed}, written $\pred{wfMKVS}{\hh}$, iff 
