...
a set of \emph{values} $\Val = \{\val, \val', \cdots\}$ which for simplicity we instantiate to be 
$\Nat \uplus \Keys$, a set of clients $\Clients = \{\cl, \cl',\cdots\}$. 
...
Specifically, we let $\PO = \{ (\txid, \txid') \mid \exists \cl, n,m. \txid = \txid_{\cl}^{n} \wedge 
\txid' = \txid_{\cl}^{m} \wedge n < m\}$; $(\txid, \txid') \in \PO$ means that 
...
Intuitively, a version $\ver = (\val, \txid, \txidset)$ consists of a value $\val = \value(\ver)$, and the meta-data of the transactions 
...
A key-value store, or \emph{kv-store} in short, is then a mapping from keys to the list of versions that have been installed by 
...
example is given by the kv-store $\hh$ of Figure \cref{fig:hheap-a} (ignore for the moment 
the vertical lines labelled $\client_1$ and $\client_2$). 
$\hh$ comprises two keys \( \ke_1\) and \( \ke_2 \), 
each of which with two versions carrying values $\val_0$ and $\val_1$, and $\val'_0$ and $\val'_1$, respectively.
...
key $\ke \in \dom(\hh)$, $\hh(\ke, 0) = (\val_0, \txid_0, \_)$ for some initial value $v_0$,
...
such transactions have been invoked: $\forall \ke \in \dom(\hh), \cl \in \Clients.\;\forall i,j; 0 \leq i < j < \lvert \hh(\ke) \rvert. 
\forall n, m \geq 0.\; (\WTx(\ke(k,i)) = \txid_{\cl}^{n} \wedge \WTx(\ke(k,j)) = \txid_{\cl}^{m}) \implies n < m$.
We call kv-stores that satisfy the conditions \textbf{(i-v)} \emph{well-formed}. Henceforth, we always assume that kv-stores 
...
\path[->]
(t0.north) edge[bend left=70] node[above, yshift=0.5pt, xshift=-1pt, pos=0.3] {$\RF(\ke_2), \VO(\ke_1)$} (t1.west)
(t0.south) edge[bend right=70] node[below, yshift=-1pt, xshift=-1pt, pos=0.3] {$\RF(\ke_1), \VO(\ke_2)$} (t2.west)
...
\end{tikzpicture}
\end{center}
\caption{The dependency graph induced by $\hh$. {\color{red} Ignore this for the moment, 
as I did not put any content on dependency graphs in the end.}}
\label{fig:hheap-b}
\end{halfsubfig} \\
\end{tabular}
\end{center}
...
as $\Views(\hh)$, and we let $\Views = \bigcup_{\hh \in \HisHeaps} 
\Views(\hh)$.

A \emph{configuration} $\conf$ is a pair $(\hh, \viewFun)$, where $\viewFun: 
\Clients \parfinfun \Views(\hh)$. The configuration $\conf = (\hh, \_)$ is 
...
keeps track of the most recent version of $\hh$ that $\cl$ accesses when executing a 
transaction. The constraint of \cref{eq:view.atomic} establishes that clients observes 
...
Dependency graphs are a formalism introduced by Adya \cite{adya} to specify 
consistency models. Dependency graphs consist of transactions as nodes, 
each of which is labelled with the respective fingerprint, and labelled edges 
between transactions that specify how information flows within a computation. 
Specifically, a transaction $\txid$ may read a version for a key $\ke$ that has been written by another transaction $\txid'$ 
(\emph{write-read dependency}), overwrite a version  of $\ke$ written by $\txid'$, or 
...
\begin{definition}
Let $\hh$ be a kv-store. We define $\graphof(\hh) = (\TtoOp{T}_{\hh}, \RF_{\hh}, 
\VO_{\hh}, \AD_{\hh})$ as follows: 
\begin{itemize}
...
\item $\txid \xrightarrow{\RF(\ke)} \txid'$ if and only if there exists an index $i: 0 < i \leq \lvert \hh(\ke) \rvert -1$ 
such that $\txid = \WTx(\hh(\ke, i))$, and $\txid' \in \RTx(\hh(\ke, i))$, 
\item $\txid \xrightarrow{\VO(\ke)} \txid'$ if and only if there exist two indexes $i,j$: $0 < i < j \leq \lvert \hh(\ke) \rvert - 1$ 
such that $\txid = \WTx(\ke, i)$, $\txid' = \WTx(\ke, j)$, 
\item $\txid \xrightarrow{\AD(\ke)} \txid'$ if and only if there exist two indexes $i,j$: $0 < i < j \leq \lvert \hh(\ke) \rvert -1$ 
such that $\txid \in \RTx(\ke, i)$ and $\txid' = \WTx(\ke, j)$.
\end{itemize}
\end{definition}

\begin{theorem}
The function $\graphof(\_)$ is a bijection from kv-stores and well-formed dependency graphs.
\end{theorem}
