...
\emph{A version} $\ver = (\val, \txid, \txidset)$ consists of a value $\val = \value(\ver)$, and the meta-data of the transactions 
...
Given a version $\ver = (\val, \txid, \txidset)$, we write $\valueOf(\ver)$ for its value ($n$),
...
\emph{A multi-version key-value store}, or \emph{kv-store} in short, 
is then a mapping from keys to the lists of versions. 
...
example is given by the \cref{fig:hheap-a} (ignore for the moment 
the vertical lines labelled $\client_1$ and $\client_2$). 
The kv-store $\hh$ in \cref{fig:hheap-a} comprises two keys \( \ke_1\) and \( \ke_2 \), 
each of which is associated two versions carrying values $\val_0$ and $\val_1$, and $\val'_0$ and $\val'_1$, respectively.
...
In this paper we focus on kv-stores under consistency model where transactions enjoy 
\emph{atomic visibility} \cite{framework-concur}. 
We also assume that kv-stores are initialised by the special transaction identifier $\tsid_0$, \ie 
install the initial version of each key. Thus, a \emph{well-formed} kv-store $\hh$ requires that:
\textbf{(i)} for each key $\ke \in \dom(\hh)$, $\hh(\ke, 0) = (\val_0, \txid_0, \stub)$ for some initial value $v_0$;
\textbf{(ii)} transactions never write more than one version per key $\fora{\ke \in \dom(\hh)} \fora{ i,j : 0 \leq i, j < \lvert \hh(\ke) \rvert}
\WTx(\ke, i) = \WTx(\ke, j) \implies i = j$;
\textbf{(iii)} transactions never read different versions 
for the same key $\fora{ \ke \in \dom(\hh)} \fora{ i,j : 0 \leq i, j < \lvert \hh(\ke) \rvert}
\RTx(\ke, i) \cap \RTx(\ke, j) \neq \emptyset \implies i = j$;
\textbf{(iv)} all the reads of a transactions precede the writes of the same transaction: $\fora{ \ke \in \dom(\hh)}
...
\emph{Dependency graphs} are introduced by Adya \cite{adya} to specify 
