...
\azalea{I have paraphrased quite a bit here. Please make sure you are happy with this.}
%\ac{
...
We model the state of a system using \emph{multi-version key-value stores} (MKVSs). An MKVS is a map from keys to lists of versions (values).
...
\azalea{Why $\hh_1$ for the store and not $\hh$? Also I thought the versions are indexed from $0$ in which case index $2$ does not make sense here?}
...
An MKVS tracks the the overall state of the system; however, different \emph{clients} may observe different versions of the same key. 
To model this, we introduce the notion of \emph{views} and \emph{configurations} (\cref{def:views}). 
...
\emph{A configuration} comprises an MKVS, and the views associated with clients.
...
\]
%
\azalea{We need a symbol for this to fill the ???? above. Also ???? below.}
...
When a client executes a transaction, it extracts a \emph{snapshot} via the \( \func{snapshot}{\mkvs, \vi} \) function, extracting the values corresponding to the versions indexed by the view \( \vi \) (\cref{def:snapshot}).
...
\azalea{Before in MKVSs we had values drawn from $\Nat$ in \cref{def:mkvs}. Now we use $\Val$. I think you mean to use $\Val$ in both places? }
