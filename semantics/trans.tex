...
\emph{A program} \( \prog \) contains fixed numbers of clients associated with their \emph{commands} (\cref{def:language}).
...
The arithmetic expression is interpreted to a value (\cref{def:eval-expr}) via a stack \( \stk \in \Stacks \) (\cref{def:stacks}) and there is no side-effect.
...
\label{def:stacks}
A \emph{stack} is a partial finite function from variables \( \Vars \) (\cref{def:language}) to program values \( \Val \) (\cref{def:program_values}): \( \stk \in \Stacks \defeq \Vars \parfinfun \Val \).
\end{definition}

\begin{definition}[Evaluation of expression]
\label{def:eval-expr}
Given a stack $\stk \in \Stacks$ (\cref{def:stacks}), the \emph{arithmetic expression evaluation} function, $\evalE[(.)]{.}:\Expressions \times \Stacks \parfun \Val$, is defined inductively over the structure of expressions as follows: 
%
\[
    \begin{rclarray}
        \evalE{\val} & \defeq & \val \\
