...
\begin{proposition}
Suppose that $(\hh, \viewFun, \thdenv), \prog \toL{(\cl, \vi, \opset)}_{\ET_{\top}} (\hh', 
...
\[
(\aexec, \thdenv), \prog \toA{(\cl, \T \cup \Tx(\hh, \vi)}_{(\RP_{\LWW}, \emptyset)}. 
(\aexec', \thdenv'), \prog'
\]
\end{proposition}
...
we have that 
\[
(\hh_{\aexec}, \viewFun\rmto{\cl}{\vi}, \thdenv), \prog \toA{(\cl, \getView(\aexec, \T)}_{\ET_{\top}} (\hh_{\aexec'}, \viewFun, \thdenv'), \prog'.
\]
\end{proposition}
...
Suppose that we have an execution test $\ET$ that precisely captures an axiomatic specification 
$(\RP_{\LWW}, \Ax)$: that is 
\begin{itemize}
...
only contains read-only transactions, such that for any $\A \in \Ax.\;\A(\extend(\aexec, \_, \T, \opset)) \times \subseteq \T$, 
