...
\[
    \inferrule[\rl{ACommit}]{
        ...
        \cl \vdash ( \aexec, \stk ), \ptrans{\trans} \ \toA{}_{(\RP, \Ax)} \ ( \aexec', \stk' ) , \pskip
    }
\]
\[
    \inferrule[\rl{ASingleThread}]{%
         \cl \vdash ( \aexec, \thdenv(\thid) ) , \prog(\thid), \ \toA{}_{(\RP, \Ax)} \  ( \aexec', \stk' ) , \cmd'  
    }{%
         (\aexec, \thdenv ), \prog  \ \toA{}_{(\RP, \Ax)} \  ( \aexec', \thdenv\rmto{\thid}{\stk'} ) , \prog\rmto{\thid}{\cmd'} ) 
    }
\]
\hrulefill
...
$(\aexec, \Env, \prog) \toA_{(\RP, \Ax)} (\aexec', \Env', \prog')$. 
...
anarchic one $\anarchic$. Because the snapshot in which client $\cl$ executes the 
...
\begin{definition}
The semantics o a program $\prog$ under a consistency model with axiomatic specification 
$(\RP, \Ax)$ is given by 
\[
\interpr{prog}_{(\RP, \Ax)} = \{ \aexec \mid (\aexec_{0}, \Env_{0}, \prog) \toA{}_{(\RP, \Ax)}^{\ast} (\aexec, \_, \prog_{f}), 
\]
