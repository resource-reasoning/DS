...
\[
    \inferrule[\rl{PCommit}]{
		...
		\forall A \in \Ax.\;\A(\aexec') \cap (\T_{\aexec} \times \{\txid\}) \subseteq \T
    }{
        \cl \vdash ( \aexec, \stk ), \ptrans{\trans} \ \toT{}_{(\RP, \Ax)} \ ( \aexec', \stk' ) , \pskip
    }
\]
...
a consistency model for specified axiomatically, via a set of 
axioms $\Ax$ over abstract executions.
That is, for any abstract execution $\aexec$ that satisfies 
the axioms $\Ax$, then $\KVtrace(\ET_{\top}, \aexec) \cap \CMs(\ET) \neq \emptyset$; 
and for any $\tr \in \CMs(\ET)$, there exists an abstract execution 
$\aexec \in \aeset(\tr)$ that satisfies the axioms $\Ax$. 
...
argument, define a Galois Connection between the set of abstract executions in the axiomatic specification 
of the consistency model, and the set of kv-stores generated by such a consistency model.}
...
$\Ax$. In this sense, our operational semantics is adequate.

To tackle this question, we need to define what is the set of all possible behaviours 
that can be produced by a program $\prog$ under a given consistency model, for 
which an axiomatic specification is known. This in turn requires addressing two orthogonal 
...
models, and a resolution policy is part of the specification of a consistency model. 
