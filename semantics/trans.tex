...
such that for any $\cl, \cl'$ such that $\cl' \neq \cl$ we have that 
...
there exist $\T \subseteq \T_{\aexec}$ and $\T' \subseteq \T_{\aexec} \cup \{\txid\}$ 
\begin{itemize}
\item $\getView(\aexec, \T) = \vi$, $I(\aexec, \cl) \subseteq \T$, 
\item $\forall \A \in \Ax. \{\txid' \mid (\txid', \txid) \in \A(\aexec')\} \subseteq \T$, 
where $\aexec' = \extend(\aexec, \txid, \T, \opset)$, 
\item $\getView(\aexec', \T')) = \vi'$, and $I(\aexec', \cl) \subseteq \T'$. 
\end{itemize}
\end{definition}
...
\begin{definition}
Aan execution test $\ET$ precisely captures an axiomatic specification 
