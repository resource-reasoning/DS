...
\[
\begin{rclarray}
    \fp \addO (\otR, \addr, \val)  & \defeq  &
    \begin{cases}
        \fp \cup \{(\otR, \addr, \val)\} & \text{if } \for{l, v'} (l, \addr, v') \notin \fp \\
        \fp &  \text{otherwise} \\
    \end{cases} 
    	\qqquad 
    \fp \addO \emptyop  \defeq  \fp  \\
    \fp \addO (\otW, \addr, \val) & \defeq & 
    \left( \fp \setminus \setcomp{(\otW, \addr, v')}{v' \in \Val} \right) \cup\ \{(\otW, \addr, \val)\} 
%    \fp \addO \emptyop & \defeq & \fp \\
\end{rclarray}
\]
%
...
\[
    \inferrule[\rl{PCommit}]{%
        \vi \orderVI  \vi''
        \qquad \h = \clpsHH{\mkvs,\vi''}
        \qquad \txid \in \nextTxId(\cl, \mkvs)
        \\\\ (\stk, \h, \emptyset), \trans \ \toL^{*} \  (\stk', \stub,  \fp) , \pskip
        \\ \ET \vdash (\mkvs, \vi'') \triangleright \fp : \vi'
    }{%
        \cl \vdash ( \mkvs, \vi, \stk ), \ptrans{\trans} \ \toT{(\cl, \vi'', \fp)}_{\ET} \ ( \updateKV(\mkvs, \vi, \txid, \fp),\vi', \stk' ) , \pskip
    }
\]
\[
...
Transactional rules are of the form $(\stk, \h, \fp), \toL (\stk', \h', \fp'), \trans'$, 
...
to $\stk'$ and $\h'$, respectively, with the extended fingerprint $\fp'$, and continuation $\trans'$.
...
$\func{fp}{\stk, \h, \transpri} = \op$, and the fingerprint recorded so far is given by $\fp$, 
then executing $\transpri$ transforms $(\stk, \h)$ to $(\stk', \h')$, with the fingerprint updated to $\fp \addO \op$ (to incorporate the effect of $\transpri$), with continuation $\pskip$. 
...
Given the new view $\vi''$, the transaction proceeds by obtaining a snapshot $\sn$ of the kv-store $\mkvs$, and executing $\trans$ locally to completion ($\pskip$), updating the stack to $\stack'$, while accumulating the fingerprint $\fp$. Note that the resulting snapshot is ignored (denoted by $\stub$) as the effect of the transaction is recorded in the fingerprint $\fp$. 
%
...
the last premise requires that the updates be permitted by the execution test $\ET$, \ie \( (\mkvs, \vi'') \etto \fp : \vi'\).
...
\begin{rclarray}         
    ...
    \updateKV(\mkvs, \vi, \txid, \fp \uplus \Set{(\otR, \ke, \stub)}) & \defeq &  
    \begin{array}[t]{@{}l}
        \texttt{let } (\val, \txid', \txidset) = \mkvs(\ke, \max_{<}(\vi(\ke))), \\
        \vilist = \mkvs(\ke)\rmto{\max_{<}(\vi(\ke))}{(\val, \txid', \txidset \uplus \{ t \})}\\
        \quad \texttt{in } \updateKV(\mkvs\rmto{\ke}{\vilist}, \vi, \txid, \fp)
    \end{array} \\
    \updateKV(\mkvs, \vi, \txid, \fp \uplus \Set{(\otW, \ke, \val)} )& \defeq &  
    \begin{array}[t]{@{}l}
        \texttt{let } \mkvs' = \mkvs\rmto{\ke}{ ( \mkvs(\ke) \lcat (\val, \txid, \emptyset) ) } \\
...
%
%
For every read operation $(\otR, \ke, \stub)$ in fingerprint $\fp$,
since a transaction reads the values of keys from 
the atomic snapshot determined by the view of the client, 
the version read by $\txid$ for key $\ke$ corresponds to $\mkvs(\ke, \max_{<}(\vi(\ke)))$.
As such, to commit the transaction, 
the reader set of the version $\mkvs(\ke, \max_{<}(\vi(\ke)))$ is extended with a new reader $\txid_\cl^n$.
Similarly, for every write operation $(\otW, \ke, \val)$ in fingerprint $\fp$, 
...
$\ke$ and an index $i=0,\cdots, \lvert \mkvs(\ke) \rvert -1$ such that $\txid \in \{\WTx(\mkvs(\ke, i)\} \cup \RTx(\mkvs(\ke, i))$.}, 
