\subsection{Operational Semantics of Key-value Stores}

In this section we define a simple programming language for client programs interacting with a key-value store.
Clients may only interact with the key-value store by issuing read and write requests via transactions. 
For simplicity, we abstract away from aborting transactions:
rather than assuming that a transaction may abort due to a violation of the consistency guarantees given by the key-value store,
we only allow the execution of a transaction when its effects are guaranteed to not violate the consistency model. 
This emulates the setting in which clients always restart a transaction if it aborts.


\subsubsection{Programming Language}

\emph{A program} \( \prog \) consists of a finite number of clients,
where each client is associated with a unique identifier \( \thid \in \ThreadID \), 
and executes a sequential \emph{command}. 
The syntax of commands is given as the following:
\[
\begin{array}{c}
   \cmdpri ::=  
   		\pass{\txvar}{\expr} \mid 
 ...
 \end{array} 
\]
Primitive commands that can be invoked by clients are split into transactional ones,
corresponding to the $\transpri$ clause,
and which is used for reading and writing keys of the key-value store; 
and non-transactional ones, corresponding to the $\cmdpri$ clause, 
and which are used for computations over client-local data. 
Transactional primitive commands can be invoked only within the boundaries of transactions 
(corresponding to $\ptrans{\trans}$) executed by clients in commands;
non-transactional primitive commands can be invoked without restrictions.
For clarity, we often write \( \cmd_{1}\ppar \dots \ppar \cmd_{n}\) as a syntactic sugar 
for a program \( \prog \) with $n$ implicit clients associated with identifiers, 
$\thid_1 \dots \thid_n$, where each client $\thid_i$ executes the command $\cmd_i$: 
\( \prog = \Set{\thid_{1} \mapsto \cmd_{1}, \dots, \thid_{n} \mapsto \cmd_{n}  }\).

As it is usual, we assume a language of expressions built from \emph{values},
which can be either keys or natural number and are ranged over by $\val, \val', \cdots$; 
and client-local variables from a set $\Vars$, which are ranged over by $\pvar{x}, \pvar{y}, \cdots$. 
The evaluation $\evalE{\expr}$ of  expression $\expr$ is parametric in the (client-local) \emph{stack} 
$\stk \in \Stacks \defeq \Vars \to \Val$, which is a function from variables to values. 
\[
\expr  ::= 
        \val \mid
        \var \mid
...
\]
\hrulefill
\caption{Selected Rules of the Operational Semantics}
...
Transactional commands are equipped with a transformer relation 
$\toLTS{\transpri} \subseteq (\Stacks \times \Heaps) \times (\Stacks \times \Heaps)$ (\cref{fig:semantics}).
The transformer $(\stk, \h)  \toLTS{\transpri} (\stk, \h')$ describes how the stack of a client 
evolve upon executing the primitive command $\transpri$.
\[
...
To extract the overall effect of a transaction,
we introduce a novel \emph{fingerprint function} \( \funcn{fp} \)
and \emph{fingerprint composition operator} \( \addO \) (\cref{fig:semantics}).
The fingerprint function is defined as 
$\func{fp}{\stub} : \Stacks \times \Heaps \times \cmdpriset \rightarrow \Ops_{\varepsilon}$ .
The function $\funcn{fp}$ identifies, for each primitive command $\transpri$,
the operation $\op \in \Ops \cup {\varepsilon}$ when a client executes $\transpri$. 
Note that non-transactional primitive commands are associated with the empty operation $\varepsilon$,
because they only read or write the local stack.
\[
\begin{array}{rcl @{\quad} rcl}
...
\end{array}
\]
The fingerprint composition operator \( \addO \) enforces transactions to record
the first read (before a write) and the last write for each key. 
This is in line with our assumption that transactions read from an atomic snapshot of the key-value store.
Because only the first read before a write fetches its value from the key-value store,
and clients observe either none or all the effects of a transaction,
which means only the last write of each key is committed.
\[
\begin{rclarray}
...
\ifTechReport%
In \cref{fig:semantics} we illustrate rules of the operational semantics of transactions. 
\else%
In \cref{fig:semantics} we illustrate few selected rules of the operational semantics of transactions. 
The reader can refer to \cref{sec:full-semantics} for the complete set of rules of the semantics of transactions. 
\fi%
The \rl{TPrimitive} is the most important rule which models the execution of primitive transactional.
The transaction maintains a client-local-stack $\stk$, 
a snapshot $\h$ which is used to fetch the value of read operations, 
and a set of operations $\opset$ to be applied to the key-value store upon committing the transaction. 
By executing a primitive command $\transpri$, 
the first two components are updated to $\stk'$ and $\h'$, respectively,
according to the transformer of the command. 
The set of operations associated with the transaction is also updated 
so to include the new operation $\op$ associated with $\transpri$ via the operator $\addO$. 

\subsubsection{Operational Semantics of Programs}

First we introduce a function \( \updateKV \) 
...
the operator $\lcat$ concatenates two lists. 
Given a list of versions $\vilist = \ver_0\cdots \ver_n$ and $i=0,\cdots,n$, 
$\vilist\rmto{i}{\ver}$ denotes the list of versions 
...
the function $\updateKV$ is well defined.
\sx{well defined mean??? Confluence?}
Furthermore, a client \( \cl \) always picks a greater transaction identifier,
via function $\nextTxId(\cl, \hh) \defeq \Setcon{\txid_{\cl}^{n}}{\fora{m} \txid_{\cl}^{m} \in \hh \implies m < n }$.
...
because a transaction reads the values of keys from 
...
Thus to commit the transaction, 
the readers set of the version $\hh(\ke, \max_{<}(\vi(\ke)))$ is extended with a new reader $\txid_\cl^n$.
...
denoting that $\txid_\cl^n$ is the writer of this version which has no readers yet. 
...
\ifTechReport%
In \cref{fig:semantics} we illustrate rules of the operational semantics of programs. 
\else%
In \cref{fig:semantics} we illustrate few selected rules of the operational semantics of programs. 
The reader can refer to \cref{sec:full-semantics} for the complete set of rules of the semantics of programs. 
\fi%
...
The operational semantics of commands are parametrised by consistency model via \emph{execution test} \( \ET \).
The execution test induces a set of transfers on key-value stores allowed by the consistency model.
For technical reasons, the semantics are labelled either as $(\cl, \iota)$,
meaning that some client-local computation that did not require an access to the key-value store,
for example, primitive commands \( \cmdpri \), or as $(\cl, \vi, \opset)$, 
meaning that client $\cl$ commits a transaction with fingerprint $\opset$ under the view $\vi$.
 
The \rl{PCommit} rule (\cref{fig:semantics}) models the execution of a transaction $\ptrans{\trans}$, under the execution test $\ET$. 
Given a client-local stack $\stk$, a key-value store $\hh$ and a view $\vi$,
first it advances the view to a new one \( \vi'' \),
takes the snapshot $\h$ of $\hh$ induced by the new view $\vi''$,
executes the transaction command $\trans$ using $\stk$ and $\h$ as the initial state,
and calculates the fingerprint $\opset$ for the transaction.
Before committing, it checks if it is allowed by the execution test \( \ET \).
The predicate \( \ET \vdash (\hh, \vi'') \triangleright \opset : \vi' \) is called execution test.
Intuitively, it means that under a specific execution test \( \ET \),
if the key-value store is \( \mkvs \) and view is \( \vi'' \),
it is allowed to commit a transaction with fingerprint \( \f \) via \( \updateKV \) function,
and after the commit, the view must also be updated to $\vi'$.
%then the effects of $\opset$ are committed to $\hh$, using the function $\updateKV$ from \cref{sec:execution.tests}. 
The formal definition and examples of execution tests are presented in \cref{sec:execution-tests}.
...
The last rule, \rl{PSingleThread}, models the execution of a command bound to a client in a program. 
The \( \thdenv \) is a partial function from clients \( \ClientID \) to stacks \( \Stacks \).
We assume \( \dom(\thdenv) = \dom(\prog) \).
