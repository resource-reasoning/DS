...
by the \cref{def:et_sound}, it remains to prove that for any set of read-only transaction \( \T_{\rd} \),
there exists a set of read-only transactions \( \T'_\rd \) such that
\[
    \begin{array}{@{}l@{}}
        \fora{ \aexec', \txid, \txid', \ke } 
        \aexec' = \extend(\aexec, \txid, \Tx(\mkvs, \vi) \cup \T_\rd \cup \T'_\rd, \f ) 
        \land \txid', \txid \in \WTa(\aexec', \ke) \land  \txid' \toEdge{\AR_{\aexec'}} \txid  \\
        \qquad {} \implies \txid' \in \Tx(\mkvs, \vi) \cup \T_\rd
    \end{array}
\]
Since the abstract execution satisfies \( \AR \in \VIS \), 
Since \( \Tx(\mkvs, \vi)  \) contains all transactions that write at least a key, 
we can pick a \( \T'_\rd \) such that \( \Tx(\mkvs, \vi) \cup \T_\rd \cup \T'_\rd = \T_\aexec\),
...
so \( \fora{\ke, i} 0 \leq i \leq \abs{\mkvs_{\cut(\aexec, i-1)}(\ke)} \).
