...
The specification for \( \SI \) is \( (\RP_{\LWW}, \Set{\lambda \aexec. \AR_\aexec ; \VIS_\aexec, \lambda \aexec \ldotp \SO_\aexec }) \).
...
%Now let assume there exists one more transaction from the same client, 
%says \( \txid \) such that \( \txid_i \toEdge{\SO_\aexec} \txid\).
%Let \( \vi' = \getView(\aexec, (\AR_\aexec^{-1})?(\txid_{i}) \cap \VIS_{\aexec}^{-1}(\txid) ) \).
%Note that \( \SO_\aexec \in \VIS_\aexec \)
%so \( \left( (\SO_{\aexec} \cup \WW_{\aexec} \cup \WR_{\aexec} ) ; \RW_{\aexec}? \right) ; \SO_{\aexec} \subseteq \VIS_\aexec \).
%Because \( \txid_i \toEdge{\SO_\aexec} \txid\), we have the following:
%\[
    %\left( (\SO_{\aexec} \cup \WW_{\aexec} \cup \WR_{\aexec} ) ; \RW_{\aexec}? \right)^{-1}(\txid_i) \subseteq  (\AR_\aexec^{-1})?(\txid_{i}) \cap \VIS_{\aexec}^{-1}(\txid)
%\]
