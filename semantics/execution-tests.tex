...
fingerprint $\opset$, we define the relation $\xrightarrowtriangle{cl, \opset, \vi}_{\ET}
\subseteq \Confs \times \Confs$ by letting $(\hh_1, \viewFun_1) \xrightarrowtriangle{cl,\opset, \vi}_{\ET} 
(\hh_2, \viewFun_2)$ whenever $\ET \vdash (\hh_1, \vi') \triangleright \opset: \vi$ for some 
$\vi': \viewFun_1(\cl) \viewleq \vi'$, $\hh_2 \in \updateKV(\hh_1, \vi', \cl, \opset)$ and 
$\viewFun_2 = \viewFun_1\rmto{\cl}{\vi}$.

The set of configurations induced by $\ET$ is given by 
\[
\Confs(\ET) = \{ \conf \mid \exists \conf_0. \conf_0 \text{ is initial } \wedge \conf_0 \xrightarrowtriangle{\_}_{\ET} \cdots \xrightarrowtriangle{\_}_{\ET} \conf \}.
\]
The consistency model induced by $\ET$ is given by 
\[ 
\CMs(\ET) = \{ \hh \mid (\hh, \_) \in \Confs(\ET) \}.
\]
...
$\SI$ & $\vi'(\ke) = \lvert \updateKV(\hh, \vi, \_, \opset) \rvert - 1 \wedge \big( i < \vi(\ke) \wedge \vi(\ke') < j \implies $\\
& $ \RTx(\hh(\ke, i)) \cap (\RTx(\hh(\ke', j)) \cup \{\WTx(\hh(\ke', j))\} \neq \emptyset \big) \wedge {}$\\
& $(\otW, \ke,  \_) \in \opset \implies \vi(\ke) = \lvert \hh(\ke) \rvert - 1$\\
\hline
...
\begin{pgfonlayer}{background}
\node(b) [background, fit=(locx) (versionx) (locw) (versionw), inner sep = \pgflinewidth] {};
\end{pgfonlayer}
\end{tikzpicture}
\end{wrapfigure}
