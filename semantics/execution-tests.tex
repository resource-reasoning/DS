...
Tuples from an execution test satisfies that
...
$\hh(\ke) = (\val_0, \txid_0, \stub)$. A configuration $\conf$ is 
initial  if $\conf = (\hh, \viewFun)$ for some initial $\hh$.
\ac{Initial kv-stores should have been defined before.}
Given an execution test $\ET$, the set of all configurations 
induced by $\ET$ is the smallest set $\mathsf{Configs}(\ET)$ 
such that:
\[
\begin{array}{l}
\fora{\conf} \conf \text{ is initial} \implies \conf \in \mathsf{Configs}(\ET)\\
\fora{\hh, \cl, \vi'} (\hh, \viewFun) \in \mathsf{Configs}(\ET) \wedge \viewFun(\cl) \viewleq \vi' \implies (\hh, \viewFun\rmto{\cl}{\vi'}) \in \mathsf{Configs}(\ET)\\
\fora{ \hh, \cl, \opset, \vi, \vi'} \vi = \viewFun(\cl) \wedge \ET \vdash (\hh, \vi) \csat \opset : \vi' \implies {}\\
\hfill \fora{ \hh' \in \updateKV(\hh, \vi, \cl, \opset)} (\hh', \viewFun\rmto{\cl}{\vi'}) \in \mathsf{Configs}(\ET) 
\end{array}
\]
The \emph{consistency model} induced by $\ET$ is defined as:
\(
\CMs(\ET) = \Setcon{ \hh}{ \exsts{\viewFun} (\hh, \viewFun) \in \mathsf{Configs}(\ET) }
\).
\end{definition}
