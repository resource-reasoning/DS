...
\sx{In the text, it mentions non-empty fingerprints. I think \( \opset_i \neq unitO \) }.
...
%\sx{If the view is not exposed in the ET-reduction, the composition will fail. 
    %I agree with you to assume no blind write in the definition of execution test.}
...
$\SI$ & $\Setcon{(\mkvs, \vi, \f, \vi')}{\dagger}  \cap \MRd \cap \RYW $\\
...
\[
    \begin{rclarray}
        ...
        \dagger & \equiv &
        \begin{array}[t]{@{}l@{}}
            \left( (\otW, \ke,  \stub) \in \opset \implies \vi(\ke) = \lvert \hh(\ke) \rvert - 1 \right) \land 
