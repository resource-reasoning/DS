...
%We have already argued that, in contrast to abstract executions, kv-stores do not contain 
%any information about the total order in which transactions have been executed.
...
%\[
%\begin{array}{lcl}
%(\aexec_{(\hh_0, \viewFun_0)}, \visibleTx_{(\hh_0, \viewFun_0)} &=& (\aexec_0, \lambda \cl.(\Tx(\hh, \viewFun(\cl))\\
%(\aexec_{\tr \xrightarrow{(\cl, \varepsilon)}_{\ET_{\top}} (\hh, \viewFun)}, \visibleTx_{\tr \xrightarrow{(\cl, \varepsilon)}_{\ET_{\top}} (\hh, \viewFun)} &=& 
%(\aexec_{\tr}, \visibleTx_{\tr}\rmto{\cl}{\Tx(\hh)}
%\end{array}
%\]
\begin{itemize}
...
if $\aexec \in \aeset(\tr)$, $\opset \neq \emptyset$, and $\T = \Tx(\hh, \viewFun'(\cl)) \cup \T'$ is a set of transactions such that 
$\forall \txid' \in \T'.\; \forall \ke, \val.(\otW, \ke, \val) \notin_{\aexec} \txid'$, and $\txid$ is a transaction appearing 
in $\lastConf(\tr)$ but not in $\hh$, then  
$\extend(\aexec, \txid, \T, \opset) \in \aeset\left(\tr \xrightarrowtriangle{(\cl, \opset)}_{\ET_{\top}} (\hh, \viewFun) \right)$.
\end{itemize}
%\[
%\begin{array}{lcl}
%\aexec_{(\hh_0, \viewFun_0)} &\ni& \aexec_0\\
