...
--------in next paragraph need to mention sessions, because we mantion
in last paragraph-------
...
to  analysise client applications.
...
the 
versions and is inspired by the views in the  C11 operational semantics
in~\cite{promises}. Like \cite{seebelieve}, our  operational semantics  is parametric in the notion of {\em
  execution test},  which determines if a client with a given view is
...
snapshot isolation (\SI) and serialisability (\SER). Since we focus on
...
Because our focus is on protocols and applications employed  by distributed databases, 
  most of which  guarantee atomic visibility, we do not find this constraint to be a severe limitation.
...
\cite{seebelieve}. 
We  provide ((Seciton\ref{5} and appendix) a general proof technique for proving the correspondence between 
our  execution tests and the axiomatic specifications of consistency
models using abstract executions (\cref{sec:other_formalisms}).  Using
this technique, we are able to demonstrate 
that our consistency models are equivalent to the  declarative definitions of
consistency model  for
abstract execution graphs~\cite{.}, and hence for  dependency
graphs~\cite{.}. 
...
abstract execution graphs~\cite{...}; clients are analysed using dependency
graphs~\cite{...}; and equivalence results are used to move between
the two~\cite{....}. Using our semantics, we establish the correctness of two database protocols: 
the COPS protocol for the fully replicated kv-stores~\cite{cops}  
which 
satisfies CC;  and the Clock-SI   propocol for
...
robust against \(\PSI\),
whereas without sessions \cite{somepeople} show that 
