...
givem by the standard ANSI/SQL isolation levels~\cite{.}. Their
...
isolation (SI)~\cite{SI}, but not  weaker ones 
 such as parallel snapshot isolation (\PSI)  \cite{PSI} and causal consistency (\CC)~\cite{.} 
 which are important 
...
We also also identify 
a new consistency model  that sits between \PSI\; and \SI\; and retains good properties of both. Since we focus on
...
%
  ...
  We prove that our operational definitions of consistency model for
  ...
  executions (\cref{sec:other_formalisms} and appendix).  Using this
  technique, we
%are able to 
prove 
...
kv-stores~\cite{clocksi} which satisfies \SI
(\ref{appendixorsupplementarymaterial}). 

% We are only at the beginning of exploring the
%usefulness of our operational semantics. 
...
%
%Our results linking our
%operational semantics using kv-stores with the declarative semantics 
%using execution graphs means that we can seemlessly move between
%formalisms
