...
%
\path[very thick,->] 
 (ad1to2s) edge[bend right=30] node[left] {$\AD$} (ad1to2e)
 (ad2to1s) edge[bend right=30] node[right] {$\AD$} (ad2to1e);
%
\path (t1.north) + (3,2) node (T1) {$T$};
\path (t2.south) + (3,-2) node (T2) {$S$};
\end{pgfonlayer}
\end{tikzpicture}
\end{center}
...
%
\path[very thick,->] 
  (t1.east) edge node[above] {$\textbf{\textsf{SO}}$} (t2.west)
 (t3.north) edge node[right] {$\textbf{\textsf{WR}}$} (t2.south)
 (t2.east) edge node[above] {$\textbf{\textsf{RW}}$} (t4.west)
 (t3.east) edge node[right] {$\textbf{\textsf{WW}}$} (t4.south);
\end{pgfonlayer}
\end{tikzpicture}
\end{center}
...
because of the strong session guarantee $\sigma_S \in_1 \Sigma_{\CP}$. Because $\graphof(\aexec) = \G$, 
...
or $T_4 \xrightarrow{\AR_{\aexec}} T_1$. But if it were thelatter case, we would have 
$T_4 \xrightarrow{\AR_{\aexec}} T_1 \xrightarrow{\VIS_{\aexec}} T_2$. Recall that 
$\Sigma_{\CP}$ has the prefix consistency guarantee $(\rho_{\Id}, \rho_{V})$, which 
enforces that $\AR_{\aexec} \rcomp \VIS_{\aexec} \subseteq \VIS_{\aexec}$: this 
means that it must be the case that $T_4 \xrightarrow{\VIS_{\aexec}} T_2$. But 
...
last write wins policy, which leads to a contradiction; this contradiction was caused 
...
its execution kernel as the pair $(\VIS_{\G}, \AR_{\G})$, where: 
\[
\begin{array}{lcl}
\VIS_{\G} &=& \bigcap \{ \VIS_{\aexec} \mid \aexec \in \aeset(\Sigma) \wedge \graphof(\aexec) = \G\}\\
\AR_{\G} &=& \bigcap \{ \AR_{\aexec} \mid \aexec \in \aeset(\Sigma) \wedge \graphof(\aexec) = \G\}
\end{array}
\]

\section{Failure of Compositionality} 
\[
\begin{array}{lcl}
...
\section{Inexpressiveness of Kitten}
