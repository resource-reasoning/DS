...
\begin{proof}
    ...
    %\begin{rclarray}
        ...
        %\RW_{\mkvs} &\defeq& \{ (\txid, \txid') \mid \exists \key, i, j.\; \txid \in \rsOf(\mkvs(\key, i)) \wedge \txid' = \wtOf(\mkvs(\key, j)) \wedge i < j\}\\
        %\SO &\defeq& \{ (\txid, \txid') \mid \exists \cl, m, n.\; \txid = \txid_{\cl}^{m} \wedge \txid' = \txid_{\cl}^{n} \wedge m < n \}\\
        %\dagger & \equiv &
        %\begin{array}[t]{@{}l@{}}
               %\fora{\key, \key', i, j} 
            %\begin{array}{@{}l@{}}
            %i \in \vi(\key)  \wedge \wtOf(\mkvs(\key', j)) \toEDGE{((\SO \cup \WR_{\mkvs} \cup \WW_{\mkvs}) ; \RW_{\mkvs}\rflx)^+} \wtOf(\mkvs(\key, i))
            %\implies j \in \vi(\key') 
            %\end{array}
        %\end{array} \\
    %\end{rclarray}
