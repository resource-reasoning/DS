...
\begin{proof}
    ...
    %\begin{rclarray}
        ...
        %\AD_{\mkvs} &\defeq& \{ (\txid, \txid') \mid \exists \ke, i, j.\; \txid \in \RTx(\mkvs(\ke, i)) \wedge \txid' = \WTx(\mkvs(\ke, j)) \wedge i < j\}\\
        %\PO &\defeq& \{ (\txid, \txid') \mid \exists \cl, m, n.\; \txid = \txid_{\cl}^{m} \wedge \txid' = \txid_{\cl}^{n} \wedge m < n \}\\
        %\dagger & \equiv &
        %\begin{array}[t]{@{}l@{}}
               %\fora{\ke, \ke', i, j} 
            %\begin{array}{@{}l@{}}
            %i \in \vi(\ke)  \wedge \WTx(\mkvs(\ke', j)) \toEdge{((\PO \cup \RF_{\mkvs} \cup \VO_{\mkvs}) ; \AD_{\mkvs}?)^+} \WTx(\mkvs(\ke, i))
            %\implies j \in \vi(\ke') 
            %\end{array}
        %\end{array} \\
    %\end{rclarray}
    ...
    \begin{itemize}
        \item \( \Setcon{(\mkvs, \func{viewOf}{\mkvs,\ct}, \f, \func{viewOf}{\mkvs,\cts''(\cl)})}{\dagger} \).
            ...
            \[ 
                \WTx(\mkvs(\ke',j)) \toEdge{((\PO \cup \RF_{\mkvs} \cup \VO_{\mkvs}) ; \AD_{\mkvs}?)^+} \WTx(\mkvs(\ke,i))
            \]
