...
\begin{proof}
    ...
    by swapping the reduction order.
    ...
    \end{array}
    \]
    By \cref{lem:mono-client-clock-time}, the two clients are different \( \cl_i \neq \cl_j\) and the two steps are unique in the trace.
    We will construct a trace that \( \txid_{\cl_i}^n \) commits after \( \txid_{\cl_j}^m  \).
    First it is important to prove that \( \txid_{\cl_j}^m \) does not read any version written by \( \txid_{\cl_i}^n\).
    ...
    Let consider any possible time tick for those shard \( \sd \) that has been updated by \( \txid_{\cl_j}^m \),
    that is, \( \sd = \func{shardOf}{\ke}\) for some key \( \ke \)  that \( (\otW, \ke, \stub) \in \f_i \) and
    \begin{equation}
    \label{equ:time-tick-move-afterwards}
    \begin{array}{@{}l@{}}
        \tr = \cdots \toG{\cl_i,\ct_i,\f_i,n} \mkvs_i, \cts_i, \cts'_i, \thdenv_i, \prog_i \toG{\stub}  \cdots \toG{\sd,\ct} \stub \toG{\stub} \cdots \toG{\cl_j,\ct_j,\f_j,m} \mkvs_j, \cts_j, \cts'_j, \thdenv_j, \prog_j \\
    \end{array}
    \end{equation}
    It follows \( \ct_j < n < \ct \), and therefore such time tick will not affect the transaction \( \txid_{\cl_j}^m \),
    which means it is safe to move the time tick step after the \( \txid_{\cl_j}^m \).
    Now we can move the commit of \( \txid_{\cl_i}^n \) and any step similar to \cref{equ:time-tick-move-afterwards} after the commit of \( \txid_{\cl_j}^m \),
    \[
    \begin{array}{@{}l@{}}
    ...
    \begin{itemize}
        ...
        \end{array}
        \]
        where \( \f \addO (\otR, \ke, \val ) = \f\), then it is safe to swap the two steps as the following
        \[
        \begin{array}{@{}l@{}}
    ...
    \begin{itemize}
        \item if it is interleaved by the commit of other transaction \( \txid' \), that is
        \[
        \begin{array}{@{}l@{}}
        ...
        \]
        \begin{itemize}
            \item if the transaction \( \txid \) does not read from the shard \( \sd \),
                \[
                    \func{shardOf}{\ke} \neq \sd
                \]
            In this case, it is safe to swap the two steps
            ...
            \end{array}
            \]
            \item if the transaction \( \txid \) read from the shard \( \sd \),
                \[
                    \func{shardOf}{\ke} = \sd
                \]
                By the \rl{ReadRemote}, we know the current clock time for the shard \( \sd \) is greater than the snapshot time of \( \txid \), 
...
\begin{theorem}
    \cref{clock-si-view} produces well-formed view.
\end{theorem}


\begin{theorem}[Clock-SI satisfying SI]
    For any normal trace clock-SI trace \( \tr \), and any transaction \( \txid_\cl^{n} \),
    \[
        \tr = \cdots \toG{\cl, \ct, \f, \perp} \mkvs, \cts, \cts', \thdenv, \prog  \toG{\cl, \ct, \f, n} \mkvs', \cts'', \cts''', \thdenv', \prog'
    \]
