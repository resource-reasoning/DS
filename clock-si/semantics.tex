...
\begin{theorem}[Simulation]
    ...
    \[
        (\thdenv(\cl), \snapshot(\mkvs,\func{viewOf}{\mkvs,\ct}), \emptyset), \trans \toL^*
        (\stk, \h, \f), \pskip
    \]
...
\begin{proof}
    ...
    \[
        (\stk_0, \snapshot(\mkvs,\func{viewOf}{\mkvs,\ct}), \f_0), \trans_0 \toL^*
    ...
    \begin{itemize}
        ...
        %\begin{itemize}
            %\item Read from transaction: \rl{ReadTrans}.
                %In this case
                %\[
                ...
                %such that
                %\[
                    %\f_{i+1} = \f_i \addO (\otR, \ke, \val) 
                    %\land (\otW,\ke, \val') \in \f_i
                %\]
                %for some key \( \key \) and values \( \val \) and \( \val' \).
        %\end{itemize}
    \end{itemize}
\end{proof}
