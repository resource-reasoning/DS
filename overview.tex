...
which then propagates the state changes caused by an operation (if any) at the other one. Two clients 
...
The number of replicas in geo-replicated key-value stores may be very high; as a 
...
key-value store graphically in \cref{fig:counter_kv1}. A version is represented as a box sub-divided 
...
and install a newer value $1$ for the same key. The result of this operation is depicted in \cref{fig:counter_kv1}.
...
Next, a second client, dented as $\cl_2$, also attempts to perform the operation $\ctrinc$. Because 
...
a new version carrying value $2$ to be installed). In particular, if the former view is adopted by $\cl_2$, 
the final result will be the key-value store depicted in Figure \ref{fig:counter_kv1}. In this case, the 
...
To avoid the lost update anomaly, geo-replicated and distributed key-value stores 
...
To prevent the long-fork anomaly from happening, we can further strengthen the 
