...
This weak behaviour is known as the \emph{lost update} anomaly, disallowed under serialisability. 
...
%\footnote{In a distributed setting, this can happen if two clients access 
...
In \cref{...}, we prove that if the key-value store 
...
the one of $\ke_1$. 
In \cref{...}, we prove that if the key-value store consists of multiple counter objects, and the execution test employed by transactions guarantees SI, then the key-value store behaves as is it were strictly serialisable.
%
