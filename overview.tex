...
\sx{
    \begin{itemize}
        \item A good question from Emanuele: we need a name for our semantics.
        \item Running example: long-fork example.
            In \cref{fig:long-fork-code}, both \( \pvar{f1} \) and \( \pvar{f2} \) might be 1 if under parallel snapshot isolation.
        \item Multi-version Key-value, View. \cref{fig:key-value-store-and-view}.
        \item How does the semantics work on long fork example.
            A transaction initially takes a local snapshot provided by the current view,

        \item Mention base line is atomic visibility.
        \item Mention it is parametrised by consistency models.
        \item Mention one-to-one mapping between key-value stores and dependency graphs.
        \item Mention relation between key-value semantics with respect to abstract executions.
            \begin{itemize}
                \item Galois connection between a trace of key-value stores + views and abstract executions satisfying last write win.
                \item The specifications using execution tests are sound and complete with axiomatic ones.
            \end{itemize}
        \item A sketch proof for long fork.
            The transactional assertions are annotated with read/write tags.
            The box assertions describe the state of the views.
    \end{itemize}
}


\sx{
    Few works on C++ semantics after talked to John Wickerson
    \begin{itemize}
        \item Promising semantics
        \item Some Batty's work
        \item A Concurrency Semantics for Relaxed Atomics that Permits Optimisation and Avoids Thin-Air Executions 
        \item On Thin Air Reads Towards an Event Structures Model of Relaxed Memory
        \item  Brookes, An Operational Semantics for C/C++11 Concurrency, Pomset things.
    \end{itemize}
}
