...
{\em weaker} consistency models \cite{tango,CORFU,ramp,rola,cops,wren,redblue,PSI,NMSI,gdur,clocksi,distrsi,PSI-RA,si}. 
...
where the view of $cl_2$ is that in \cref{fig:counter_kv_view}. 
This test corresponds to what is known in the 
literature as \emph{write-conflict freedom}~\cite{.}, ensuring that at most one 
concurrent transaction can write to a key at any one time.
...
As we discuss in \cref{sec:applications}, although existing techniques in the literature can verify such robustness properties, they typically do so by examining \emph{full traces}.
...
This is because unlike the kv-stores in our operational semantics, existing techniques do not record the library \emph{state}; rather, they record full execution traces, making them less amenable for reasoning about such properties.
