...
\pg{In figure 1, say Example centralised key-value stores.}

\mypar{Centralised Operational Semantics}


\pg{This should be in the introduction or later in (new) section on
  comparison with execution test, not here: A well-known declarative approach for a range of consistency models
...
information about the transactions that accessed  it. \pg{now
  something about order of versions always know because of ....}
...
First, suppose that \(\cl_1\) invokes \(\ctrinc(\key)\) on
...
\pg{Maybe something here, maybe somewhere else.}
