...
\SI\, where transactions
appear to execute in a sequential (serial) order, one after another.
The implementation of  \SI\ in distributed kv-stores comes at a
...
%----better link with below, I'm not sure we need the bit at the beginning
%although it would be nice if this message was somewhere.......

%....... If you mean `better link from mentioning lost update and causal consistency to below':
%I think these sentences are for those experts who actually know what can happen for this counter example;
%It is good to mention so they have the mind set for the next bit, centralised operational semantics.
%..........
...
consistency models. 
Using our operational semantics, we can derive invariant of programs 
under a weak consistency model and
then prove invariant properties such as robustness, that is, the invariant has no weaker behaviours
than invariant obtained by execution the same programs under serialisability.
...
against our new model \WSI\ (\cref{sec:applications}).
...
\mypar{Verifying Implementation Protocol} 
...
COPS protocol \citep{cops} for implementing a replicated database satisfies {\em causal consistency}, 
and the Clock-SI protocol \citep{clocksi} for implementing a
partitioned database satisfies the consistency model called {\em snapshot isolation}. 

%... shale begin....
%We argue that states of distributed sites can be easily projected into our global centralised kv-store.
%Our notion of partial views allows us 
%to model clients and distributed sites that only have partial information of the overall database.
%... shale end....
...
%To verify implementation protocols,
%it boils down to verifying the views, for example that COPS explicitly tracks dependency relations between versions of keys,
%By modelling the distributed sites and clients into our framework,
