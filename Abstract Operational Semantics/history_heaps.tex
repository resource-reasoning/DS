...
\qquad
\hh' = \mathsf{HHupdate}(\hh, V, \mathcal{O}) \qquad \mathsf{ViewUpdate}(\hh, V, \mathcal{O}) \sqsubseteq V' \qquad 
...
where $\mathcal{V}, \mathcal{V}'$ are non-empty, finite sets of views,
and we often write $(\hh, \mathcal{V}) \leadsto_{CM} (\hh, \mathcal{V}')$ 
in lieu of $(\hh, \mathcal{V}, \hh', \mathcal{V}') \in CM$. 
\ac{Subtle error here, two threads may have the same view, so that 
multi-sets should be used rather than sets. I really don't like multisets.}
...
To this purpose, we define an enviroment $\mathsf{Env}$ to be 
...
\[
\infer{\langle \hh, \mathsf{Env}, \prog \rangle \rightarrow \langle \hh', \mathsf{Env}', \prog' \rangle}{}
\]

\ac{The part below must be rewritten, as the semantics of programs is not up-to-date with 
the rest of the document. Following Philippa's and Azalea's comments on wanting to 
preserve the invariant of a consistency model at each step, the rule for parallel composition 
