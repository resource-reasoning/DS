...
\newcommand{\versionList}{\ensuremath{\mathsf{VersionList}}}

\newcommand{\ruleATX}{\ensuremath{\mathit(A-Tx)}}
\newcommand{\kvTrace}{\ensuremath{\mathsf{KVtrace}}}
%\newcommand{\Clients}{\ensuremath{\mathsf{Clients}}}

\newcommand{\nextView}{\ensuremath{\mathsf{nextView}}}
\newcommand{\intermediateView}{\ensuremath{\mathsf{intermediateView}}}

\newcommand{\lastWrite}{\ensuremath{\mathsf{lastWrite}}}
\newcommand{\indexOf}{\ensuremath{\mathsf{index}}}
\newcommand{\getView}{\ensuremath{\mathsf{getView}}}
\newcommand{\absTrace}{\ensuremath{\mathsf{ATraces}}}
\newcommand{\transclient}{\ensuremath{\mathsf{clientOf}}}
\newcommand{\transmove}{\ensuremath{\mathsf{moveOf}}}
\newcommand{\progset}{\ensuremath{\mathbb{P}}}
...
In the following, it will be important to fix the structure of computations. 
...
\paragraph{Compatibility of Abstract Executions and Key-value Stores.}
...
\begin{itemize}
\item let $\mathcal{R}(\tsid_{0}, \key{k}) = \{ \tsid' \mid (\RD; \key{k}: \_ ) \in_{\aexec} \tsid' \wedge 
\RF(\key{k})^{-1}(\tsid') = \emptyset\}$; we define the version $\nu_{\aexec}(\tsid_{0}, \key{k}) = (0, \tsid_{0}, 
\mathcal{R}(\tsid_{0}, \key{k}) \}$; 
\item for each transaction $\tsid \in \T_{\aexec}$ such that $(\WR\;\key{k}:n) \in_{\aexec} \tsid$, 
let $\mathcal{R}(\tsid, \key{k}) = \{ \tsid' \mid \tsid \xrightarrow{\RF_{\aexec}(\key{k})} \tsid' \}$. 
Then we let $\nu_{\aexec}(\tsid, \key{k}) = (n, \tsid, \mathcal{R}(\tsid, \key{k})$; 
\item For each transaction $\tsid \in \T_{\aexec}$ and key $\key{k}$, 
we define the set 
$\Versions_{\aexec}(\key{k}) = \{ \nu_{\aexec}(\tsid, \key{k}) \mid \tsid = \tsid_{0} \vee (\WR\;\key{k}:\_) \in_{\aexec} \tsid \}$;
%\begin{enumerate}
...
%\end{enumerate}
\item for each key $\key{k}$ and set of transactions $\T$, define the list of versions 
$\versionList'_{\aexec}(\key{k}, \T)$ inductively as follows: 
\[
\versionList'_{\aexec}(\key{k}, \T) = 
\begin{cases}
\varepsilon & \text{if } \T \cap \{ \tsid \mid (\WR\; \key{k}: \_) \in_{\aexec} \tsid \} = \emptyset\\
\nu_{\aexec}(\tsid, \key{k}) \cdot \versionList_{\aexec}(\key{k}, \T \setminus \{ \tsid \}) & \text{if } 
\tsid = \min_{\VO(\key{k})}(\T)
\end{cases}
\]
then we let $\versionList_{\aexec}(\key{k}, \T) = \nu_{\aexec}(\tsid_0, \key{k}) \cdot 
\versionList'_{\aexec}(|key{k}, \T)$ 
\ac{this last bit made me feel more like doing programming rather than mathematics.} 
\item The key-value store $\hh_{\aexec}$ is defined as 
$\lambda \key{k}. \versionList_{\aexec}(\key{k}, \T_{\aexec})$.
%to be the unique one such that 
%for any key $\key{k} \in \Addr$, $\nu \in \hh_{\aexec}(\key{k})$ if and only if 
%$\nu \in \Versions_{\hat{\aexec}}(\key{k})$; furthermore, for any $\nu, \nu' \in \hh_{\aexec}(\key{k})$, 
%we have that $\nu <_{\hh_{\aexec}(\key{k})} \nu'$ if and only if $\nu = (\_, \tsid, \_)$, 
...
\begin{definition}
Let $\aexec$ be an abstract execution. Let $\T \subseteq \key{k}$, 
Define 
\begin{itemize}
\item $\lastWrite(\T, \key{k}) = \tsid_0$ if 
$(\T \cap \{\tsid \mid (\WR; \key{k}: \_) \in_{\aexec} \tsid \}) = \emptyset$, 
otherwise $\lastWrite(\T, \key{k}) = \max_{\AR}(\T \cap \{\tsid \mid (\WR\;\key{k}: \_) \in_{\aexec} \tsid \})$.
\item $\version(\T, \key{k}) = \nu_{\aexec}(\lastWrite(\T, \key{k}), \key{k})$, 
\item let $i$ be the only index such that $\hh_{\aexec}(\key{k}, i) = \version(\T, \key{k})$\footnote{
We know that at least one such index exists and is unique from the definition of $\Versions_{\aexec}(\key{k})$}.
We define $\indexOf(\T, \key{k}) = i$.
\item $\getView(\T) := \lambda \key{k}. \indexOf(\T, \key{k})$.
\end{itemize}
\end{definition}
...
%Let also $\nu_{\T}^{\key{k}} = \nu_{\aexec}(\tsid_{\T}^{\key{k}}, \key{k})$, which we know to exist 
%and be included in $\hh_{\aexec}(\key{k})$ from Definition \ref{def:absexec2hh}. 

%Therefore, there exists a unique index $i_{\T}^{\key{k}} = 0,\cdots, \lvert \hh_{\aexec}(\key{k}) \rvert - 1$ 
%such that $\hh_{\aexec}(\key{k}, i_{\T}^{\key{K}}) = \nu_{\T}^{\key{k}}$. 
%be the unique index 
...
\begin{proof}[Proof of Proposition \ref{prop:aexec2hh}]
...
%For simplicity, for any key $\key{k}$, we let $i_{\key{k}} = V(\key{k})$.
...
%In particular, either $i_{\key{k}} = 0$ and 
%$\hh_{\aexec}(\key{k}, V(\key{k})) = \nu_{\tsid_0}^{\key{k}}$, 
%$i_{\key{k}} > 0$, and 
%$\hh_{\aexec}(\key{k}, i_{\key{k}}) = \nu_{\aexec}(\tsid_{V}^{\key{k}}), \key{k})$ 
%for some transaction $\tsid_{V}^{\key{k}} \in \T_{\aexec}$. 
%$\nu_{\aexec}(\tsid_V^{\key{k}}, \key{k}) = \hh_{\aexec}(\key{k}, V(\key{k})$. 
%By definition of $\nu_{\aexec}(\tsid_V^{\key{k}}, \key{k})$, and because $i_{\key{k}} > 0$, 
%we have that 
%$\hh_{\aexec}(\key{k}, V(\key{k})) = (n, \tsid_{V}^{\key{k}})$, 
...
%$V(\key{k}) = i_{\key{k}}$, 
%and $f(\T_{V})(\key{k}) = i_{\T_{V}}^{\key{k}}$. 
...
\]
%is a trace in the abstract execution semantics. 
%Formally, we define the set of abstract execution traces as the smallest set $\Atraces$ such that: 
%\[
%\begin{array}{l}
%\forall \aexec, \thdStackFun, \prog. \langle \aexec, \thdStackFun, \prog \rangle \rangle \in \Atraces\\
%%\mathsf{head}(\langle \aexec, \thdstackFun, \prog\rangle ) = \langle \aexec, \thdstackFun, \prog \rangle\\
%%~\\
%\forall \aexec, \thdstackFun, \prog, \alpha. \langle \aexec, \thdstackFun, \prog \rangle 
%\absrightarrow_{\CM} \alpha \in \Atraces
%\end{array}
%\]
%In the following, we will often need to perform a case analysis on the 
%structure of a trace. To this end, we define the partial composition operator $\cdot$ 
%so that $\alpha_1 \cdot \alpha_2$ is defined if and only if the last state of $\alpha_1$ coincides 
%with the first state of $\alpha_1$, i.e. 
%\[
%\begin{array}{l}
%\alpha_1 = \alpha_1' \absrightarrow_{\CM} \langle \aexec, \thdstackFun, \prog \rangle \\
%\alpha_2 = \langle \aexec, \thdstackFun, \prog \rangle \absrightarrow_{\CM} \alpha_2'\\
%\alpha_1 \cdot \alpha_2 = \alpha_1 \absrightarrow_{\CM}
%\]
...
\ac{Main Idea: translate abstract execution into compatible history heaps, as per 
