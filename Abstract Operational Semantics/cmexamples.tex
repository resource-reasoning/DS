...
literature.
\ac{There is still a long-way to go before proving correspondence with axiomatic specifications,  
...
\trans_1^2 = \begin{array}{c} 
     ...
     \end{array}
\]
in the client-local configuration of $\mathcal{C}'$ relative to $\tid_2$, which is obtained 
...
views is up-to-date with respect to some transaction $\tsid$ for some location $\key{k}_1$, 
but not for some other location $\key{k}_2$. This leads to the following definition.
\begin{definition}
...
Let $V$ be  view, $\hh$ be a MKVS, $\tsid$ be a transaction identifier. 
We say that $V$ \emph{sees} transaction $\tsid$ in $\hh$, written 
$\mathsf{Visible}(\tsid, \hh, V)$, iff 
\[
\forall \key{k}.\;\forall i=0,\cdots, \lvert \hh([\loc_x]) \rvert.\; \WTx(\hh(\key{k}, i)) = 
\tsid \implies i \leq V(\key{k}).
\]
...
visibility and the history heap $\hh$,  written $\mathsf{Atomic}(\hh, V)$, 
if
\[
\forall \tsid. \left( \exists\;\key{k}. \exists i \leq V(\key{k}).\; \WTx(\hh(\key{k}, i)) = \tsid 
\right) \implies \mathsf{Visible}(\tsid, \hh, V).
\]
...
\[
\mathsf{Atomic}(\hh, V) \implies \ET_\RA \vdash  (\hh, V) \triangleright \_: \_
\]
...
Suppose that we execute the program $\prog_1$ using the execution test $\ET_{\RA}$.
We can proceed as in Section \ref{sec:semantics.example} to infer the transition 
$\langle \mathcal{S}_0, \prog_1 \rangle \rightarrow_{\ET_{\RA}} \langle \mathcal{S}_1, \prog_1' \rangle$, 
where we recall that $\mathcal{S}_0$, $\mathcal{S}_1$ are depicted in Figure \ref{fig:opsem.exampe}(a), 
...
is the following: 
\begin{itemize}
\item first, push the view $V$ of client $\tid_2$ in the configuration 
$\mathcal{C}_1$ of Figure \ref{fig:opsem.example}(b) to observe the update of key $\key{k}_1$, but not the update of 
$\key{k}_2$. This view is the one labelled with $\tid_2$ in Figure \ref{fig:opsem.example}(c), and we refer 
to it as $V'$;
\item then, execute the transaction $\ptrans{\trans}$ in $\tid_2$. 
\end{itemize}

However, this second step cannot be performed if the execution test $\ET_{\RA}$ is used. 
In fact, the view $V'$ is not atomic, according to Definition \ref{def:readatomic}, and therefore, 
client $\tid_2$ cannot execute transaction $\ptrans{\trans}$ using the snapshot extracted from view 
$V'$. A consequence of this fact is that, under the execution test $\ET_{\RA}$ there exists no execution 
of $\prog_1$ in which the transaction $\ptrans{\trans}$ returns value ${\Large \frownie}$. 

One may wonder whether it is the case that an execution of a program may get stuck because 
of a ill-defined execution test. For example, for a given execution test $\ET$, 
it may be possible to reach a state $\mathcal{S}$ 
of the system, with MKVS $\hh$ and view $V$ for client $\tid$ $V$, 
such that for any $V': V \sqsubseteq V'$ we have that $\ET_{\RA} \not\vdash 
(\hh, V') \triangleright \_ : \_$. We show that this is not the case
for the execution test $\ET_{\RA}$. 

\begin{proposition}
\label{prop:ra.progress}
The execution test $\ET_{\RA}$ \emph{does not hinder progress}: 
...
for any MKVS $\h$, views $V$, and set of operations $\mathcal{O}$, 
there exist two views $V': V \sqsubseteq V'$, $V'': \Vupdate(\hh, V', \mathcal{O}) \sqsubseteq V''$ 
such that $\ET_{\mathsf{RA}} \vdash (\hh, V) \triangleright \mathcal{O} : V'$.
\end{proposition}

\begin{proof}[Proof Sketch]
For a given MKVS $\hh$, let $\mathsf{up-to-date}(\hh) = \lambda \key{k}. \lvert \hh \rvert$ be the 
view that points to the most recent version of each object. It is immediate to note that $\mathsf{Atomic}(\hh, 
\mathsf{up-to-date}(\hh))$, and therefore $\ET_{\RA} \vdash (\hh, \mathsf{up-to-date}(\hh, V)) \triangleright \_ : \_$. 
\end{proof}
%
...
\hline
\end{tabular}
\caption{Configurations obtained in a execution of $\prog_2$.}
\label{fig:cc.exec}
\end{figure}
...
It is easy to see that, if no constraints are placed on the execution test
(i.e if the execution test $\ET_{\top}$ from Section \ref{sec:opsem.example} is used, 
then we can have $\ptrans{\trans_3}$ return ${\Large \frownie{}}$. 
The same is true even if the execution test $\ET_{\RA}$ is used. 
...
initial (and only) version for each key; after this transaction is 
executed, a new version $\langle 1, T_1, \emptyset \rangle$ is appended 
at the end of $\hh(\loc_{x})$. The resulting configuration is depicted in Figure \ref{fig:cc.exec}(b).
%\begin{center}
%\begin{tikzpicture}[font=\large]
%
%\begin{pgfonlayer}{foreground}
...
%\end{tikzpicture}
%\end{center}
\item next, $\tid_2$ updates its view as to see the version of $\key{k}_2$ installed by $\tid_1$, after 
which it proceeds to execute $\ptrans{\trans_2}$. This view is the one labelled with $\tid_2$ in Figure 
\ref{fig:cc.exec}(c); according to this view, the snapshot under which $\ptrans{\trans_2}$ is 
executed is $[\key{k}_1 \mapsto 1, \key{k}_2 \mapsto 0]$. The code $\ptrans{\trans_2}$ 
copies the value of $\key{k}_1$ into $\key{k}_2$, hence after the transaction is executed, 
a new version for $\key{k}_2$ with value $1$ will be created.
%is executed, This results in a new version with value $1$ 
%to be installed for $\loc_y$. 
The configuration obtained after the execution of $\ptrans{\trans_2}$ is 
given in Figure \ref{fig:cc.exec}(d).
%The configurations before, and after the execution of $\ptrans{\trans_2}$, 
...
%\end{tikzpicture}
%\end{center}
\item Finally, client $\tid_3$ updates its view to observe the update of location $\key{k}_2$, but not the update of 
location $\key{k}_1$, before executing transaction $\ptrans{\trans_3}$. This view is the one labelled $\tid_3$ 
in Figure \ref{fig:cc.exec}(e). 
The transaction $\ptrans{\trans_3}$ is executed 
using the snapshot $[\key{k}_1 \mapsto 0, \key{k}_2 \mapsto 1]$, 
which causes the local variable $\retvar$ of $\tid_3$ to be set to 
%The execution of $\ptrans{\trans_3}$ will 
%return the value 
${\Large \frownie{}}$. The configuration obtained after $\ptrans{\trans_3}$ is executed 
is depicted in Figure \ref{fig:cc.exec}(f).
...
state where its view observes the update to location $\key{k}_2$, but 
not the update to location $\key{k}_1$. However, latter directly depends on the 
former, in the sense of Definition \ref{def:ddep}. 
...
In other words, the execution of transaction $\ptrans{\trans_3}$ resulted in a violation of 
causality: the update of $\key{k}_2$ is observed, but not the update of $\key{k}_1$ on which 
it depends.
...
\ac{Note to self: why not going for a coinductive definition? The set of 
causally consistent views is the largest set such that whenever $V$ is 
causally consistent w.r.t. $\hh$, then for each version $\nu, \nu' \in \hh$, 
such that $\nu'$ depends on $\nu$, then if $\nu'$ is included in $V$ also 
$\nu$ is included in $V$.}
...
to include a newer version $\nu$, then we must ensure that all the versions on 
which $\nu$ directly depends in $\hh$ are already included in the view.


\begin{definition}
%Let $\nu_1 = (n_1, \tsid_1, \mathcal{T}_1)$, $\nu_2 = (n_2, \tsid_2, \mathcal{T}_2)$. 
...
Let $\hh$ be a MKVS. The set of views that are \emph{causally consistent} 
with respect to $\hh$, $\mathsf{CCViews}(\hh)$, is defined as the smallest set such that: 
\begin{enumerate} 
\item $V_0 := \lambda \key{k}.0 \in \mathsf{CCViews}(\hh)$, 
\item let $V \in \mathsf{CCViews}(\hh)$, $V' = V[\key{k} \mapsto (V(x) + 1)]$, 
%for some $i: V([\loc_x]) < i \leq \lvert \hh \rvert -1$
for some $\key{k}$ such that $V(\key{k}) < \lvert \hh(\key{k}) \rvert$.
Suppose that 
\[
\forall \key{k}'.\;\forall j = 0,\cdots, \lvert \hh(\key{k}') \rvert .\; 
\hh(\key{k}', j) \xrightarrow{\mathsf{ddep}} \version(\hh, \key{k}, V') 
\implies j \leq V(\key{k'}).
\]
...
\path (locy.south) + (0,-1.5) node (locy) {$\key{k}_3 \mapsto$};
...
\path (locy.south) + (0,-1.5) node (locz) {$\key{k}_3 \mapsto$};
\matrix(loczcells) [version list, text width=7mm, anchor=west]
...
intuitive after all...}
Consider the MKVS $\hh$, depicted in Figure \ref{fig:cc.view}(a) 
...
%\end{tikzpicture}
%\end{center}
We want to find a view $V$ that points to the latest version of key $\key{k}_3$ in 
$\hh$, i.e. $\hh(\key{k}_3, 2)$; that is, we require that $V(\key{k}_3) = 2$.
%that is, $V[\loc_z] = 1$, and $\hh([\loc_z])(V[\loc_z]) = (1, \tsid_3, \emptyset)$.
Furthermore, we want the view $V$ to be causally consistent. 

We construct the view $V$ incrementally, as outlined below. 
Note that for the MKVS $\hh$, we have that 
$\hh(\key{k}_3, 2) \xrightarrow{\mathsf{ddep}} \hh(\key{k}_2, 2)$, 
and $\hh(\key{k}_2, 2) \xrightarrow{\mathsf{ddep}} \hh(\key{k}_1, 2)$.

We start from the initial view $V_0$, $V_0(\key{k}_{\_}) = 0$. This view is 
depicted in Figure \ref{fig:cc.view}(a), and it is 
causally consistent by definition. However, it does not point to the last version for $\key{k}_3$: in
 fact, $V_0(\key{k}_3) = 1$. As a first try, one could immediately consider a view $V'$ 
where the value for location $\key{k}_3$ is updated to $1$, 
that is $V' = V_0[ \key{k}_3 \mapsto 2]$. However, in this case 
the view $V'$ is not causally consistent. In fact, 
we have that $\version(\hh, \key{k}_3, V') = (1, \tsid_3, \emptyset) \xrightarrow{\mathsf{ddep}} 
(1, \tsid_2, \{\tsid_3\}) = \version(\hh,\key{k}_2, V')$, but $V'(\key{k}_2) = 1 < 2$. 
That is, the version of $\key{k}_2$ observed by $V'$ directly depends on a version 
of $\key{k}_2$ that is not observed by $V'$.
%
...
$\key{k}_2$ to value $1$, resulting in the value $V'' = V_0[ \key{k}_2 \mapsto 2]$, 
but similarly we would find that the version 
$\hh(\key{k}_2, 2) = (\_, \tsid_2, \_)$ directly depends from the version 
$\hh(\key{k}_1, 2) = (\_, \_, \{\tsid_2\})$, and $V''(\key{k}_1) = 1 < 2$. 

Finally, we can update the view $V_0$ to include the update of $\key{k}_1$: 
this results in the view $V_1 = V_0[ \key{k}_1 \mapsto 2]$. Because $\hh(\key{k}_1, 2) 
= (\_, \tsid_1, \_)$, and there is no version $\nu$ in $\hh$ on which 
$\hh(\key{k}_1, 1)$ directly depends - i.e. such that  
...
%\end{tikzpicture}
%\end{center}
We can now update the view $V_1$ to include the version $\hh(\key{k}_2, 2)$, 
resulting in the view $V_2 := V_1[\key{k}_2 \mapsto 2]$, depicted in Figure \ref{fig:cc.view}(c). 
Because the view $V_2$ is 
causally consistent, and because all the versions on which $\version(\hh, \key{k}_2, V_2)$ directly depends
are observed by $V_1$, then $V_2$ is also causally consistent. Similarly, we can define 
the view $V_3 := V_2[\key{k}_2 \mapsto 2]$, depicted in Figure \ref{fig:cc.view}(d), 
and prove that it casually consistent.  Because $V_3$ points to $\hh(\key{k}_3, 2)$, there is nothing 
left to do.
...
\ac{Note: It appears to be that causal consistency is broken (and so is PSI). 
The problem is that causalities between program dependencies are not tracked down, 
nor they can be tracked down in the MKVS model as it is now. For example, if we execute 
the program 
\[
\begin{session}
	\begin{array}{c||c}
		...
		\begin{session}
			\begin{transaction}
				\pderef{\pvar{x}}{\key{k}_1};\\
				\pderef{\pvar{y}}{\key{k}_2};\\
				\pifs{ \pvar{x} = 0 \wedge \pvar{y} = 1}\\
					\;\;\;\;\passign{\retvar}{{\Large\frownie{}}} \;:
		 	\end{transaction}
		\end{session}
	\end{array}
\end{session}
\]
under causal consistency, we can still obtain an execution where the value ${\Large \frownie{}}$ 
is assigned to $\retvar$. The reason why this happened is a violation of the \emph{monotonic writes} 
session guarantee (assuming clients are wrapped within sessions.)\\
I finally managed to get hold to the session guaranteee paper, and it looks that the MKVS semantics 
only ensures monotonic reads and read my writes. Monotonic writes and write follows reads are 
not ensured by default. It also appear that we cannot model these last two session guarantees 
without recording the program order in clients. Note that if we change the model so that 
MKVSs keep track of the program order of clients, then the isomorphism with dependency graphs 
is back, and we should point out that any dependency graph specification can be converted into 
an execution test easily.
}
...
We model causal consistency by introducing the execution test $\ET_{\CC}$, 
defined below.

\begin{definition}
$\ET_{\CC} \vdash (\hh, V) \triangleright \mathcal{O} : V'$ if and only if 
$\ET_{\RA} \vdash (\hh, V) \triangleright \mathcal{O}: V'$, 
and $V \in \mathsf{CCViews}(\hh)$.
\end{definition}
\ac{
Update to note to self: we do need to track down the program order of clients, and introduce 
strong sessions (maybe just monotonic writes could be enough). See previous note.\\
Note to self: here there is something subtle going on. We also need to ensure that dependencies 
...
because $V(\key{k}_2) = 1$, and $V(\key{k}_1) = 0$. However, $\hh(\key{k}_2, 2)$ directly depends 
in $\hh(\key{k}_1,2)$, which is not included in the view. 
...
by a non-causally consistent view. There exists no execution of $\prog_2$ using the execution test $\ET_\mathsf{CC}$ 
that causes $\ptrans{\trans_3}$ to return value ${\Large \frownie{}}$.
...
%Location y
\path (locx.south) + (0,-1.5) node (locf1) {$\key{f}_1 \mapsto$};
...
%Location y
\path (locf1.south) + (0,-1.5) node (locf2) {$\key{f}_2 \mapsto$};
...
conflicts. The informal specification of Update Atomic states that 
...
\[
        ...
        \begin{array}{@{}c || c || c@{}}
            \begin{transaction}
              ...
              \begin{transaction}
            	  ...
            	  \end{transaction} &
              \begin{transaction}
            		   \pderef{\pvar{a}}{\key{f}_{1}};\\
            		   \pderef{\pvar{b}}{\key{f}_{1}};\\
            		   \pderef{\pvar{c}}{\key{k}};\\
    ...
    \end{session}
 \]
\ac{Intuitive behaviour of the litmus test: two transactions concurrently increment $\key{k}$. 
...
We consider an execution in which the transactions contained in the code of clients 
...
for key $\key{k}$. Finally, thread $\tid_3$ updates its view to include the most recent 
version for all the keys $\key{k}, \key{f}_1, \key{f}_2$. When executing the 
...
a view which did not include the most up-to-date version for location $\key{k}$. 
However, the same transaction installed a new version for $\key{k}$. That is, 
...
writes to some key $\key{k}$, then it must have been executed 
in a snapshot obtained from a view pointing to the most recent version of 
$\key{k}$.

\begin{definition}
$\ET_\UA \vdash (\hh, V) \triangleright \mathcal{O}: V'$ iff 
$\ET_\RA \vdash (\hh, V) \triangleright \mathcal{O}: V'$, and 
for all locations $\key{k}$ such that $(\WR\; \key{k} : \_ \in \mathcal{O})$ 
then $V(\key{k}) = \lvert \hh(\key{k}) \rvert$. 
\end{definition}

\begin{proposition}
The execution test $\ET_{\UA}$ does not hinder progress. 
For any $\hh, V, \mathcal{O}$, there exist $V' : V \sqsubseteq V'$ 
and $V'': \Vupdate(\hh, V', \mathcal{O}). \sqsubseteq V''$ such that 
$\ET_{\UA} : (\hh, V') \triangleright \mathcal{O}, V''$.
\end{proposition}

Note that in $prog_3$, under $\UA$, 
...
not include the most recent version for location $\key{k}$. 
Instead, before executing its transaction $\tid_2$ must update 
its view to include the most recent version of $\key{k}$. The 
...
for the three keys $\key{k}, \key{f}_1$ and $\key{f}_2$. 
In this case the transaction will not return value ${\Large \frownie{}}$, 
\item one in which the view of $\tid_3$ for $\key{k}$ points to the 
version $(1, \tsid_1, \{\tsid_2\})$. Because of atomic visibility, 
it must also be the case that the same view for key $\key{k}_2$ 
does not include the update performed by $\tsid_2$ (otherwise, 
the view for $\key{k}$ should point to its most recent version, which 
was also installed by $\tsid_2$. Thus in this case the transaction executes 
w.r.t a snapshot where $\key{f}_2 = 0$, and the value ${\Large \frownie{}}$ 
will not be returned, 
\item one in which the view of $\tid_3$ for $\key{k}$ points to 
...
\[
\begin{array}{cc}
\ptrans{\trans_1} = \ptrans{\pmutate{\key{k}_1}{1}} & 
\ptrans{\trans_2} = \begin{transaction} 
                                ...
                                \end{transaction}
                             \\
\ptrans{\trans_3} = \ptrans{\pmutate{\key{k}_2}{1}} & 
...
We argue that, using either the execution tests $\ET_\RA$ or $\ET_{\CC}$, it is possible to obtain an execution 
...
The code $\trans_2$ is executed using  $[\key{k}_1 \mapsto 1, \key{k}_2 \mapsto 0]$ 
...
In the MKVS framework, transactions are executed in a single step; 
...
consistency models). By requiring that, after a client $\tid$ executes 
...
\begin{definition}
We say that $\CP \vdash (hh, V) \triangleright \mathcal{O}: V'$ 
if and only if $\RA \vdash (\hh, V) \triangleright \mathcal{O}: V'$, 
and for any location $\key{k}$, $V'(\key{k}) = \lvert \HHupdate(\hh, V, \mathcal{O})(k) \rvert $. 
\end{definition}

Consider again the program $\prog_4$, this time to be executed 
...
the one depicted in Figure \ref{fig:cp.exec}(a). Initially, either client $\tid_1$ executes 
the code $\ptrans{\trans_1}$, or client $\tid_2$ executes transaction $\ptrans{\trans_3}$; without 
...
to be executed next.
At this point, note that if we use the execution test $\ET_\CP$ it is not possible for $\tid_2$ to execute $\ptrans{\trans_3}$ and obtain 
the configuration of Figure \ref{fig:cp.exec}(c) as a result. This is because, $\ET_{\CP}$ require the view of $\tid_2$ 
\textbf{after} executing $\ptrans{\trans_3}$ to be up-to-date, i.e. to point to the last version of each key. That is, 
...
location $\key{k}$, hence it won't be able to set the value of $\retvar$ to ${\Large \frownie{}}$. One possible 
...
geo-replicated and distributed systems \cite{NMSI}, a weakening of this model 
called \emph{Parallel Snapshot Isolation} (PSI) \cite{PSI} has been recently proposed. 
...
consistent prefix (if a transaction $\tsid_1$ 
...
% ([xshift=-2pt, yshift=5pt]locy-v0.north east) -- 
 ...
 \path (tid1start) node[anchor=south, rectangle, fill=blue!20, draw=blue, font=\small, inner sep=1pt] {$\tsid_1$};
...
If we execute the program $\prog_5$ using any of the execution tests
 presented so far, we find out that it is possible to infer 
an execution in which both clients $\tid_1$ and $\tid_2$ set their 
local variable $\retvar$ to value ${\Large \frownie{}}$.
For example, under $\mathsf{SI}$, this can happen as follows: 
\begin{itemize}
\item first, client $\tid_1$ executes its transactions in the 
configuration of Figure \ref{fig:ser.exec}(a). The snapshot 
under which the transaction is executed is given by $[\key{k}_1] \mapsto 0, \key{k}_2 \mapsto 0]$, 
...
view $[\key{k}_1 \mapsto 0, \key{k}_2 \mapsto 0]$. The snapshot under which 
the transaction is executed is again $[\key{k}_1 \mapsto 0, \key{k}_2 \mapsto 0]$. After 
the transaction has been executed, the local variable $\retvar$ of $\tid_2$ is 
set to ${\Large \frownie{}}$, and the final configuration is the one of Figure \ref{fig:ser.exec}(c). 
Note that, because we are assuming snapshot isolation as our consistency model, in this configuration 
...
\begin{definition}
$\ET_{\SER} \vdash (\hh, V) \triangleright \mathcal{O} : V'$ iff, for any location $\key{k}$, 
$V(\key{k}) = \lvert \hh(\key{k}) \rvert$. 
\end{definition}

Consider again the program $\prog_5$, this time to be executed using the execution test $\ET_\SER$. 
Similar as for the execution test $\ET_\SI$, after client $\tid_1$ has executed its transaction, 
...
not execute the transaction of client $\tid_2$ without updating his view beforehand. 
This is because such a view does not point to the most up-to-date version for key  
$\key{k}_1$. Instead, before executing its transaction $\tid_2$, updates its view to 
point to the most up-to-date version of each location, Figure \ref{fig:ser.exec}(d). 
Thus, client $\tid_2$ will execute its transaction using the snapshot $[\key{k}_1 \mapsto 1, [\key{k}_2] \mapsto 0]$. 
A consequence of this fact is that the thread-local variable $\retvar$ of $\tid_2$ will not be 
set to ${\Large \frownie{}}$, and no new version for location $\key{k}_2$ will be created 
